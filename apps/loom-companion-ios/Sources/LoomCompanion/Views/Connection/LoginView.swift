        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // Admin Token — separate from the bearer, needed only for
                    // Mills mutations (spinning a plan, escalating a pipeline).
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Admin Token (Optional)")
                            .font(.subheadline)
                            .fontWeight(.medium)

                        SecureField("HUD admin token", text: $viewModel.adminTokenInput)
                            .textFieldStyle(.roundedBorder)
                            .autocorrectionDisabled()
                            #if os(iOS)
                            .textInputAutocapitalization(.never)
                        ...
                        Text("Only needed to spin plans or escalate pipelines from the phone. This is HUD_ADMIN_TOKEN — not the bearer above. You can also add it later under Connection settings.")
                            .font(.caption)
                            .foregroundStyle(LoomColors.fgSecondary)
                    }
