...
struct ConnectionDiagnosticsView: View {
    ...
    /// Set/replace/clear the HUD admin token in place. The pairing bearer only
    /// authenticates reads; Mills mutations (spin, escalate) need the separate
    /// admin token, so surface it here for an already-connected operator instead
    /// of forcing a full re-pair. Saved to the Keychain; the next
    /// `buildAPIClient()` attaches it as `X-Admin-Token`.
    private var millsAdminTokenBlock: some View {
        VStack(alignment: .leading, spacing: LoomSpacing.sm) {
            Label("Mills Admin Token", systemImage: "key.horizontal")
                .font(LoomTypography.headlineMedium)

            Text(connectionVM.hasAdminToken
                ? "An admin token is set — spinning plans and escalating pipelines are enabled."
                : "Not set. Mills reads work, but spinning a plan or escalating a pipeline needs the HUD admin token (HUD_ADMIN_TOKEN — separate from the bearer).")
                .font(LoomTypography.caption)
                .foregroundStyle(LoomColors.fgSecondary)

            SecureField(
                connectionVM.hasAdminToken ? "Replace admin token" : "HUD admin token",
                text: $connectionVM.adminTokenInput
            )
            ...
            HStack(spacing: LoomSpacing.sm) {
                Button {
                    connectionVM.saveAdminToken()
                } label: {
                    Label("Save Admin Token", systemImage: "checkmark.circle")
                }
                .buttonStyle(.borderedProminent)
                .tint(LoomColors.info)
                .disabled(
                    connectionVM.adminTokenInput
                        .trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                )

                if connectionVM.hasAdminToken {
                    Button(role: .destructive) {
                        connectionVM.adminTokenInput = ""
                        connectionVM.saveAdminToken() // empty input clears it
                    } label: {
            ...
            if let status = connectionVM.adminTokenStatus, !status.isEmpty {
