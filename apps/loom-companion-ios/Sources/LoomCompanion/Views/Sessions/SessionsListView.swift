    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
            List {
                ...
                // The context sessions below are loom's ENVELOPES (entries,
                // tasks, telemetry). The transcripts browser is the real
                // conversations — the vendor-session bridge's mirror of what
                // the CLIs actually said and did.
                NavigationLink {
                    TranscriptsListView(apiClient: apiClient as? APIClient)
                } label: {
                    HStack(spacing: 8) {
                        ...
                        Text("what the CLIs actually did")
            ...
            .onChange(of: viewModel.statusFilter) { _, _ in
                // The status filter changes what the SERVER returns (it
                // defaults to active-only), so a refetch is required.
                Task { await viewModel.load() }
            }
