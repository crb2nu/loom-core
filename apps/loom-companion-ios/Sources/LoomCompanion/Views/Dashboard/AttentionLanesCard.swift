...
struct AttentionLanesCard: View {
    ...
    private var groups: [AttentionGroup] {
        ...
        for lane in visibleLanes {
            // Stalled MRs share `type: "merge"` with merge-ready lanes but are
            // their semantic opposite (Kit resolves them via target_kind) —
            // bucket them under a synthetic key so they never collapse into
            // "Merge-ready branches".
            let key: String
            if lane.kind == .mergeStalled {
                key = AttentionGroup.mergeStalledKey
            } else {
                key = lane.type.isEmpty ? "_other" : lane.type
            }
    ...
    private func laneIcon(_ type: String) -> String {
        ...
        if type == AttentionGroup.mergeStalledKey { return AttentionLaneKind.mergeStalled.rowIcon }
...
private struct AttentionGroup: Identifiable {
    ...
    static func typeTitleIfKnown(for type: String) -> String? {
        ...
        if type == mergeStalledKey { return AttentionLaneKind.mergeStalled.aggregateTitleIfKnown }
        return AttentionLaneKind(typeKey: type).aggregateTitleIfKnown
    }

    /// Synthetic group key for stalled-MR lanes (`type: "merge"` +
    /// `target_kind: "merge_request"`) so they never share the merge-ready
    /// bucket. Never collides with backend types: the lane contract's type
    /// vocabulary contains no ':'.
    static let mergeStalledKey = "merge:mr"
}
