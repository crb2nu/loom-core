...
struct AgentDetailView: View {
    ...
    private func sessionSummary(_ session: SessionInfo) -> some View {
        HStack(spacing: LoomSpacing.md) {
            ...
            statPill(label: "Tokens", value: LoomFormat.compact(session.totalTokens))
    ...
    private var pipelineSection: some View {
        VStack(alignment: .leading, spacing: LoomSpacing.xs) {
            ...
            ForEach(pipelines) { pipeline in
                HStack(spacing: LoomSpacing.sm) {
                    StatusAccentBar(color: LoomColors.pipelineStatusColor(pipeline.status))
                    VStack(alignment: .leading, spacing: 2) {
                        HStack(spacing: 6) {
                            Text(LoomFormat.lastPathComponent(pipeline.project))
                                .font(LoomTypography.bodyMedium)
                                .foregroundStyle(LoomColors.textPrimary)
                                .lineLimit(1)
                            StatusBadge(pipeline.status, color: LoomColors.pipelineStatusColor(pipeline.status))
                        }
                        HStack(spacing: 4) {
                            Text(pipeline.ref)
                                .font(LoomTypography.monoCaption)
                        ...
                        if pipeline.totalStages > 0 {
                            ProgressView(value: Double(pipeline.completedStages), total: Double(pipeline.totalStages))
                                .tint(LoomColors.pipelineStatusColor(pipeline.status))
                        }
                    }
