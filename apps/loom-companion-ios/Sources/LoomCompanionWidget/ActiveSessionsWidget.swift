...
struct ActiveSessionsProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<ActiveSessionsEntry>) -> Void) {
        ...
        let stored = SharedDataStore.load()?.sessions
        let entry = ActiveSessionsEntry(
            date: .now,
            data: stored ?? SessionWidgetData(activeCount: 0, topSessions: []),
            isPaired: stored != nil
        )
...
struct ActiveSessionsWidgetView: View {
    ...
    var body: some View {
        if !entry.isPaired {
            WidgetUnpairedView(title: "Sessions", systemImage: "person.2.fill")
        } else {
            pairedBody
        }
    }
