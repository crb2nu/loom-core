...
struct TasksProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<TasksEntry>) -> Void) {
        // Real home screen — no sample data. See WidgetUnpairedView.
        let stored = SharedDataStore.load()?.tasks
        let entry = TasksEntry(
            date: .now,
            data: stored ?? TaskWidgetData(pending: 0, inProgress: 0, blocked: 0, completed: 0, recentTitles: []),
            isPaired: stored != nil
        )
...
struct TasksWidgetView: View {
    ...
    var body: some View {
        if !entry.isPaired {
            WidgetUnpairedView(title: "Tasks", systemImage: "checklist")
        } else {
            switch family {
            case .systemSmall:
                smallView
