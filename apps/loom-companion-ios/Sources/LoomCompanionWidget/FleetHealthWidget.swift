...
struct FleetHealthEntry: WidgetKit.TimelineEntry {
    ...
    /// False when no App-Group snapshot exists — the install has never paired
    /// (or never synced). The view then renders an explicit empty state
    /// instead of sample numbers.
...
struct FleetHealthProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<FleetHealthEntry>) -> Void) {
        // Never fall back to `placeholder` here: this is the real home screen,
        // and sample data reads as live fleet health.
        let stored = SharedDataStore.load()?.fleet
        let entry = FleetHealthEntry(
            date: .now,
            data: stored ?? FleetWidgetData(
...
struct FleetHealthWidgetView: View {
    ...
    var body: some View {
        if !entry.isPaired {
            WidgetUnpairedView(title: "Fleet", systemImage: "heart.text.square")
        } else {
            switch family {
            case .systemSmall:
                smallView
