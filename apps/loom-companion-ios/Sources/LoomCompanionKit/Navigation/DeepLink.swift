...
public enum DeepLink: Equatable, Sendable {
    ...
    // Mills · pipeline oversight. `loom://pipeline/<id>/escalate` — issued by
    // the Mills widget's per-pipeline button. Routes to the Mills tab, where a
    // confirm sheet performs the admin-gated escalate (the app holds the
    // credentials; the widget extension can't). Only `escalate` is supported —
    // the operator's pause/resume are still 501 stubs.
    case pipelineEscalate(id: String)
    ...
    public struct ConfigureSpec: Equatable, Sendable {
        ...
        /// The HUD admin token (HUD_ADMIN_TOKEN) for Mills mutations, separate
        /// from `bearer` (the mobile operator token). Optional — omit for a
        /// read-only pairing. Rides the URL like the other secrets, so only ever
        /// send `loom://configure` over a trusted transport (USB).
    ...
    public static func from(_ url: URL) -> DeepLink? {
        ...
        case "pipeline":
            // loom://pipeline/<id>/<action> — only `escalate` today.
            guard let id = pathComponents.first?.nilIfEmpty else { return nil }
            let action = pathComponents.count > 1 ? pathComponents[1].lowercased() : ""
            switch action {
            case "escalate": return .pipelineEscalate(id: id)
            default: return nil
            }
    ...
    public var urlString: String {
        ...
        case .pipelineEscalate(let id): return "loom://pipeline/\(Self.escape(id))/escalate"
    ...
    public var destinationGroup: DestinationGroup {
        ...
        case .mills, .pipelineEscalate: return .mills
    ...
    public var shareTitle: String {
        ...
        case .pipelineEscalate(let id): return "Escalate pipeline \(id)"
