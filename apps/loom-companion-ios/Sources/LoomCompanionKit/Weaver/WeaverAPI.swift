...
public struct WeaverAPI: WeaverAPIProtocol, Sendable {
    ...
    // The `/api/weaver/*` and `/api/aimodels/*` surfaces are served with the
    // HUD's bare `writeJSON` (no mobile envelope), so they must use
    // `requestRaw`. Routing them through `request(_:)` fails envelope decoding
    // on every response — the same class of bug that surfaced as
    // "Couldn't reach Mills".
    /// Weaver's own contract for "not enabled" is `{"enabled": false}`, but a
    /// daemon that predates the weaver domain answers 404 and an unconfigured
    /// upstream answers 503/502. Without the same degradation catch its
    /// siblings have, those rendered WeaverScreen's red errorCard instead of
    /// the calm "Weaver is not configured" empty state.
    public func status() async throws -> WeaverStatus {
        do {
            return try await client.requestRaw(.weaverStatus)
        } catch let LoomAPIError.apiError(code, _, _)
            where code == .notFound || code == .notConfigured || code == .upstreamError
        {
            return WeaverStatus(enabled: false)
        }
    }

    public func history() async throws -> WeaverHistoryResponse {
        do {
            return try await client.requestRaw(.weaverHistory)
        } catch let LoomAPIError.apiError(code, _, _)
    ...
    public func metrics() async throws -> WeaverMetrics {
        do {
            return try await client.requestRaw(.weaverMetrics)
        } catch let LoomAPIError.apiError(code, _, _)
            where code == .notFound || code == .notConfigured
        {
    ...
    public func roles() async throws -> AIModelRolesResponse {
        do {
            return try await client.requestRaw(.aimodelsRoles)
        } catch let LoomAPIError.apiError(code, _, _)
            where code == .notFound || code == .notConfigured
        {
