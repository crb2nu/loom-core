...
public enum AttentionLaneKind: String, Sendable, CaseIterable {
    ...
    case merge
    /// A tracked merge request that is STUCK — mrwatch attention lanes reuse
    /// `type: "merge"` but carry `target_kind: "merge_request"`, and labeling
    /// them "ready to merge" inverts their meaning. Resolved only via
    /// `DashboardAttentionLane.kind` (the type-key classifier cannot see
    /// target_kind).
    case mergeStalled
...
public extension AttentionLaneKind {
    ...
    var heroIcon: String {
        ...
        case .mergeStalled: return "clock.arrow.circlepath"
    ...
    var singularTitle: String {
        ...
        case .mergeStalled: return "Unstick merge request"
    ...
    var aggregateTitleIfKnown: String? {
        ...
        case .mergeStalled: return "Stalled merge requests"
...
public extension DashboardAttentionLane {
    ...
    var kind: AttentionLaneKind {
        // mrwatch stalled-MR lanes reuse `type: "merge"` (the notifier's closed
        // lane contract) but are the semantic OPPOSITE of merge-ready; the
        // target_kind disambiguates. Checked before the type-key classifier,
        // which cannot see target_kind.
        if type == "merge" && targetKind == "merge_request" { return .mergeStalled }
