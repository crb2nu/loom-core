...
public struct SpawnConfig: Codable, Sendable {
    ...
    public init(agentTypes: [SpawnAgentTypeInfo], projects: [SpawnProjectInfo], defaults: SpawnDefaults) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        agentTypes = try container.decodeIfPresent([SpawnAgentTypeInfo].self, forKey: .agentTypes) ?? []
        // Older HUDs marshal a nil project slice as JSON null; don't let that
        // sink the agent-types and defaults that arrived alongside it.
        projects = try container.decodeIfPresent([SpawnProjectInfo].self, forKey: .projects) ?? []
        defaults = try container.decode(SpawnDefaults.self, forKey: .defaults)
    }
}
