...
/// Response payload for `POST /workflows/{id}/approve` and `.../reject`.
///
/// The backend does NOT echo the workflow record back — it returns
/// `mobileWorkflowMutationResponseDTO`
/// (`internal/hud/domain/mobile/handler_workflows.go`), i.e.
/// `{workflow_id, step_id, action, deprecated?, deprecation_message?}`.
/// Decoding these responses as `MobileWorkflowDetail` fails, which is why
/// callers must use this type and then re-fetch the detail.
public struct MobileWorkflowMutationResponse: Decodable, Sendable {
    public let workflowId: String
    public let stepId: String
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.stepId = try c.decodeIfPresent(String.self, forKey: .stepId) ?? ""
        self.action = try c.decodeIfPresent(String.self, forKey: .action) ?? ""
        self.deprecated = try c.decodeIfPresent(Bool.self, forKey: .deprecated) ?? false
        self.deprecationMessage = try c.decodeIfPresent(String.self, forKey: .deprecationMessage)
    }

    public init(workflowId: String, stepId: String, action: String, deprecated: Bool = false, deprecationMessage: String? = nil) {
        self.workflowId = workflowId
        self.stepId = stepId
        self.action = action
        self.deprecated = deprecated
...
/// Response envelope payload for `POST /handoffs/{id}/accept` and
/// `.../reject`. The backend replies `{status, handoff_id, session_id?, result}`
/// where `result` is the raw agent-bridge payload — the app only needs the
/// status and id, so `result` is deliberately not decoded.
public struct MobileHandoffActionResponse: Decodable, Sendable {
    public let status: String
    ...
    public init(status: String, handoffId: String, sessionId: String? = nil) {
        self.status = status
