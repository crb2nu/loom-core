...
public final class DashboardViewModel {
    ...
    public func load() async {
        ...
        #if os(iOS)
        // Publish once for the final coalesced snapshot rather than once per
        // overlapping poll/SSE trigger.
        Task { [apiClient] in
            await MillsWidgetPublisher.publish(using: apiClient)
        }
        #endif
        isLoading = false
    ...
    private static let notificationEventTypes: Set<String> = [
        ...
        // Fired alerts from the HUD alert engine. The payload is the store's
        // own `alerting.Alert` record, so it merges onto the same id as the
        // `GET /api/mobile/v1/alerts` history instead of double-listing.
        "pipeline.alert",
    ]
