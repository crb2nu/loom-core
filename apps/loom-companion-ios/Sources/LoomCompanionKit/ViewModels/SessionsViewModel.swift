...
public final class SessionsViewModel {
    ...
    /// Server-side status query derived from the UI filter. The backend
    /// defaults to active-only when no status is sent, so "All" and "Ended"
    /// must ask explicitly or they'd filter an active-only list.
    var serverStatusQuery: String {
        switch statusFilter {
        case nil:
            return "all"
        case .ended, .summarized:
            // The UI's "Ended" bucket matches SessionStatus.isTerminal.
            return "ended,summarized"
        case .active, .unknown:
            return "active"
        }
    }
    ...
    public func load() async {
        ...
        do {
            let tree: SessionTreeResponse = try await apiClient.request(.sessionsTree(status: serverStatusQuery))
            sessionTree = tree
    ...
    private func loadFlatFallback(preferredError: LoomAPIError) async {
        do {
            let response: SessionsResponse = try await apiClient.request(.sessions(status: serverStatusQuery))
