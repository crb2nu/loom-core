...
struct OpsViewModelTests {
    ...
    @Test("Concurrent first-load calls share one Work request")
    func concurrentFirstLoadIsSingleFlight() async {
        let client = MockAPIClient()
        client.requestDelayNanoseconds = 50_000_000
        client.tasksResponse = MobileTasksResponse(
            tasks: [],
            counts: MobileTaskCounts(pending: 0, inProgress: 0, blocked: 0, completed: 0)
        )
        client.workflowsResponse = MobileWorkflowsResponse(workflows: [], pendingApprovals: 0)

        let vm = OpsViewModel(apiClient: client)
        let parentLoad = Task { await vm.loadSectionIfNeeded(.work) }
        while client.requestCount(for: .tasks(limit: 50)) == 0 {
            await Task.yield()
        }
        let childLoad = Task {
            await vm.loadSectionIfNeeded(.work)
            return vm.loadedSections.contains(.work)
        }

        await parentLoad.value
        let childObservedSettledState = await childLoad.value

        #expect(client.requestCount(for: .tasks(limit: 50)) == 1)
        #expect(client.requestCount(for: .workflows(limit: 50)) == 1)
        #expect(vm.loadedSections.contains(.work))
        #expect(childObservedSettledState)
    }

    @Test("Work exposes initial loading state until requests settle")
    func initialWorkLoadPublishesProgress() async {
        let client = MockAPIClient()
        client.requestDelayNanoseconds = 50_000_000
        client.tasksResponse = MobileTasksResponse(
            tasks: [],
        ...
        let load = Task { await vm.loadSectionIfNeeded(.work) }
        while client.requestCount(for: .tasks(limit: 50)) == 0 {
            await Task.yield()
        }

        #expect(vm.isLoading(.work))
        #expect(!vm.loadedSections.contains(.work))
        ...
        #expect(!vm.isLoading(.work))
        #expect(vm.loadedSections.contains(.work))
        #expect(vm.workLastUpdatedAt != nil)
    }

    @Test("Retry clears a failed initial Work load")
    func retryRecoversInitialWorkLoad() async {
        let client = MockAPIClient()
        client.endpointFailures[Endpoint.tasks(limit: 50).path] = .networkError(underlying: "offline")
        client.workflowsResponse = MobileWorkflowsResponse(workflows: [], pendingApprovals: 0)
        let vm = OpsViewModel(apiClient: client)
        ...
        client.endpointFailures.removeValue(forKey: Endpoint.tasks(limit: 50).path)
        client.tasksResponse = MobileTasksResponse(
            tasks: [],
            counts: MobileTaskCounts(pending: 0, inProgress: 0, blocked: 0, completed: 0)
        )
        await vm.reloadSection(.work)
