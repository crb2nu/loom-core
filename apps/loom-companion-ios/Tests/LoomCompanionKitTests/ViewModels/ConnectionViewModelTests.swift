...
struct ConnectionViewModelTests {
    ...
    @Test("Injected authenticated client keeps a stable identity")
    func injectedClientIsStable() {
        let client = APIClient(baseURL: URL(string: "http://localhost")!, token: "fixture")
        let viewModel = ConnectionViewModel(authenticatedClient: client)

        #expect(viewModel.isAuthenticated)
        #expect(viewModel.buildAPIClient() === client)
        #expect(viewModel.buildAPIClient() === client)
    }
