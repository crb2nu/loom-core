...
struct SessionsViewModelTests {
    ...
    @Test("Load forwards the server status derived from the UI filter")
    func loadForwardsServerStatus() async {
        let client = MockAPIClient()
        client.sessionsResponse = SessionsResponse(sessions: [])

        let vm = SessionsViewModel(apiClient: client)

        // Default (All) must ask for everything — the server defaults to
        // active-only when no status is sent, which broke Ended/All.
        await vm.load()
        #expect(client.lastSessionsTreeStatus == "all")
        #expect(client.lastSessionsStatus == "all")

        vm.statusFilter = .ended
        await vm.load()
        #expect(client.lastSessionsTreeStatus == "ended,summarized")

        vm.statusFilter = .active
        await vm.load()
        #expect(client.lastSessionsTreeStatus == "active")
    }
