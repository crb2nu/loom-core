...
struct DashboardViewModelTests {
    ...
    @Test("Overlapping refreshes coalesce into one queued reload")
    func overlappingRefreshesCoalesce() async throws {
        let client = MockAPIClient()
        client.requestDelayNanoseconds = 100_000_000
        client.dashboardResponse = DashboardData(
        ...
        async let third: Void = vm.load()
        _ = await (first, second, third)

        #expect(client.requestCount(for: .dashboard) == 2)
        #expect(client.requestCount(for: .tasks(limit: 1)) == 2)
