...
struct DeepLinkTests {
    ...
    @Test("Configure carries an admin token and round-trips")
    func configureAdminTokenRoundTrip() {
        let spec = DeepLink.ConfigureSpec(
            mode: "lan",
            url: "http://127.0.0.1:3333",
            bearer: "dev-token",
            adminToken: "hud-admin-secret"
        )
        let link = DeepLink.configure(spec)
        let rt = DeepLink.from(URL(string: link.urlString)!)
        #expect(rt == link)
        guard case let .configure(parsed) = rt else {
            #expect(Bool(false), "expected .configure case")
            return
        }
        #expect(parsed.adminToken == "hud-admin-secret")
    }

    @Test("Configure admin token is optional (nil when omitted)")
    func configureAdminTokenOptional() {
