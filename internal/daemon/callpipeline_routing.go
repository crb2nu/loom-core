...
func (p *callPipeline) releaseConnection() {
	if p.conn != nil {
		...
		if p.localCheckout != nil {
			p.localCheckout.close()
			p.localCheckout = nil
		} else if cancelled {
...
func (p *callPipeline) retireObservedLocalGeneration(reason string, cause error) bool {
	if p == nil || p.daemon == nil {
		return false
	}
	generationID := p.localGeneration
	if generationID == 0 && p.conn != nil {
		generationID, _ = transportGeneration(p.conn.Transport)
	}
	if generationID == 0 && p.daemon.serverSupervisor != nil {
		if snapshot, ok := p.daemon.serverSupervisor.current(p.serverName); ok {
			generationID = snapshot.Generation
		}
	}
	retired, err := p.daemon.failServerGeneration(p.serverName, generationID, cause)
	if err != nil && p.daemon.logger != nil {
		p.daemon.logger.Warn("failed to retire local server generation",
			"server", p.serverName, "generation", generationID, "reason", reason, "error", err)
	}
	if retired && p.daemon.serverSupervisor == nil {
...
func (p *callPipeline) retryLocalAfterLocalSendFailure(err error, req *mcp.Message, start time.Time) (*mcp.Message, bool) {
	...
	retired := p.retireObservedLocalGeneration("transport_send_retry", err)
	if retired && p.daemon.eventBus != nil {
...
func (p *callPipeline) resetLocalServer(reason string) {
	...
	retired := p.retireObservedLocalGeneration(reason, nil)
	if retired && p.daemon.eventBus != nil {
		p.daemon.eventBus.Publish(EventProcessStop, map[string]any{
			"server":     p.serverName,
			"reason":     reason,
...
func (p *callPipeline) connectTarget(target router.Target, reason string) error {
	...
	if target == router.TargetLocal {
		if p.daemon.muxStdio {
			if p.daemon.serverSupervisor == nil && p.daemon.procMgr != nil {
	...
	case router.TargetLocal:
		if p.daemon.pool == nil {
			...
			p.localCheckout, err = p.daemon.checkoutLocalConnection(p.ctx, p.serverName)
			if err == nil {
				p.conn = p.localCheckout.conn
				p.localGeneration = p.localCheckout.generation
			}
		}
...
func (p *callPipeline) retryHubAfterHubFailure(stage string, err error, req *mcp.Message) (*mcp.Message, bool) {
	...
	p.daemon.logger.Warn("hub transport failed; retiring owning connection and retrying once",
		"server", p.serverName,
		"tool", p.toolName,
		"stage", stage,
		"error", err)
