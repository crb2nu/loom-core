...
func (p *callPipeline) scanOutputForPII(resp *mcp.Message) *mcp.Message {
	...
	p.addStageEvent(span, "daemon.pipeline.scan",
		attribute.String("pipeline.stage", stageOutputScan),
		attribute.String("scan.mode", string(mode)),
		attribute.Int("scan.findings", len(findings)),
	)

	if len(findings) == 0 {
		span.SetAttributes(attribute.String("output_scan.result", "clean"))
		p.addStageEvent(span, "daemon.pipeline.scan.result",
			attribute.String("pipeline.stage", stageOutputScan),
			attribute.String("scan.result", "clean"),
		)
		return nil
	}
...
func (p *callPipeline) handleScanFindings(span trace.Span, mode OutputScanningMode, resp *mcp.Message, text string, findings []scanFinding) *mcp.Message {
	...
	case OutputScanRedact:
		...
		p.addStageEvent(span, "daemon.pipeline.scan.result",
			attribute.String("pipeline.stage", stageOutputScan),
			attribute.String("scan.result", "redact"),
		)
	...
	case OutputScanWarn:
		...
		p.addStageEvent(span, "daemon.pipeline.scan.result",
			attribute.String("pipeline.stage", stageOutputScan),
			attribute.String("scan.result", "warn"),
		)
