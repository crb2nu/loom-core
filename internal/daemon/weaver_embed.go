...
func (d *Daemon) startEmbeddedWeaver(ctx context.Context) error {
	...
	flexProxyURL := firstNonEmpty(d.fileCfg.EmbeddedHUD.FlexInferProxyURL, os.Getenv("FLEXINFER_PROXY_URL"))

	// The LiteLLM gateway (FLEXINFER_URL) requires an API key; when none
	// is provisioned every weaver call — the preflight /v1/models check
	// *and* live completions — fails with 401 "No api key passed in",
	// surfacing as a permanent "model preflight: degraded" banner in the
	// HUD. The keyless FlexInfer proxy (FLEXINFER_PROXY_URL) serves the
	// same router/subagent/domain models and is already the catalog
	// source of truth the aimodels domain uses, so fall back to it when
	// no gateway key is set.
	if flexKey == "" && flexProxyURL != "" {
		d.logger.Info("weaver: no FlexInfer gateway key configured; routing through keyless FlexInfer proxy",
			"proxy_url", flexProxyURL)
	}
	flexURL = flexinfer.ResolveBaseURL(flexURL, flexKey, flexProxyURL)
	if flexURL == "" {
