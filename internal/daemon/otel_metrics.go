...
type DaemonOTelMetrics struct {
	...
	ToolCallStatusDuration metric.Float64Histogram
...
func NewDaemonOTelMetrics() *DaemonOTelMetrics {
	...
	toolCallStatusDuration, _ := meter.Float64Histogram("loom.daemon.tool_call.status.duration",
		metric.WithDescription("Tool call duration in milliseconds by server, tool, and status"),
		metric.WithUnit("ms"),
	)
	...
	return &DaemonOTelMetrics{
		...
		ToolCallStatusDuration: toolCallStatusDuration,
...
func (m *DaemonOTelMetrics) RecordToolCall(ctx context.Context, server, tool, agentID, status, target string, durationMs int64) {
	...
	m.ToolCallStatusDuration.Record(ctx, float64(durationMs), metric.WithAttributes(
		attribute.String("server", server),
		attribute.String("tool", tool),
		attribute.String("status", status),
	))
