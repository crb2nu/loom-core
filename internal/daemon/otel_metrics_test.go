...
func TestDaemonOTelMetrics_InstrumentsEmitData(t *testing.T) {
	...
	expected := []string{
		...
		"loom.daemon.tool_call.status.duration",
...
func TestDaemonOTelMetrics_ToolCallStatusDurationAttributes(t *testing.T) {
	reader := setupTestMeterProvider(t)
	m := NewDaemonOTelMetrics()
	ctx := context.Background()

	m.RecordToolCall(ctx, "mcp-test", "do_thing", "agent-1", "cached", "hub", 42)

	met := findMetric(t, reader, "loom.daemon.tool_call.status.duration")
	if met == nil {
		t.Fatal("loom.daemon.tool_call.status.duration metric not found")
	}
	hist, ok := met.Data.(metricdata.Histogram[float64])
	if !ok {
		t.Fatalf("unexpected data type for status duration: %T", met.Data)
	}
	...
	if attrMap["tool"] != "do_thing" {
		t.Errorf("tool attr = %q, want do_thing", attrMap["tool"])
	}
	if attrMap["status"] != "cached" {
		t.Errorf("status attr = %q, want cached", attrMap["status"])
	}
	if dp.Count != 1 {
...
func TestDaemonOTelMetrics_RecordToolCallFromAudit(t *testing.T) {
	...
	if !names["loom.daemon.tool_call.status.duration"] {
		t.Error("missing tool_call.status.duration after RecordToolCallFromAudit")
	}
