...
func expandVarsWithRegistry(s string, repoRoot string, reg *registry.Registry) string {
	return expandVarsWithRegistryContext(context.Background(), s, repoRoot, reg)
}

// expandVarsWithRegistryContext is the cancellation-aware expansion path used
// by background refresh work that daemon shutdown must be able to join.
func expandVarsWithRegistryContext(ctx context.Context, s string, repoRoot string, reg *registry.Registry) string {
	// Expand ${HOME}
	...
	return exp.ExpandContext(ctx, s)
}
