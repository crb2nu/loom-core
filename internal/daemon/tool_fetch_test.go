...
func TestFetchToolsBounded_CancelJoinsActiveWorkers(t *testing.T) {
	t.Parallel()
	...
	select {
	case results := <-done:
		t.Fatalf("fetchToolsBounded returned before canceled worker exited: %+v", results)
	...
	case <-time.After(500 * time.Millisecond):
		t.Fatal("fetchToolsBounded did not join canceled workers")
	}
