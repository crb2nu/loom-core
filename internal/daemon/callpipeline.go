...
type callPipeline struct {
	...
	callMu          *gosync.Mutex
	lockHeld        bool
	localCheckout   *localPoolCheckout
	...
	// callTimeoutExtended is set in the execute stage when the resolved RPC
	// timeout exceeds the method default — i.e. the caller (via _timeout /
	// timeout_seconds) or operator (per-server config) opted this call into a
	// long budget. A recv timeout on such a call means the tool is
	// legitimately slow, not that the transport is wedged, so it must not count
	// toward the recv-timeout teardown breaker.
	callTimeoutExtended bool
}
...
func (p *callPipeline) addStageEvent(span trace.Span, name string, attrs ...attribute.KeyValue) {
