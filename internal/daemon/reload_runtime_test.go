...
func TestReloadGateSerializesAndHonorsWaiterContext(t *testing.T) {
	d := &Daemon{}
	if err := d.acquireReload(context.Background()); err != nil {
		t.Fatalf("first acquireReload() error = %v", err)
	}
	waitCtx, cancel := context.WithTimeout(context.Background(), 20*time.Millisecond)
	defer cancel()
	if err := d.acquireReload(waitCtx); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("contending acquireReload() error = %v, want DeadlineExceeded", err)
	}
	d.releaseReload()
	if err := d.acquireReload(context.Background()); err != nil {
		t.Fatalf("acquireReload() after release error = %v", err)
	}
	d.releaseReload()
}
