...
func TestRunWithTimeout_ReturnsOnCompletion(t *testing.T) {
	err, completed := runWithTimeout(func() error { return errors.New("boom") }, 100*time.Millisecond)
	if !completed {
		t.Fatal("expected completed=true when fn returns before timeout")
	}
...
func TestRunWithTimeout_ReturnsOnTimeout(t *testing.T) {
	start := time.Now()
	...
	if completed {
		t.Fatal("expected completed=false when fn exceeds timeout")
	}
	if err != nil {
		t.Errorf("expected nil error on timeout, got %v", err)
	}
	if waited > 200*time.Millisecond {
		t.Errorf("runWithTimeout blocked %v, much longer than the 50ms timeout", waited)
	}
}

func TestStopServerProcBounded_DoesNotBlockOnHangingStop(t *testing.T) {
	defer pkgtestutil.CheckGoroutineLeaksWithThreshold(t, 3)()
	...
	// stopServerProc on an unknown server is a no-op and returns nil quickly.
	// The bounded variant must surface that result without waiting the full
	// timeout. We assert it returns well under the 5s cap.
	start := time.Now()
	if err := d.stopServerProcBounded("never-started", 5*time.Second); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if waited := time.Since(start); waited > 1*time.Second {
		t.Fatalf("stopServerProcBounded blocked %v for a no-op stop", waited)
	}
}
