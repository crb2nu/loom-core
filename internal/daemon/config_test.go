...
func TestHealthConfig_ToHealthMonitorConfig_Custom(t *testing.T) {
	cfg := HealthConfig{
		...
		RestartPressureThreshold:     6,
		RestartPressureWindowSeconds: 120,
	}
	hmc := cfg.ToHealthMonitorConfig()
	...
	if hmc.RestartPressureThreshold != 6 {
		t.Errorf("RestartPressureThreshold = %d, want 6", hmc.RestartPressureThreshold)
	}
	if hmc.RestartPressureWindow != 120*time.Second {
		t.Errorf("RestartPressureWindow = %v, want 120s", hmc.RestartPressureWindow)
	}
}
...
func TestHealthConfig_ToHealthMonitorConfig_NegativePressureDisables(t *testing.T) {
	cfg := HealthConfig{RestartPressureThreshold: -1}
	hmc := cfg.ToHealthMonitorConfig()
	if hmc.RestartPressureThreshold != 0 {
		t.Errorf("RestartPressureThreshold = %d, want 0 (hysteresis disabled)", hmc.RestartPressureThreshold)
	}
}
...
`)
	...
	if cfg.Hub.PingIntervalSeconds != 30 || cfg.Hub.ReconnectIntervalSeconds != 5 {
		t.Fatalf("hub timing defaults = ping %d reconnect %d, want 30 and 5",
			cfg.Hub.PingIntervalSeconds, cfg.Hub.ReconnectIntervalSeconds)
	}
	if cfg.Hub.MaxMissedPongs != 2 || cfg.Hub.MaxReconnectBackoffSeconds != 30 {
		t.Fatalf("hub liveness defaults = misses %d cap %d, want 2 and 30",
			cfg.Hub.MaxMissedPongs, cfg.Hub.MaxReconnectBackoffSeconds)
	}
}

func TestParseAndValidateConfig_HubKeepaliveOverrides(t *testing.T) {
	cfg, _, err := parseAndValidateConfig([]byte(`hub:
  reconnect_interval_seconds: 7
  ping_interval_seconds: 45
  max_missed_pongs: 4
  max_reconnect_backoff_seconds: 21
  max_retries: 5
`))
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Hub.ReconnectIntervalSeconds != 7 || cfg.Hub.PingIntervalSeconds != 45 ||
		cfg.Hub.MaxMissedPongs != 4 || cfg.Hub.MaxReconnectBackoffSeconds != 21 || cfg.Hub.MaxRetries != 5 {
		t.Fatalf("hub overrides not preserved: %+v", cfg.Hub)
	}
}

func TestParseAndValidateConfig_HubBackoffCapIsPositiveAndAtLeastInitial(t *testing.T) {
	cfg, _, err := parseAndValidateConfig([]byte(`hub:
  reconnect_interval_seconds: 40
  max_reconnect_backoff_seconds: -1
`))
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Hub.MaxReconnectBackoffSeconds != 40 {
		t.Fatalf("backoff cap = %d, want initial delay 40", cfg.Hub.MaxReconnectBackoffSeconds)
	}
}
