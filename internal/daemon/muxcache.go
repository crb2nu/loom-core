...
func newPerConnTransport(mux *muxstdio.Transport, generation ...uint64) *perConnTransport {
	var observed uint64
	if len(generation) > 0 {
		observed = generation[0]
	}
	return &perConnTransport{mux: mux, generation: observed}
}

func (p *perConnTransport) serverGeneration() uint64 {
	if p == nil {
		return 0
	}
	return p.generation
