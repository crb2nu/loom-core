...
func buildEmbeddedHUDConfig(cfg EmbeddedHUDConfig, registryPath string) hud.Config {
	hudCfg := hud.Config{
		...
		// Cloudflare Access SSO → HUD admin. Env-driven (set on the mobile-hud
		// deployment); empty team domain / emails leaves the feature off and the
		// token path is unchanged. See internal/hud/cfaccess.
		CFAccessTeamDomain:  os.Getenv("HUD_CF_ACCESS_TEAM_DOMAIN"),
		CFAccessAUD:         os.Getenv("HUD_CF_ACCESS_AUD"),
		CFAccessAdminEmails: os.Getenv("HUD_ADMIN_EMAILS"),
		// LAN trusted-network admin (internal-ingress path that bypasses Cloudflare).
		AdminTrustedCIDRs:                 os.Getenv("HUD_ADMIN_TRUSTED_CIDRS"),
		...
		SpawnControllerID:                 firstNonEmpty(cfg.SpawnControllerID, os.Getenv("SPAWN_CONTROLLER_ID")),
		SpawnRecoveryAuthority:            cfg.SpawnRecoveryAuthority || envBoolTrue(os.Getenv("SPAWN_RECOVERY_AUTHORITY")),
		...
		AutofixEnabled:     cfg.AutofixEnabled || envBoolTrue(os.Getenv("HUD_AUTOFIX_ENABLED")),
