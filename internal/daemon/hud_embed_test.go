...
func TestBuildEmbeddedHUDConfig_SpawnControllerOwnership(t *testing.T) {
	t.Run("environment configures shared owner", func(t *testing.T) {
		t.Setenv("SPAWN_CONTROLLER_ID", "loom-hub/mobile-hud")
		t.Setenv("SPAWN_RECOVERY_AUTHORITY", "true")
		got := buildEmbeddedHUDConfig(EmbeddedHUDConfig{}, "")
		if got.SpawnControllerID != "loom-hub/mobile-hud" {
			t.Fatalf("SpawnControllerID = %q", got.SpawnControllerID)
		}
		if !got.SpawnRecoveryAuthority {
			t.Fatal("SPAWN_RECOVERY_AUTHORITY was not propagated")
		}
	})

	t.Run("file owner wins and file authority enables", func(t *testing.T) {
		t.Setenv("SPAWN_CONTROLLER_ID", "from-env")
		t.Setenv("SPAWN_RECOVERY_AUTHORITY", "false")
		got := buildEmbeddedHUDConfig(EmbeddedHUDConfig{
			SpawnControllerID:      "from-file",
			SpawnRecoveryAuthority: true,
		}, "")
		if got.SpawnControllerID != "from-file" || !got.SpawnRecoveryAuthority {
			t.Fatalf("ownership config = %q/%v", got.SpawnControllerID, got.SpawnRecoveryAuthority)
		}
	})
}

// TestBuildEmbeddedHUDConfig_AutofixEnabled pins the autofix feature flag:
// default off (routes serve honest empty/503 states), enabled via file
// config or HUD_AUTOFIX_ENABLED env.
func TestBuildEmbeddedHUDConfig_AutofixEnabled(t *testing.T) {
	t.Run("default off", func(t *testing.T) {
		t.Setenv("HUD_AUTOFIX_ENABLED", "")
		got := buildEmbeddedHUDConfig(EmbeddedHUDConfig{}, "")
		if got.AutofixEnabled {
			t.Fatal("AutofixEnabled must default to false")
		}
	})

	t.Run("env enables", func(t *testing.T) {
		t.Setenv("HUD_AUTOFIX_ENABLED", "true")
		got := buildEmbeddedHUDConfig(EmbeddedHUDConfig{}, "")
		if !got.AutofixEnabled {
			t.Fatal("HUD_AUTOFIX_ENABLED=true was not propagated")
		}
	})

	t.Run("file config enables", func(t *testing.T) {
		t.Setenv("HUD_AUTOFIX_ENABLED", "")
		got := buildEmbeddedHUDConfig(EmbeddedHUDConfig{AutofixEnabled: true}, "")
		if !got.AutofixEnabled {
			t.Fatal("file config AutofixEnabled was not propagated")
		}
	})
}
