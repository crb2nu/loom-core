...
type Daemon struct {
	...
	// registry is the immutable construction-time fallback used by focused
	// tests that assemble Daemon literals. Runtime readers use registrySnapshot;
	// Reload never mutates this pointer.
	registry              *registry.Registry
	registrySnapshot      atomic.Pointer[registry.Registry]
	repoRoot              string // Repository root for ${repo} expansion
	procMgr               *process.Manager
	localProcController   *shardedProcessController
	serverSupervisor      *serverSupervisor
	registryEpoch         atomic.Uint64
	reloadGateOnce        gosync.Once
	...
	// hubKeepaliveDowngradeWarned records the hub servers whose raw (non-
	// envelope) pong has already been warned about, so the keepalive
	// compatibility downgrade logs once per server instead of once per ping.
	hubKeepaliveDowngradeWarned gosync.Map // serverName -> struct{}
	...
	routingPreferences          map[string]RoutingPreference    // Per-server routing pins as configured (immutable after New; see effectiveRoutingPreference)
	...
	cacheRuntime                atomic.Pointer[cacheRuntimeState]
	cacheRefreshWorkMu          gosync.Mutex
	cacheRefreshWorkCtx         context.Context
	cacheRefreshWorkCancel      context.CancelFunc
	cacheRefreshWorkWG          gosync.WaitGroup
	cacheRefreshWorkStopped     bool
	...
	// When true, every pool.Conn for a serverName shares the generation-owned
	// muxstdio.Transport and the per-server callLock is skipped for TargetLocal
	...
	muxStdio bool
	// muxCache remains only as a compatibility fallback for focused tests and
	// manually constructed daemons without serverSupervisor. Production local
	// resources are owned by serverSupervisor generations.
	muxCache *muxCache

	// hubTransports owns every independently dialed physical hub connection.
	// The hub pool owns idle entries; this registry additionally retains active
	// entries so daemon shutdown can close and unblock them before waiting.
	hubTransports *hubTransportRegistry

	// offLAN reports whether the home-LAN sentinel is currently unreachable.
	// While set, effectiveRoutingPreference upgrades local pins on hub-capable
	// servers to prefer-hub. Maintained by netmodeWatchLoop; seeded at New.
	offLAN atomic.Bool
	// offLANHubCapable maps server name -> hub-capable, immutable after New.
	// nil disables off-LAN pin upgrades entirely (no hub configured).
...
func (d *Daemon) currentRegistry() *registry.Registry {
	if d == nil {
		return nil
	}
	if current := d.registrySnapshot.Load(); current != nil {
		return current
	}
	// Compatibility for focused tests that assemble Daemon literals directly.
	return d.registry
}
...
func (d *Daemon) daemonTracer() trace.Tracer {
	if d != nil {
		// A configured exporter that failed to initialize must remain disabled.
		// Falling through to the process-wide provider could otherwise export
		// daemon request spans through a provider installed by another component.
		if d.otelRuntimeState.Configured && !d.otelRuntimeState.Enabled {
			return mcpotel.NoopTracer("loomd")
		}
		if d.tracer != nil {
			return d.tracer
		}
