...
func (d *Daemon) handleTools(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	...
	if hasCache {
		if cacheStale {
			// Trigger daemon-owned background refresh (non-blocking, deduplicated).
			d.startBackgroundToolRefresh("stale_cache")
		}
		result := toolsResult{
	...
	if len(staticTools) > 0 {
		...
		// Trigger daemon-owned background refresh to get live tools.
		d.startBackgroundToolRefresh("static_registry")
		result := toolsResult{
...
func (d *Daemon) getStaticToolsFromRegistry() []mcp.Tool {
	...
	staticSchemas := reg.GetStaticTools(d.cfg.Target)
	...
	return d.profiles.Filter(tools, snapshot.profile).Tools
}
