...
func TestParseDaemonMetrics_ToolCallSeries(t *testing.T) {
	body := `# TYPE loom_daemon_tool_call_duration_seconds_bucket histogram
loom_daemon_tool_call_duration_seconds_bucket{server="test",tool="search",status="success",le="0.01"} 5
loom_daemon_tool_call_duration_seconds_bucket{server="test",tool="search",status="success",le="0.1"} 9
loom_daemon_tool_call_duration_seconds_bucket{server="test",tool="search",status="success",le="1"} 10
loom_daemon_tool_call_duration_seconds_bucket{server="test",tool="search",status="success",le="+Inf"} 10
loom_daemon_tool_call_duration_seconds_count{server="test",tool="search",status="success"} 10
`
	result := parseDaemonMetrics(body)
	require.Len(t, result.ToolCalls, 1)

	series := result.ToolCalls[0]
	assert.Equal(t, "test", series.Server)
	assert.Equal(t, "search", series.Tool)
	assert.Equal(t, "success", series.Status)
	assert.Equal(t, float64(10), series.RequestCount)
	assert.Equal(t, float64(0), series.ErrorCount)
	assert.True(t, series.P50Ms > 0)
	assert.True(t, series.P95Ms >= series.P50Ms)
}
