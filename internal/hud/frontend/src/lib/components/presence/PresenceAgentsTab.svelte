<script lang="ts">
  ...
  import type { FileClaimInfo, Session } from '../../stores/fleet.svelte.ts';
  ...
  function reactiveRelativeTime(ts: string | number | null | undefined) {
    void tick;
    return relativeTime(ts);
  }

  let branchCollisions = $derived.by(() => {
    const branchAgents: Record<string, string[]> = {};
    agents
  ...
  function codexInfrastructureGroupKey(agent: UnifiedAgent) {
  ...
  function groupKeyFor(agent: UnifiedAgent) {
  ...
  let agentGroups = $derived.by(() => {
    ...
    for (const group of groups.values()) {
      if (!group.root && group.children.length > 0) {
        group.root = group.children.shift() ?? null;
      }
  ...
  let flatGroupedAgents = $derived.by(() => {
    const rows: Array<{ id: string; agent: UnifiedAgent; depth: number; groupKey: string }> = [];
        ...
        >
          {#snippet row({ row, hiddenColumns })}
            ...
            <td class="text-mono text-muted">
              {#if agent.pr_url}
                <a href={agent.pr_url} target="_blank" rel="noopener" class="btn btn-xs pr-link" title={agent.pr_url}>
                  PR
                </a>
              {/if}
              {agent.branch || '---'}
...
<style>
  ...
  /* Sizing comes from the global .btn/.btn-xs pair; these rules only supply
     the fill. Fill and text must share a hue — the cyan fill previously
     carried accent-orange text. */
  .pr-link {
    background: rgba(var(--info-rgb), var(--opacity-light));
    color: var(--info);
    text-decoration: none;
    margin-right: 4px;
    border: 1px solid rgba(var(--info-rgb), var(--opacity-strong));
  }

  .pr-link:hover {
    background: rgba(var(--info-rgb), var(--opacity-strong));
    text-decoration: none;
  }
  ...
  .btn-nudge {
    background: rgba(var(--warning-rgb), var(--opacity-light));
    color: var(--warning);
    border: 1px solid rgba(var(--warning-rgb), var(--opacity-strong));
  }

  .btn-nudge:hover {
    background: rgba(var(--warning-rgb), var(--opacity-strong));
  }
