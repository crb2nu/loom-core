...
<Modal title="Create Handoff" open={presenceActionsStore.showHandoffModal} onClose={() => presenceActionsStore.closeHandoffModal()}>
  ...
  {#if presenceActionsStore.newHandoffType === 'selective'}
    <div class="form-group">
      ...
      <input id="handoff-entry-ids" type="text" value={presenceActionsStore.newHandoffEntryIds.join(', ')} oninput={(e) => presenceActionsStore.newHandoffEntryIds = (e.target as HTMLInputElement).value.split(',').map((s) => s.trim()).filter(Boolean)} placeholder="entry-1, entry-2, ..." class="form-input" />
    </div>
  {/if}
