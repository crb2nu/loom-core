<script lang="ts">
  import type { CoordinationAgent, CoordinationNamespace, CoordinationRelation } from '../stores/coordination.svelte.ts';
  import type { ZoomPreset } from '../stores/lifecycle.svelte.ts';
  ...
  const presets: ZoomPreset[] = ['6h', '12h', '24h', '48h'];
  ...
  function openAttentionAgent(agent: CoordinationAgent) {
    navigateToAgentSessionOrTraces(router, agent, (agentId) => fleetStore.sessionForAgent(agentId));
  }

  // Deep-link into Dispatch focused on the card's namespace (Dispatch
  // reads the detail segment as a roster namespace filter). Cards with
  // no namespace still land on the full dispatch view.
  function openNamespacePressure(namespace: CoordinationNamespace) {
    router.navigate('agents', 'dispatch', namespace?.namespace || null);
  }

  function openRelationWatchlist(relation: CoordinationRelation) {
    router.navigate('agents', 'dispatch', relation?.namespace || null);
  }
</script>

<div class="panel lifecycle-panel">
  <PanelHeader title="Lifecycle" icon={'\u21C6'} count={totalAgents}>
    {#snippet stats()}
      <span class="text-muted text-xs">{totalSessions} sessions \u00B7 {activeCount} active</span>
    {/snippet}
    {#snippet actions()}
      <div class="zoom-presets" role="group" aria-label="Time range">
        {#each presets as preset}
          <button class="zoom-btn" class:active={zoomPreset === preset} aria-pressed={zoomPreset === preset} onclick={() => lifecycleStore.setZoom(preset)}>{preset}</button>
        {/each}
      </div>
    {/snippet}
  </PanelHeader>
  ...
  {#if timelineStore.error}
    <!-- A failed timeline poll previously rendered the same "No agent
         activity" empty as a genuinely quiet range \u2014 indistinguishable. -->
    <ErrorBanner prefix="Lifecycle refresh failed" message={timelineStore.error} />
  {/if}
  ...
  <footer class="lifecycle-footer">
    <span class="legend-item"><span class="legend-glyph legend-start" aria-hidden="true">{'\u25B2'}</span> Start</span>
    <span class="legend-item"><span class="legend-glyph legend-end" aria-hidden="true">{'\u25A0'}</span> End</span>
    <span class="legend-item"><span class="legend-glyph legend-task" aria-hidden="true">{'\u25CF'}</span> Task</span>
    <span class="legend-item"><span class="legend-glyph legend-conflict" aria-hidden="true">{'\u25C6'}</span> Conflict</span>
  </footer>
</div>
...
<style>
  ...
  .zoom-btn.active { background: var(--bg-elevated); color: var(--fg-primary); box-shadow: var(--glow-shadow-md) var(--glow-accent); }
  ...
  .rail-card-critical { border-color: color-mix(in srgb, var(--warning) 45%, var(--border)); box-shadow: var(--glow-shadow-lg) var(--glow-warning); }
  ...
  .legend-start { color: var(--success); }
  .legend-end { color: var(--error); }
  .legend-task { color: var(--info); }
  .legend-conflict { color: var(--warning); }
