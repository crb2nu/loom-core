<script lang="ts">
  ...
  function priorityColor(priority: string) {
    const map: Record<string, string> = { critical: 'var(--error)', high: 'var(--warning)', medium: 'var(--accent)', low: 'var(--fg-muted)' };
          ...
          <tbody>
            {#each filtered.slice(0, 20) as task (task.id)}
              <tr>
                ...
                <td class="cell-mono cell-time">{relativeTime(task.updated_at)}</td>
              </tr>
            {/each}
          </tbody>
        </table>
