<script lang="ts">
  ...
  import { MRWATCH_STATE_VARIANTS, type MRWatchState } from '../../utils/mrwatchStates.ts';
  ...
  let mqError = $derived(mergeQueueStore.error);
  ...
  let readySortDir = $state<'asc' | 'desc'>('asc');
  let blockedSortKey = $state('blockers');
  let blockedSortDir = $state<'asc' | 'desc'>('desc');

  function cmpStr(a: string | undefined, b: string | undefined) {
    return (a || '').localeCompare(b || '');
  }

  function cmpNum(a: number | undefined, b: number | undefined) {
    return (a || 0) - (b || 0);
  }
  ...
  function sortBlocked(key: string) {
  ...
  function sortIndicator(activeKey: string, dir: string, key: string) {
    if (activeKey !== key) return '';
    return dir === 'asc' ? ' ▴' : ' ▾';
  }

  function mrStateVariant(state: string) {
    return MRWATCH_STATE_VARIANTS[state as MRWatchState] ?? 'muted';
  }
</script>
...
<section class="dispatch-section">
  ...
  {#if !collapsed}
    {#if mqError && (ready.length > 0 || blocked.length > 0)}
      <ErrorBanner prefix="Merge queue refresh failed" message={mqError} />
    {/if}
            ...
            {#each sortedReady as candidate (candidate.agent_id + candidate.branch)}
              <tr>
                ...
                <td class="cell-actions">
                  ...
                  {/if}
                  {#if candidate.mr_iid && candidate.mr_state}
                    <Badge text={candidate.mr_state} variant={mrStateVariant(candidate.mr_state)} />
                  {/if}
                  {#if candidate.mr_web_url}
                    <a class="action-link action-link-primary" href={candidate.mr_web_url} target="_blank" rel="noopener noreferrer" title={`View merge request !${candidate.mr_iid}`}>
                      MR !{candidate.mr_iid} ↗
                    </a>
                  {:else if candidate.merge_request_new_url}
                  ...
                  {/if}
                  {#if !candidate.branch_url && !candidate.mr_web_url && !candidate.merge_request_new_url}
    ...
    <!-- The ✓ copy asserts every branch was evaluated, so it must never render
         on a failed fetch — an unreachable queue is not an empty queue. -->
    {#if mqError && ready.length === 0 && blocked.length === 0}
      <EmptyState
        icon={'⚠'}
        heading="Merge queue unavailable"
        description={mqError}
        compact
      />
    {:else if ready.length === 0 && blocked.length === 0}
      <EmptyState
        icon={'✓'}
