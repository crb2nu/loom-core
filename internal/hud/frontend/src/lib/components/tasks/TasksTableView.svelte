<script lang="ts">
  import { taskStore, type Task } from '../../stores/tasks.svelte.ts';
...
{:else}
  ...
  >
    {#snippet row({ row: task, hiddenColumns })}
      ...
      <td>
        <!-- stopPropagation: the row itself opens the detail drawer; without
             it a priority click both cycled the value and opened the drawer. -->
        <button class="priority-btn" onclick={(e) => { e.stopPropagation(); onCyclePriority(task); }} title="Click to cycle priority">
      ...
      <td class="actions-col">
        {#if task.status !== 'completed' && task.status !== 'cancelled'}
          <button class="btn-resolve" onclick={(e) => { e.stopPropagation(); onResolve(task); }} title="Resolve task" aria-label="Resolve task">✓</button>
        {/if}
      </td>
    {/snippet}
  </DataTable>
...
<style>
  ...
  .btn-resolve:hover {
    ...
    box-shadow: var(--glow-shadow-lg) var(--glow-success);
  }
  .context-hint {
    font-size: var(--text-xs);
    margin-left: 4px;
