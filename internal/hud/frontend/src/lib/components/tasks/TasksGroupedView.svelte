...
<div class="grouped-view">
  {#if !taskStore.lastUpdated && groups.length === 0}
    <!-- First-load gate: without it the pre-fetch render claimed
         "No tasks match filters" before any data had arrived. -->
    <EmptyState icon={'◯'} heading="Loading tasks…" compact />
  {:else}
                  ...
                  <tr>
                    ...
                    <td class="actions-col">
                      {#if task.status !== 'completed' && task.status !== 'cancelled'}
                        <button class="btn-resolve" onclick={() => onResolve(task)} title="Resolve task" aria-label="Resolve task">✓</button>
                      {/if}
                    </td>
                  </tr>
                {/each}
...
<style>
  ...
  .btn-resolve:hover { background: rgba(var(--success-rgb), 0.18); box-shadow: var(--glow-shadow-lg) var(--glow-success); }
