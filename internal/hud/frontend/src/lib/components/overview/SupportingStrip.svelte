...
<section class="support-section">
  <!-- The chips are siblings of the toggle, not children: nested inside it they
       were role="button" spans whose Space keypress fell through to the ancestor
       button, collapsing the strip and unmounting the chip under the cursor. -->
  <button class="support-toggle" onclick={() => { expanded = !expanded; }} aria-expanded={expanded}>
  ...
  {#if !expanded}
    <span class="support-inline">
      {#each surfaces as s (s.route)}
        <button type="button" class="support-chip-inline" onclick={() => onSelect(s.route)}>
...
<style>
  /* Row, not column: the toggle and the collapsed chip strip are now siblings.
     The expanded grid takes a full basis so it still wraps onto its own line. */
  .support-section {
    display: flex;
    flex-wrap: wrap;
    align-items: center;
  ...
  .support-chip-inline:focus-visible {
