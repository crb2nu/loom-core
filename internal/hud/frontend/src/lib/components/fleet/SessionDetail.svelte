<script lang="ts">
  import type { Session, SessionTraceResponse, SessionTraceError } from '../../stores/fleet.svelte.ts';
  import type { UnifiedAgent } from '../../utils/agents.ts';
  import type { TraceEntry } from '../../stores/traces.svelte.ts';
  ...
  // Context-timeline entries and lifecycle events arrive from the loosely
  // typed SessionTraceResponse (Record<string, unknown>[]); these capture the
  ...
  let sessionEntries = $state<TimelineEntry[]>([]);
  let sessionEvents = $state<SessionEvent[]>([]);
  let sessionTraceEntries = $state<TraceEntry[]>([]);
  let sessionTraceErrors = $state<SessionTraceError[]>([]);
  let sessionTraceMeta = $state<SessionTraceResponse | null>(null);
  ...
  async function loadSessionTrace(id: string, limit = 100) {
    loadingEntries = true;
    try {
      const data = await fleetStore.fetchSessionTrace(id, limit);
      // The trace response types entries/events/traces as loose record arrays;
      // narrow to the structured views the template renders.
      sessionEntries = (data?.entries ?? []) as unknown as TimelineEntry[];
      sessionEvents = (data?.events ?? []) as unknown as SessionEvent[];
      sessionTraceEntries = (data?.traces ?? []) as unknown as TraceEntry[];
  ...
  function sessionMetaLabel(session: Session | null) {
    if (!session) return '---';
