<script lang="ts">
  ...
  interface EconSnapshot {
    ratios: Record<string, EconRatio>;
    tokens: { frontier_tokens: number; local_tokens: number };
    generated_at?: string;
  }

  let snapshot = $state<EconSnapshot | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      snapshot = (await res.json()) as EconSnapshot;
    } catch (e) {
  ...
  function formatRatio(r: EconRatio | undefined) {
  ...
  function statusLabel(r: EconRatio | undefined) {
  ...
  let ratios = $derived<Record<string, EconRatio>>(snapshot?.ratios ?? {});
