<script lang="ts">
  ...
  // matches the existing spawn-detail polling cadence. On the shared poller so
  // a slow trace fetch can't stack concurrent requests and a hidden tab pauses.
  $effect(() => {
    if (!spawnId) return;
    const p = createPoller(() => load(), 5000);
    p.start();
    return () => p.stop();
  });
...
<div class="tab-content">
  ...
  {:else}
    <div class="messages-list" aria-label="Spawn transcript">
      {#each messages as m, i (`${i}-${m.time}-${m.kind}`)}
