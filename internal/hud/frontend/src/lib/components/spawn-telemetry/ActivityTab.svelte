<script lang="ts">
  ...
  // Row pitch baked into one constant: 23px row + 1px gap. Absolutely
  // positioned VirtualList children can't use flex gap.
  const ACTIVITY_ROW_HEIGHT = 24;
...
<div class="activity-tab">
  ...
  {:else}
    ...
    <div class="event-list" style:height="{reversed.length * ACTIVITY_ROW_HEIGHT}px">
      <VirtualList items={reversed} itemHeight={ACTIVITY_ROW_HEIGHT} label="Spawn activity log">
        {#snippet children({ item: ev })}
...
<style>
  ...
  /* VirtualList owns the scroll viewport; .event-list is just the bounded slot
     it measures against. Height is set inline from the row count and clamped
     here, so a short feed still shrinks to fit. */
  .event-list {
    max-height: 28rem;
    overflow: hidden;
    position: relative;
