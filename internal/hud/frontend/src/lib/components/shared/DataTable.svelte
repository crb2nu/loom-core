<script lang="ts" generics="Row">
  ...
  let {
    ...
    row: Snippet<[{ row: Row; index: number; expanded: boolean; hiddenColumns: Set<string> }]>;
    expandedRow?: Snippet<[{ row: Row; index: number }]>;
  } = $props();

  // Extract a row's identity key. Rows are a caller-defined shape indexed by
  // the dynamic `idKey`; the id value is contractually a string (selectedIds
  // is Set<string>), so we narrow at this boundary.
  const rowId = (r: Row): string => (r as Record<string, unknown>)[idKey] as string;
  ...
  $effect(() => {
    ...
    try {
      ...
      if (saved) {
        const { key, dir } = JSON.parse(saved) as { key?: string; dir?: 'asc' | 'desc' };
  ...
  function handleSelectAll() {
    ...
    if (selectedIds.size === rows.length) {
      ...
      onSelect(new Set(rows.map(rowId)));
    }
  }
  ...
  function invokeRowAction(rowData: Row) {
    if (onRowClick) onRowClick(rowData);
    else if (onToggleExpand) onToggleExpand(rowData);
  }

  // App.svelte binds a window-level handler where `j` and `k` are Mills
  // sub-view keys, so a key this table consumes must not also bubble — without
  // stopPropagation, `j` on the Sparks table moved the cursor AND navigated to
  // Patterns.
  function handleWrapKeydown(e: KeyboardEvent) {
...
<!-- The wrap is deliberately tabbable: it is the delivery target for the j/k
     row cursor, which has no other focusable host. -->
  ...
  <table class="data-table" class:stable-layout={stableLayout} role="grid">
    <thead>
      <tr>
        ...
        {#each visibleColumns as col (col.key)}
          ...
          >
            {#if col.sortable}
              <button type="button" class="dt-sort-btn" onclick={() => handleHeaderClick(col)}>{col.label}</button>
            {:else}
    ...
    <tbody>
      {#if loading}
        ...
        {/each}
      {:else}
        {#each displayRows as rowData, index (rowId(rowData) ?? index)}
          {@const isExpanded = expandedIds.has(rowId(rowData))}
          <tr
            data-row-index={index}
            class:selected={selectable && selectedIds.has(rowId(rowData))}
          ...
          >
            {#if selectable}
              <td class="data-table-check">
                <input
                  type="checkbox"
                  checked={selectedIds.has(rowId(rowData))}
...
<style>
  ...
  /* The sort affordance is a real button inside the columnheader th, so the
     th keeps its implicit role (and therefore a valid aria-sort). `display:
     inline` matters: stable-layout th truncates with overflow/text-overflow,
     which stops applying against a block-level child. */
  .dt-sort-btn {
    background: none;
    border: 0;
    padding: 0;
