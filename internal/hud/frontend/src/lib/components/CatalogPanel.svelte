<script lang="ts">
  import type { CatalogServer } from '../stores/catalog.svelte.ts';
  ...
  const statusChips: Array<{ value: 'all' | 'enabled' | 'disabled' | 'running'; label: string }> = [
    { value: 'all', label: 'All' },
  ...
  function toggleConsequence(srv: CatalogServer) {
    if (srv.enabled) {
  ...
  function hintSummary(values: string[] | undefined) {
  ...
  function runtimePill(srv: CatalogServer) {
    if (srv.running) {
  ...
  {#snippet header()}
    <div class="catalog-header">
      {#if catalogStore.error}
        ...
        <ErrorBanner prefix="Catalog refresh failed" message={catalogStore.error} />
      {/if}
  ...
  {#snippet toolbar()}
    <div class="catalog-toolbar">
      ...
      <div class="catalog-controls">
        <div class="status-chips" role="group" aria-label="Status filter">
          {#each statusChips as chip (chip.value)}
            <button
              ...
              aria-pressed={catalogStore.statusFilter === chip.value}
        ...
        <div class="view-toggle" role="group" aria-label="Catalog layout">
          <button class="view-btn" class:active={viewMode === 'table'} aria-pressed={viewMode === 'table'} aria-label="Table view" onclick={() => { viewMode = 'table'; }} title="Table view">{'\u2630'}</button>
          <button class="view-btn" class:active={viewMode === 'cards'} aria-pressed={viewMode === 'cards'} aria-label="Card view" onclick={() => { viewMode = 'cards'; }} title="Card view">{'\u25A3'}</button>
        </div>
      </div>
    </div>
  {/snippet}
  ...
  {#if viewMode === 'table'}
    <div class="catalog-table-wrap">
      <table class="catalog-table">
        <thead>
          <tr>
            <!-- The sort control is a real button inside the columnheader th so
                 the th keeps its implicit role and its aria-sort stays valid;
                 same shape as DataTable's header. -->
            <th class="sortable" scope="col" aria-sort={ariaSortFor('enabled')}>
              <button type="button" class="ct-sort-btn" onclick={() => handleSort('enabled')}>State</button>
              <span aria-hidden="true">{sortKey === 'enabled' ? (sortDir === 'asc' ? '\u25B2' : '\u25BC') : ''}</span>
            </th>
            <th class="sortable" scope="col" aria-sort={ariaSortFor('name')}>
              <button type="button" class="ct-sort-btn" onclick={() => handleSort('name')}>Server</button>
              <span aria-hidden="true">{sortKey === 'name' ? (sortDir === 'asc' ? '\u25B2' : '\u25BC') : ''}</span>
            </th>
            <th scope="col">Description</th>
            <th scope="col">Categories</th>
            <th scope="col">Tools</th>
            <th class="sortable" scope="col" aria-sort={ariaSortFor('running')}>
              <button type="button" class="ct-sort-btn" onclick={() => handleSort('running')}>Runtime</button>
              <span aria-hidden="true">{sortKey === 'running' ? (sortDir === 'asc' ? '\u25B2' : '\u25BC') : ''}</span>
            </th>
...
<style>
  ...
  .catalog-table .ct-sort-btn:focus-visible {
  ...
  .server-card:hover {
    ...
    box-shadow: var(--glow-shadow-lg) var(--glow-accent);
  }
  ...
  .status-dot.on {
    background: var(--success);
    box-shadow: var(--glow-shadow-sm) var(--success);
  }
