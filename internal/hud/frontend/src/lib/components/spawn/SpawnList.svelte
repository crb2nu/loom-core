...
<section class="spawn-results">
  ...
  {#if spawnStore.error}
    <!-- A failed spawn-list poll previously left the grid silently stale. -->
    <ErrorBanner prefix="Spawn list refresh failed" message={spawnStore.error} />
  {/if}
...
<style>
  ...
  .spawn-empty {
    ...
    background:
      radial-gradient(circle at top, rgba(var(--info-rgb), 0.05), transparent 48%),
  ...
  .spawn-row.active {
    ...
    box-shadow: var(--glow-shadow-xl) var(--glow-success);
  }
