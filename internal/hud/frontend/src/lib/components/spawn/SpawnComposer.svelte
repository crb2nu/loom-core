...
<style>
  ...
  .form-header::after {
    ...
    right: 0;
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(var(--accent-rgb), 0.12) 25%, rgba(var(--accent-rgb), 0.08) 75%, transparent);
    pointer-events: none;
  }
  ...
  .spawn-button:hover:not(:disabled) {
    ...
    box-shadow: 0 12px 28px rgba(var(--accent-rgb), 0.16), 0 0 10px var(--glow-accent);
  }
