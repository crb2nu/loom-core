<script lang="ts">
  ...
  // Both stores hold their payload in a nullable field, so a failed fetch
  // leaves every derived getter at its false-y default — "RBAC is disabled",
  // "No denied calls captured", "off", "0%". Those are security-posture
  // statements, so an unread config must read as unknown, never as measured.
  let rbacUnknown = $derived(!!rbacStore.error && rbacStore.config === null);
  let otelUnknown = $derived(!!otelStore.error && otelStore.data === null);
...
<div class="infra-cards">
  <div class="infra-card">
    <div class="infra-card-header">
      ...
      <Badge
        text={rbacUnknown ? 'unknown' : rbacStore.enabled ? 'active' : 'off'}
        variant={rbacUnknown ? 'error' : rbacStore.enabled ? 'success' : 'info'}
      />
    </div>
    <div class="infra-card-body">
      {#if rbacUnknown}
        <span class="text-xs card-error">RBAC status unavailable — {rbacStore.error}</span>
      {:else if rbacStore.enabled}
        <div class="rbac-tabs">
        ...
        {#if rbacView === 'policy'}
          ...
          {/if}
          <!-- "No roles or bindings configured" reads as verified security
               posture, so it must never render on a failed fetch. Reaching
               here with an error means the policy above is last-known-good. -->
          {#if rbacStore.error}
            <span class="text-xs card-error">Policy refresh failed — {rbacStore.error}</span>
          {:else if rbacStore.roles.length === 0 && rbacStore.bindings.length === 0}
        ...
        {:else}
          ...
          {:else if rbacStore.error}
            <!-- Same reason: an unread audit feed is not a clean audit feed. -->
            <span class="text-xs card-error">Denied-call refresh failed — {rbacStore.error}</span>
          {:else}
  ...
  <div class="infra-card">
    <div class="infra-card-header">
      ...
      <Badge
        text={otelUnknown ? 'unknown' : otelInfo.otlp_configured ? 'active' : 'off'}
        variant={otelUnknown ? 'error' : otelInfo.otlp_configured ? 'success' : 'info'}
      />
    </div>
    <div class="infra-card-body">
      ...
      {/if}
      <!-- Without this the 0/0 · 0% stats read as a measured tracing gap
           rather than an unread one. -->
      {#if otelUnknown}
        <div class="text-xs card-error">Tracing config unavailable — {otelStore.error}</div>
      {/if}
    </div>
  </div>
</div>
