<script lang="ts">
  ...
  let {
    ...
    rows: MergedServer[];
    onSelect: (server: MergedServer) => void;
  } = $props();
...
<div class="table-container">
  {#if healthStore.error && !healthStore.lastUpdated}
    <!-- Cold-start health failure: the DataTable's skeleton is keyed off
         `!lastUpdated`, so without this branch a failed first fetch spun an
         endless skeleton. Show the error instead so the state is legible. -->
    <EmptyState
      icon={'⚠'}
      heading="Server health unavailable"
      description={healthStore.error}
      compact
    />
  {:else if rows.length === 0 && healthStore.lastUpdated}
    <EmptyState
      icon={'♥'}
