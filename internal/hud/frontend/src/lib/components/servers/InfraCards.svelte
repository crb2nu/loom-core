<script lang="ts">
   ...
   */
  import { healthStore, type CacheStats } from '../../stores/health.svelte.ts';
  ...
  // The endpoint's own admission that it fell back to local counters because
  // the daemon cache RPC was unreachable: entries/hit_rate are placeholders,
  // not measurements, so the card must not present them as stats.
  let cacheDegraded = $derived(cacheStats?.degraded === true);
...
<div class="infra-cards">
  ...
  <div class="infra-card">
    ...
    <div class="infra-card-body">
      ...
      {:else if cacheDegraded}
        <!-- The daemon's cache RPC is unreachable, so the HUD answered from
             its own local counters with a placeholder hit rate. Showing
             "0.0% hit rate" here would be a fabricated measurement. -->
        <div class="cache-degraded">
          <span class="cache-degraded-value text-mono" aria-hidden="true">—</span>
          <span class="text-muted text-xs">
            No cache data — the daemon cache RPC is unreachable, so hit rate and
