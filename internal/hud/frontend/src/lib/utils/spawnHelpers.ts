...
// creating/building are healthy startup states, not faults — 'degraded'
// rendered a booting spawn as an amber pulsing fault triangle.
export function spawnStatusDot(status: string): 'healthy' | 'degraded' | 'down' | 'idle' {
  if (status === 'running') return 'healthy';
  if (status === 'creating' || status === 'building') return 'idle';
