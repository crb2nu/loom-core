...
import { relativeTime, sanitizeText } from './format.ts';
...
// Delegates to the shared relativeTime so servers age the same way as every
// other surface; keeps the blank-for-unparseable contract its call site relies
// on (an empty <span>, not a '---' placeholder).
