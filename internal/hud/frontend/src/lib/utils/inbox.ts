...
export interface CardAction {
   ...
   *
   * Resolves to `unknown`, not `void`: store mutations that report their
   * outcome by return value (`Promise<boolean>`) are wired here directly. The
   * card ignores the value — outcome is surfaced by useAction — but the type
   * must not force a store to throw away its own return contract.
   */
