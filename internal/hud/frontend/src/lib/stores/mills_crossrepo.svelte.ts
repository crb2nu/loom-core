...
class MillsCrossRepoStore {
  ...
  private poller = createPoller(() => { void this.refresh(); }, 15000);
  ...
  // abort POSTs to /abort through adminFetch, so authorization follows the
  // same rules as every other Mills mutation: a Labs access-bar token when
  // present, or a Cloudflare Access SSO identity with no local token. The
  ...
  async abort(id: string): Promise<CrossRepoAbortResponse> {
    ...
    const res = await adminFetch(
      `/api/mills/cross-repo/runs/${encodeURIComponent(id)}/abort`,
      {
        method: 'POST',
        requireToken: true,
        action: 'Aborting a cross-repo run',
      },
    );
