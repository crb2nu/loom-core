 ...
 *            focusable element inside `node` (falling back to `node` itself), and
 *            bumps dialogStore so window-level shortcuts stand down.
 ...
 *            and releases the dialogStore claim.
 *
 ...
 */
// tabindex="-1" is excluded throughout, not just on the bare [tabindex] rule:
// a programmatically-focusable-only control (e.g. the command palette's
// aria-activedescendant options) is not part of the Tab cycle, and counting it
// as the last item let native Tab walk straight out of the trap.
