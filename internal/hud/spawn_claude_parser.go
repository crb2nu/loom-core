...
type ClaudeJSONLParser struct {
	...
	// lastRetryNote is the most recent api_retry diagnostic. Claude's
	// stream-json has no top-level fatal `error` event, so a run the API kills
	// (529 overloaded, 401 after a token expires) shows up only as a retry
	// storm followed by silence — which is exactly the evidence needed to
	// explain a stream that stops without its terminal `result`.
...
type claudeUserEvent struct {
	// Claude Code 1.x emitted tool results in a top-level content field. In
	// 2.x they are nested in the user message envelope, mirroring assistant
	// messages. Keep the legacy field while drains finish so older runtimes
	// remain observable.
...
type claudeResultEvent struct {
	...
	// IsError signals an SDK-level failure even when Subtype is "success"
	// (e.g. claude-code 1.x returns subtype=success + is_error=true with
	// `result: "Invalid API key · Please run /login"` when the OAuth token
	// is rejected). Without this, the spawn appears as a clean completion
	// with turn_count=0 / cost=$0, and mills marks the stage successful
...
func (p *ClaudeJSONLParser) handleResult(line []byte) {
	...
	// the spawn doesn't appear to succeed when claude actually exited with
	// "Invalid API key" or a similar auth/billing failure. Surface them in
	// the same stop_reason vocabulary as native error subtypes so downstream
	// callers (Mills runner, HUD UI) treat them uniformly.
	if ev.IsError && !isClaudeErrorSubtype(ev.Subtype) {
	...
	switch {
	case isClaudeErrorSubtype(ev.Subtype):
		p.sink.AddError(ev.Subtype, ev.Result)
	case ev.IsError:
		// Subtype was "success" but is_error=true; the Result field carries
		// the human-readable message ("Invalid API key · Please run /login",
		// etc.). Record both as an error and the last_message so callers
		// see context.
		p.sink.AddError("sdk_error", ev.Result)
		if ev.Result != "" {
...
// sweepOpenTools closes calls that Claude Code 2.x left without a matching
// user tool_result before its terminal result event. A terminal result means
// the CLI completed its stream; on a clean process exit these are protocol
// stragglers, not interrupted agent work. Without this sweep the completion
// guard reports a false "tool call(s) still open" failure after real work has
// already finished. A process that exits before a result event still fails
// closed through that guard.
func (p *ClaudeJSONLParser) sweepOpenTools() {
	for id := range p.toolStarts {
		p.logger.Info("claude tool call left open at terminal result; closing", "tool_use_id", id)
...
// streamAbortReason explains a Claude stream that stopped before its terminal
// `result` event, when the API retry log says why. A run the API gives up on
// (529 overloaded, an expired token) leaves no fatal event of its own — the
// CLI just stops — so a mid-tool truncation used to escalate as a bare count of
// tool calls left open, with the retry storm visible only in Loki.
//
// Empty once a result arrives, and empty when no retry was ever logged. That
// second guard matters because this is a completion veto (see
// spawnCompletionError): only a RECORDED diagnostic fails a spawn here, and a
// terminal result — including an error-subtype one such as error_max_turns —
// keeps its existing, unchanged outcome.
func (p *ClaudeJSONLParser) streamAbortReason() string {
	if p.sawResult {
		return ""
	}
	if p.lastRetryNote == "" {
	...
	return "claude stream ended after " + p.lastRetryNote + " with no terminal result"
}
