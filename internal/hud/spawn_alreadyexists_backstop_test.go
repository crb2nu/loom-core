...
func (b *startBackstopBackend) ProbeStartIdentity(_ context.Context, _ backend.StartOpts) (bool, error) {
	return b.runtimeExists, b.probeErr
}
func (b *startBackstopBackend) Clientset() kubernetes.Interface { return nil }
func (b *startBackstopBackend) RestConfig() *rest.Config        { return nil }
func (b *startBackstopBackend) Namespace() string               { return "devbox" }
func (b *startBackstopBackend) NFSFlush() bool                  { return false }
...
func TestStartSpawnPod_KeyedAlreadyExistsReAttaches(t *testing.T) {
	...
	be := &startBackstopBackend{startErr: newAlreadyExistsErr(podName), runtimeExists: true}
...
func TestPreflightKeyedRuntime_RowlessRuntimeFailsBeforeRegistration(t *testing.T) {
	store, err := spawn.NewFileStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	ctrl := spawn.NewK8sController(
		nil, "devbox", store, nil,
		spawn.WithControllerOwnership("loom-hub/mobile-hud", true),
	)
	be := &startBackstopBackend{runtimeExists: true}
	o := &SpawnOrchestrator{
	...
	req := SpawnRequest{
		AgentType: "codex", Project: "loom-core", TaskDescription: "task",
		IdempotencyKey: "mills/rowless-runtime",
	}
	err = o.preflightKeyedRuntime(t.Context(), req)
	if !errors.Is(err, backend.ErrRuntimeIdentityConflict) {
		t.Fatalf("preflight error = %v, want runtime identity conflict", err)
	}
	spawnID := spawn.DeriveSpawnID(req.IdempotencyKey)
	if durable, loadErr := store.Load(t.Context(), spawnID); loadErr != nil || durable != nil {
		t.Fatalf("preflight installed durable state: durable=%+v err=%v", durable, loadErr)
	}
	if _, ok := ctrl.Get(spawnID); ok {
		t.Fatal("preflight installed local state")
	}
}
