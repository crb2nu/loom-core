...
func TestHandler_MobilePolicy_AllowlistDenylistMatrix(t *testing.T) {
	...
	allowedEndpoints := []struct {
		...
		// Unauthenticated-for-everyone health endpoint: allowed for the mobile
		// token too (the mills operator sentinel probes it with LOOM_HUD_TOKEN).
		{"GET", "/api/health", ""},
	}
	...
	deniedEndpoints := []struct {
		...
		// Read-only HUD endpoints (still denied — mobile token is restricted to
		// /api/mobile/v1; /api/health is the deliberate exception, tested above).
		{"GET", "/api/status"},
		{"GET", "/api/servers"},
		{"GET", "/api/fleet"},
		{"GET", "/api/sessions"},
...
func TestDomainRegistry_OperatorSubsetSkipsNonAllowlisted(t *testing.T) {
	...
	mustAllow := []string{"fleet", "mobile", "coordinator", "workflow", "memory", "context", "alerting", "aimodels", "vendorsessions"}
