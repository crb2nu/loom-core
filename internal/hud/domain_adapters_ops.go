...
// MRWatchSnapshot returns the current branch→MR status registry snapshot. When
// the poller is disabled (GitLab unconfigured) it returns an empty-but-valid
// snapshot so the /api/mrwatch endpoints always emit well-formed JSON.
func (a *App) MRWatchSnapshot() mrwatch.Snapshot {
	return a.mrwatchPoller.Snapshot()
}

// MRWatchActions returns the mrwatch shepherd's bounded audit log. A nil
// shepherd (GitLab unconfigured, or shepherd disabled at build time) yields an
// empty-but-non-nil slice so /api/mrwatch/actions always emits a JSON array.
func (a *App) MRWatchActions() []mrwatch.ActionRecord {
	return a.mrwatchShepherd.Actions()
}

// MRWatchShepherdEnabled reports whether the optional shepherd can take
// auto-actions. It is intentionally separate from its empty audit log.
func (a *App) MRWatchShepherdEnabled() bool {
	return a.mrwatchShepherd.Enabled()
}
...
func (m *mergeDepsAdapter) MRWatchSnapshot() mrwatch.Snapshot {
	return m.app.MRWatchSnapshot()
}

// --- mrwatch domain Deps adapter ---

// mrwatchDepsAdapter wraps *App to satisfy the mrwatch domain's Deps.
type mrwatchDepsAdapter struct {
	app *App
}

func (m *mrwatchDepsAdapter) WriteJSON(w http.ResponseWriter, status int, v any) {
	m.app.WriteJSON(w, status, v)
}

func (m *mrwatchDepsAdapter) WriteError(w http.ResponseWriter, status int, msg string, err error) {
	m.app.WriteError(w, status, msg, err)
}

func (m *mrwatchDepsAdapter) Logger() *slog.Logger { return m.app.Logger() }

func (m *mrwatchDepsAdapter) MRWatchSnapshot() mrwatch.Snapshot {
	return m.app.MRWatchSnapshot()
}

func (m *mrwatchDepsAdapter) MRWatchActions() []mrwatch.ActionRecord {
	return m.app.MRWatchActions()
}

func (m *mrwatchDepsAdapter) MRWatchShepherdEnabled() bool {
	return m.app.MRWatchShepherdEnabled()
}

// --- mrwatch M5 notifier wiring ---

// mrwatchOwnerResolver joins an MR source branch to its owning agent via the
// coordination snapshot (branch is the join key, mirroring the merge domain's
// use of CoordinationSnapshot). An unowned branch reports ok=false so the
// notifier emits no inbox message (the attention lane still fires).
func (a *App) mrwatchOwnerResolver(branch string) (mrwatch.Owner, bool) {
	branch = strings.TrimSpace(branch)
	if branch == "" || a.fleetMonitor == nil {
		return mrwatch.Owner{}, false
	}
	for _, ag := range a.fleetMonitor.Snapshot().Coordination.Agents {
		if strings.TrimSpace(ag.Branch) == branch {
			return mrwatch.Owner{
				AgentID:   ag.AgentID,
...
// mrwatchMessageSender adapts the agent-context bridge to the notifier's
// MessageSender, delivering nudges via agent_message_send. Errors propagate to
// the notifier, which logs and swallows them (delivery is best-effort).
type mrwatchMessageSender struct {
	agent *bridge.AgentBridge
}

func (s *mrwatchMessageSender) SendInbox(_ context.Context, msg mrwatch.InboxMessage) error {
	if s == nil || s.agent == nil {
		return errNoAgentBridge
	}
	return s.agent.SendAgentMessage(msg.ToAgentID, msg.FromAgentID, msg.Subject, msg.Body, msg.SessionRef)
}

// errNoAgentBridge is returned when the notifier has no agent-context bridge to
// deliver through; the notifier logs and swallows it.
var errNoAgentBridge = errors.New("mrwatch notifier: agent bridge unavailable")
