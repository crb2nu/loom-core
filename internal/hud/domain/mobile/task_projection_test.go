...
func TestHandleMobileAgents_IgnoresHistoricalSessionOnlyEntries(t *testing.T) {
	// Pin the snapshot clock near the fixture heartbeats: the handler's
	// re-join evaluates heartbeat staleness against UpdatedAt, and a zero
	// value falls back to time.Now(), which would downgrade every fixture
	// row to offline (fleetview.LivePresenceStaleAfter).
	updatedAt, err := time.Parse(time.RFC3339, "2026-03-25T19:30:30Z")
	if err != nil {
