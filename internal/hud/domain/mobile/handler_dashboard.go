...
func (d *MobileDomain) handleMobileDashboard(w http.ResponseWriter, r *http.Request) {
	...
	// Attention lanes: coordination-derived {agent, namespace, merge, conflict}
	// plus mrwatch M5 stalled-MR merge/conflict lanes (same closed contract),
	// merged into one severity-ordered list under a single global cap so the
	// most urgent lane from either source can claim the hero card.
	attentionLanes := mergeMobileAttentionLanes(
		buildMobileAttentionLanes(fleetSnap.Coordination),
		buildMRWatchAttentionLanes(d.deps.MRWatchAttention()),
	)
