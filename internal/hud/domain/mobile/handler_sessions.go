...
func (d *MobileDomain) handleMobileSessions(w http.ResponseWriter, r *http.Request) {
	...
	sessions = d.filterMobileSessions(sessions, r.URL.Query().Get("status"))
...
func (d *MobileDomain) filterMobileSessions(sessions []bridge.SessionInfo, rawStatus string) []bridge.SessionInfo {
	statusFilter := parseMobileSessionStatusFilter(rawStatus)
	if statusFilter == nil {
		return sessions
	}
	...
	for _, session := range sessions {
		if statusFilter[strings.ToLower(strings.TrimSpace(session.Status))] {
			filtered = append(filtered, session)
		}
	}

	// Agent-context status is durable lifecycle state, not a heartbeat. Use
	// the same Fleet join as the web HUD to keep an "active" row out of Live
	// once its backing presence has gone offline. Historical status=all calls
	// intentionally bypass this visibility filter.
	if statusFilter["active"] {
		if fleet := d.deps.Monitors().Fleet; fleet != nil {
			snapshot := fleet.Snapshot()
			filtered = filterMobileLiveSessions(filtered, snapshot.Agents, snapshot.UpdatedAt)
		}
	}
...
func filterMobileLiveSessions(sessions []bridge.SessionInfo, agents []presence.PresenceInfo, now time.Time) []bridge.SessionInfo {
	joined := fleetview.Join(agents, sessions, now)
	observed := make(map[string]bool, len(joined))
	live := make(map[string]bool, len(joined))
	for _, agent := range joined {
		sessionID := strings.TrimSpace(agent.SessionID)
		if sessionID == "" || !agent.HasSession {
			continue
		}
		observed[sessionID] = true
	...
	for _, session := range sessions {
		...
		if mobileSessionIsLive(session.Status) && sessionID != "" && observed[sessionID] && !live[sessionID] {
			continue
		}
		filtered = append(filtered, session)
	}
	...
	if len(events) == 0 && d.deps.Agent() != nil {
		if entries, err := d.deps.Agent().SessionEntries(sessionID, limit); err == nil {
			for _, e := range entries {
				...
				entrySessionID := strings.TrimSpace(e.Entry.SessionID)
				if entrySessionID == "" {
