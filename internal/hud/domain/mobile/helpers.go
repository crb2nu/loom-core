...
func buildMobileAttentionLanes(snapshot coordination.Snapshot) []map[string]any {
	...
	// No cap here: the builder is inherently bounded (≤3 agents + ≤3
	// namespaces + merge + conflict) and the dashboard applies the single
	// global cap in mergeMobileAttentionLanes.
	return lanes
}

// buildMRWatchAttentionLanes renders the mrwatch M5 notifier's attention items
// (stalled MRs) into the same wire lane shape as the coordination-derived
// lanes. It emits only the closed-contract lane types {merge, conflict} — the
// item's Lane is already classified by TYPE FIRST in the notifier (a conflicted
// MR → "conflict", every other unhealthy state → "merge"), matching
// project_mobile_attention_lane_contract. Each lane deep-links to the MR's web
// URL for one-tap triage. A large stalled-MR set is bounded downstream by the
// dashboard's single global cap in mergeMobileAttentionLanes.
func buildMRWatchAttentionLanes(items []MergeAttentionItem) []map[string]any {
	lanes := make([]map[string]any, 0, len(items))
	for _, item := range items {
		laneType := item.Lane
		if laneType != "merge" && laneType != "conflict" {
			// Defensive: never emit a type outside the closed contract even if a
			// future notify-state forgets to classify. Collapse to the merge lane.
			laneType = "merge"
		}
		...
		summary := fmt.Sprintf("MR !%d entered %s", item.IID, item.State)
		if reason := strings.TrimSpace(item.Reason); reason != "" {
			summary += ": " + reason
		}
		lane := map[string]any{
			"type":     laneType,
			"id":       fmt.Sprintf("mr:%s!%d", item.Repo, item.IID),
			"label":    "Merge request",
			"route":    "dispatch",
			"scope":    mrwatchLaneScope(item.Branch, item.Repo),
			"summary":  summary,
			"severity": severity,
		}
		deepLink := strings.TrimSpace(item.WebURL)
		if deepLink == "" {
			deepLink = "loom://work"
		}
		addMobileAttentionTarget(lane, "merge_request", lane["id"].(string), deepLink, nil, "Review merge request")
		lane["freshness"] = map[string]any{"source": "mrwatch"}
		lanes = append(lanes, lane)
	}
	return lanes
}

// mobileLaneSeverityRank orders the closed lane severity contract for the
// dashboard sort: critical > warning > info. Unknown values sink below info
// rather than breaking the sort — the contract is closed, but a misbehaving
// producer shouldn't be able to claim the hero card with a typo.
func mobileLaneSeverityRank(severity string) int {
	switch severity {
	case "critical":
		return 0
	case "warning":
...
// mergeMobileAttentionLanes folds the coordination-derived and mrwatch lane
// slices into the single list the dashboard ships: stable-sorted by severity
// so the most urgent lane — whichever source produced it — lands first (iOS
// anchors its hero card on lane[0]), under one global cap. This replaces the
// old append-with-per-source-caps flow, where payloads could carry 16 lanes
// and a critical MR conflict always sorted below every info-level
// coordination lane. The sort is stable so within-severity relative order
// (coordination lanes first, each builder's own ordering) is preserved.
func mergeMobileAttentionLanes(coordLanes, mrwatchLanes []map[string]any) []map[string]any {
	lanes := make([]map[string]any, 0, len(coordLanes)+len(mrwatchLanes))
	lanes = append(lanes, coordLanes...)
	lanes = append(lanes, mrwatchLanes...)
	sort.SliceStable(lanes, func(i, j int) bool {
		si, _ := lanes[i]["severity"].(string)
		sj, _ := lanes[j]["severity"].(string)
		return mobileLaneSeverityRank(si) < mobileLaneSeverityRank(sj)
	})
	return limitMobileSlice(lanes, 8)
}

// mrwatchLaneScope picks the most useful short scope string for an MR lane:
// the source branch when known, else the repo path.
func mrwatchLaneScope(branch, repo string) string {
