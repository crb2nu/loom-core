...
// TestHandleMobileSpawnConfig_EmptyProjects guards the wire contract with the
// iOS client: when the spawner reports zero projects, the config payload must
// carry "projects": [] rather than null, because the app decodes projects
// into a non-optional array and null fails the entire SpawnConfig decode.
func TestHandleMobileSpawnConfig_EmptyProjects(t *testing.T) {
	spawner := &mobileMockSpawner{} // Projects() returns nil
	mux, _ := mobileControlFixture(t, spawner)

	req := httptest.NewRequest("GET", "/api/mobile/v1/agent/spawn/config", nil)
	req.Header.Set("Authorization", "Bearer test-token")
	...
	var envelope struct {
		Data struct {
			AgentTypes []json.RawMessage `json:"agent_types"`
	...
	if string(envelope.Data.Projects) != "[]" {
		t.Errorf("projects must marshal as [], got %s", envelope.Data.Projects)
	}
	if len(envelope.Data.AgentTypes) == 0 {
		t.Errorf("agent_types should be populated; body: %s", rec.Body.String())
	}
}
