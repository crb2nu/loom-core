...
func (d *FleetDomain) RegisterRoutes(mux *http.ServeMux, mw func(http.HandlerFunc) http.HandlerFunc) {
	...
	// Presence deregistration. `loom agent keepalive` posts here on
	// SIGINT/SIGTERM (cmd/loom/cmd_agent_presence.go) so a terminating agent
	// leaves the fleet view immediately rather than lingering until the
	// monitor's orphan reaper threshold expires. The bridge capability
	// (PresenceDeregister) already existed for the reaper; without this route
	// the CLI's final POST silently 404'd against the SPA catch-all.
	mux.HandleFunc("POST "+bridge.AgentDeregisterEndpoint, mw(d.handleAgentDeregister))
