...
// TestFleetDomainRegistersDeregisterEndpoint pins the presence-deregistration
// route against the SHARED contract constant that `loom agent keepalive` posts
// to on SIGINT/SIGTERM (cmd/loom/cmd_agent_presence.go). Before this route
// existed the CLI's final POST fell through to the SPA catch-all and silently
// 404'd, leaving terminated agents in the fleet view until the orphan reaper
// timed them out. The companion assertion — that the CLI actually posts to
// bridge.AgentDeregisterEndpoint — lives in
// cmd/loom TestAgentKeepaliveDeregisterUsesSharedEndpoint.
func TestFleetDomainRegistersDeregisterEndpoint(t *testing.T) {
	d := New(&mockDeps{})
	mux := http.NewServeMux()
	d.RegisterRoutes(mux, func(next http.HandlerFunc) http.HandlerFunc { return next })

	req := httptest.NewRequest(http.MethodPost, bridge.AgentDeregisterEndpoint,
		strings.NewReader(`{"agent_id":"claude-code"}`))
	rec := httptest.NewRecorder()

	// The mock's Agent() returns nil, so the handler panics once it reaches the
	// bridge call. Reaching that point is itself the proof the route resolved —
	// an unregistered path would 404 from the mux before any handler ran.
	func() {
	...
	if rec.Code == http.StatusNotFound || rec.Code == http.StatusMethodNotAllowed {
		t.Fatalf("POST %s: route not registered (got %d)", bridge.AgentDeregisterEndpoint, rec.Code)
	}

	// A malformed body must be rejected with 400 by the shared contract's
	// Validate, not silently accepted.
	badReq := httptest.NewRequest(http.MethodPost, bridge.AgentDeregisterEndpoint,
		strings.NewReader(`{"agent_id":"  "}`))
	badRec := httptest.NewRecorder()
	mux.ServeHTTP(badRec, badReq)
	if badRec.Code != http.StatusBadRequest {
		t.Errorf("empty agent_id: status = %d, want 400", badRec.Code)
	}
}
