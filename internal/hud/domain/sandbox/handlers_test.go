...
func (m *mockDeps) AdminIdentity(_ *http.Request) (email, via string, ok bool) {
	if m.allowAdmin {
		return "", "token", true
	}
	return "", "", false
...
func (m *mockDeps) ClientIP(_ *http.Request) string { return "" }
...
func (m *mockDeps) DoSandboxDetect(_ string) (map[string]any, error) {
	return m.detectResult, m.detectErr
}

func (m *mockDeps) DoSandboxQualityGate(_ string, _ []string, _ bool) (map[string]any, error) {
	return m.qualityResult, m.qualityErr
}
...
func TestHandleSandboxCapabilities_ReportsFullActionSet(t *testing.T) {
	deps := newMockDeps()
	deps.snapshot = map[string]any{"backend": "k8s"}
	d := New(deps)
	...
	var body map[string]any
	json.NewDecoder(rec.Body).Decode(&body) //nolint:errcheck // test helper; assertion failures catch issues
	actions, _ := body["supported_actions"].([]any)
	want := map[string]bool{"detect": false, "build": false, "quality_gate": false, "exec_async": false, "stop": false}
	...
	for action, seen := range want {
		if !seen {
			t.Errorf("supported_actions missing %q; got %v", action, actions)
		}
	}
	notes, _ := body["notes"].(map[string]any)
	if notes["quality_gate"] != true {
		t.Errorf("expected notes.quality_gate=true, got %v", notes["quality_gate"])
	}
}

func TestHandleSandboxDetect_Success(t *testing.T) {
	deps := newMockDeps()
	// devbox_detect wraps the fingerprint under "fingerprint"; the handler
	// hoists it to a flat shape.
	deps.detectResult = map[string]any{
		"fingerprint": map[string]any{
			"languages": []any{map[string]any{"language": "go", "version": "1.26"}},
			"hash":      "abcdef1234567890",
		},
	}
	d := New(deps)

	req := httptest.NewRequest("GET", "/api/sandbox/detect/loom-core", nil)
	req.SetPathValue("project", "loom-core")
	rec := httptest.NewRecorder()
	d.handleSandboxDetect(rec, req)
	...
	var body map[string]any
	json.NewDecoder(rec.Body).Decode(&body) //nolint:errcheck // test helper; assertion failures catch issues
	if body["hash"] != "abcdef1234567890" {
		t.Fatalf("expected hoisted hash, got %v", body["hash"])
	}
	if body["project"] != "loom-core" {
		t.Fatalf("expected project=loom-core, got %v", body["project"])
	}
}

func TestHandleSandboxDetect_NotAdminGated(t *testing.T) {
	// Detect is read-only and must render for tokenless viewers.
	deps := newMockDeps()
	deps.allowAdmin = false
	deps.detectResult = map[string]any{"fingerprint": map[string]any{"hash": "x"}}
	d := New(deps)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("expected 200 for tokenless detect, got %d", rec.Code)
	}
}

func TestHandleSandboxDetect_MissingProject(t *testing.T) {
	deps := newMockDeps()
	d := New(deps)

	req := httptest.NewRequest("GET", "/api/sandbox/detect/", nil)
	rec := httptest.NewRecorder()
	d.handleSandboxDetect(rec, req)
...
func TestHandleSandboxQualityGate_Success(t *testing.T) {
	deps := newMockDeps()
	deps.qualityResult = map[string]any{
		"language": "go",
		"passed":   true,
		"checks": []any{
			map[string]any{"name": "fmt", "passed": true, "duration_ms": 120},
			map[string]any{"name": "lint", "passed": true, "duration_ms": 800},
			map[string]any{"name": "test", "passed": true, "duration_ms": 4200},
		},
		"total_duration_ms": 5120,
	}
	d := New(deps)

	req := httptest.NewRequest("POST", "/api/sandbox/quality-gate", strings.NewReader(`{"project":"loom-core"}`))
	rec := httptest.NewRecorder()
	d.handleSandboxQualityGate(rec, req)
	...
	if deps.broadcasts[0].eventType != "hud.sandbox.event" {
		t.Fatalf("unexpected broadcast event %q", deps.broadcasts[0].eventType)
	}
}

func TestHandleSandboxQualityGate_RequiresAdminToken(t *testing.T) {
	deps := newMockDeps()
	deps.allowAdmin = false
	d := New(deps)
...
func TestHandleSandboxQualityGate_MissingProject(t *testing.T) {
	deps := newMockDeps()
	d := New(deps)

	req := httptest.NewRequest("POST", "/api/sandbox/quality-gate", strings.NewReader(`{}`))
	rec := httptest.NewRecorder()
	d.handleSandboxQualityGate(rec, req)
