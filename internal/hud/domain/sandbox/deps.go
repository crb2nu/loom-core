// deps.go defines the Deps interface for the sandbox domain handlers.
// These interfaces decouple the sandbox domain from the hud.App implementation,
// preventing import cycles and enabling testability.
package sandbox

import (
	"net/http"
	"time"
)

// Deps exposes the subset of App capabilities that sandbox handlers need.
// The hud.App satisfies this interface via accessor methods in domain_adapters.go.
type Deps interface {
	WriteJSON(w http.ResponseWriter, status int, v any)
	WriteError(w http.ResponseWriter, status int, msg string, err error)
	RequireAdminToken(w http.ResponseWriter, r *http.Request) bool
	// AdminIdentity reports how a request is authorized as admin, if at all:
	// via="access" (with the SSO email) or via="token". ok=false when neither.
	// Read-only — used by /api/labs/auth-check to tell the browser it's already
	// admin via Cloudflare Access without presenting a token.
	AdminIdentity(r *http.Request) (email, via string, ok bool)
	// ClientIP is the client IP the HUD attributes to a request (post-ingress
	// forwarding headers). Surfaced by auth-check so an operator can tune
	// HUD_ADMIN_TRUSTED_CIDRS to their LAN. "" when undetermined.
	ClientIP(r *http.Request) string
	BroadcastAgentEvent(eventType string, payload any)
	SandboxSnapshot() map[string]any
	CacheGet(key string) (any, bool)
	CacheSet(key string, value any, ttl time.Duration)
	DoSandboxStart(project, agentID string) (map[string]any, error)
	DoSandboxStop(project string) error
	DoSandboxExecAsync(project, command, timeout, agentID string) (map[string]any, error)
	DoSandboxExecPoll(execID string) (map[string]any, error)
	DoSandboxStatus(project string) ([]map[string]any, error)
	// DoSandboxDetect returns the environment fingerprint (languages, tools,
	// system deps, hash) that determines a project's sandbox image. Read-only.
	DoSandboxDetect(project string) (map[string]any, error)
	// DoSandboxQualityGate runs fmt → lint → test in the project sandbox and
	// returns the structured per-check result.
	DoSandboxQualityGate(project string, checks []string, failFast bool) (map[string]any, error)
}
