...
func TestHandleStatus(t *testing.T) {
	caller := &codebaseTestCaller{
		callToolFn: func(name string, _ map[string]any) (json.RawMessage, error) {
			return json.RawMessage(fmt.Sprintf(`{"content":[{"type":"text","text":"%s"}]}`,
				`{\"aggregate\":true,\"collection\":\"codebase_memory_v1\",\"total_chunks\":1000,\"by_language\":{\"go\":800},\"by_chunk_type\":{\"function\":600}}`)), nil
		},
	}
	agent := bridge.NewAgentBridge(caller)
	mon := monitor.NewCodebaseMonitor(agent, slog.Default())
	...
	if snap.TotalChunks != 1000 {
		t.Fatalf("expected 1000 chunks, got %d", snap.TotalChunks)
	}
	if snap.ByLanguage["go"] != 800 {
		t.Fatalf("expected 800 go chunks, got %d", snap.ByLanguage["go"])
	}
}
