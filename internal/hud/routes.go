...
func (a *App) registerRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/engrams", a.withCORS(a.handleEngramList))
	mux.HandleFunc("GET /api/engrams/graph", a.withCORS(a.handleEngramGraph))

	// API routes — Pattern Loom front door (list catalog + stamp a pattern).
	mux.HandleFunc("GET /api/patterns", a.withCORS(a.handlePatternList))
	mux.HandleFunc("POST /api/patterns/stamp", a.withCORS(a.handlePatternStamp))
	mux.HandleFunc("GET /api/patterns/{id}/instances", a.withCORS(a.handlePatternInstances))
