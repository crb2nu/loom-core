...
func TestClaudeParser_ClaudeCode2xToolLifecycleFixtures(t *testing.T) {
	fixture, err := os.ReadFile("testdata/claude-2.1.220-tool-pair.jsonl")
	if err != nil {
		t.Fatalf("read Claude Code 2.x fixture: %v", err)
	}
	...
	thirteenOpenCalls := make([]string, 0, 14)
	for i := 0; i < 13; i++ {
		thirteenOpenCalls = append(thirteenOpenCalls, fmt.Sprintf(
			`{"type":"assistant","session_id":"capture-claude-2.1.220","message":{"id":"msg_2x_bash_%02d","usage":{"input_tokens":1,"output_tokens":1,"cache_creation_input_tokens":0,"cache_read_input_tokens":0},"content":[{"type":"tool_use","id":"toolu_2x_bash_%02d","name":"Bash","input":{"command":"echo hi"}}]}}`,
			i, i,
		))
	}
	thirteenOpenCalls = append(thirteenOpenCalls, lines[2]) // captured terminal result
	...
	tests := []struct {
		...
		{
			name:          "nested user message closes captured tool result",
			lines:         lines,
			wantStarts:    1,
			wantCompletes: 1,
		},
		{
			name:          "terminal result sweeps 13 open Bash calls",
			lines:         thirteenOpenCalls,
			wantStarts:    13,
			wantCompletes: 13,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, line := range tt.lines {
				p.HandleLine([]byte(line))
			}

			if got := len(sink.toolStarts); got != tt.wantStarts {
				t.Errorf("tool starts = %d, want %d", got, tt.wantStarts)
			}
			if got := len(sink.toolCompletes); got != tt.wantCompletes {
				t.Errorf("tool completions = %d, want %d", got, tt.wantCompletes)
			}
			if got := len(p.toolStarts); got != 0 {
				t.Errorf("open tool calls = %d, want 0", got)
			}
			if sink.result == nil || sink.result.StopReason != "end_turn" {
				t.Errorf("terminal result = %+v, want end_turn", sink.result)
			}
		})
	}
}

func TestClaudeParser_TerminalResultSweepsCompletionGuard(t *testing.T) {
	acc := bridge.NewSpawnTelemetryAccumulator()
	guard := newCompletionGuardSink(acc)
	p := NewClaudeJSONLParser(guard, "agent-1", "spawn-1", nil, nil)

	for i := 0; i < 13; i++ {
		p.HandleLine([]byte(fmt.Sprintf(
			`{"type":"assistant","session_id":"sess-2x","message":{"id":"msg_2x_bash_%02d","usage":{"input_tokens":1,"output_tokens":1,"cache_creation_input_tokens":0,"cache_read_input_tokens":0},"content":[{"type":"tool_use","id":"toolu_2x_bash_%02d","name":"Bash","input":{"command":"echo hi"}}]}}`,
			i, i,
		)))
	}
	if got := guard.openToolCallCount(); got != 13 {
		t.Fatalf("open tool calls before result = %d, want 13", got)
	}

	p.HandleLine([]byte(`{"type":"result","subtype":"success","is_error":false,"duration_ms":120000,"num_turns":4,"session_id":"sess-2x","total_cost_usd":0.01,"result":"done"}`))
	if got := guard.openToolCallCount(); got != 0 {
		t.Fatalf("open tool calls after result = %d, want 0 (sweep)", got)
	}
	if err := guard.completionError(); err != nil {
		t.Fatalf("completion error after terminal result = %v, want nil", err)
	}
}

func TestClaudeParser_TruncatedStreamStillFailsClosed(t *testing.T) {
	acc := bridge.NewSpawnTelemetryAccumulator()
	guard := newCompletionGuardSink(acc)
	p := NewClaudeJSONLParser(guard, "agent-1", "spawn-1", nil, nil)

	p.HandleLine([]byte(`{"type":"assistant","session_id":"sess-2x","message":{"id":"msg_1","usage":{"input_tokens":1,"output_tokens":1,"cache_creation_input_tokens":0,"cache_read_input_tokens":0},"content":[{"type":"tool_use","id":"toolu_open_1","name":"Bash","input":{"command":"make test"}}]}}`))
	p.HandleLine([]byte(`{"type":"assistant","session_id":"sess-2x","message":{"id":"msg_2","usage":{"input_tokens":1,"output_tokens":1,"cache_creation_input_tokens":0,"cache_read_input_tokens":0},"content":[{"type":"tool_use","id":"toolu_open_2","name":"Bash","input":{"command":"go test ./..."}}]}}`))
	// Process exits here: no nested user tool_result and no terminal result.
	...
	if !strings.Contains(err.Error(), "Bash[toolu_open_1]") || !strings.Contains(err.Error(), "Bash[toolu_open_2]") {
...
// TestClaudeParser_ResultSDKError guards the failure mode where claude-code
// 1.x returns subtype="success" + is_error=true (e.g. "Invalid API key ·
// Please run /login" on a bad OAuth token). Without is_error handling, the
// spawn appears as a clean completion and Mills marks the stage success.
func TestClaudeParser_ResultSDKError(t *testing.T) {
	sink := &mockSink{}
	p := NewClaudeJSONLParser(sink, "test-agent", "", nil, nil)

	line := []byte(`{"type":"result","subtype":"success","is_error":true,"session_id":"sess-x","duration_ms":10076,"num_turns":1,"total_cost_usd":0,"result":"Invalid API key · Please run /login"}`)
	p.HandleLine(line)

	if sink.result == nil {
		t.Fatal("expected result to be set")
	}
	if sink.result.StopReason != "sdk_error" {
		t.Errorf("expected stop reason 'sdk_error', got %q", sink.result.StopReason)
	}
	if len(sink.errors) != 1 {
		t.Fatalf("expected 1 error, got %d", len(sink.errors))
	}
	if sink.errors[0].ErrType != "sdk_error" {
		t.Errorf("expected error type 'sdk_error', got %q", sink.errors[0].ErrType)
	}
	if sink.errors[0].Message != "Invalid API key · Please run /login" {
		t.Errorf("unexpected error message: %q", sink.errors[0].Message)
	}
	if sink.lastMessage != "Invalid API key · Please run /login" {
		t.Errorf("expected last_message to carry the SDK error text, got %q", sink.lastMessage)
	}
}
