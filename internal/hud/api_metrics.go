...
func parseDaemonMetrics(body string) metricsResponse {
	...
	getOrCreateToolCall := func(server, tool, status string) *toolCallData {
		if server == "" || tool == "" || status == "" {
			return nil
		}
		key := server + "\x1f" + tool + "\x1f" + status
		if tc, ok := toolCalls[key]; ok {
			return tc
		}
		tc := &toolCallData{server: server, tool: tool, status: status}
		toolCalls[key] = tc
		return tc
	}
	...
	for _, line := range strings.Split(body, "\n") {
		...
		case "loom_daemon_tool_call_duration_seconds_bucket":
			tc := getOrCreateToolCall(serverName, labels["tool"], labels["status"])
			if tc == nil {
				continue
			}
			leStr := labels["le"]
			...
			tc.buckets = append(tc.buckets, histBucket{le: le, count: value})

		case "loom_daemon_tool_call_duration_seconds_count":
			tc := getOrCreateToolCall(serverName, labels["tool"], labels["status"])
			if tc == nil {
				continue
			}
			tc.histCount += value
			tc.requestCount += value
			if labels["status"] == "error" {
				tc.errorCount += value
			}
		}
	...
	for _, tc := range toolCalls {
		...
		if tc.requestCount > 0 {
			sm.ErrorRate = tc.errorCount / tc.requestCount
		}
		sm.P50Ms = histogramPercentile(tc.buckets, tc.histCount, 0.50) * 1000
		sm.P95Ms = histogramPercentile(tc.buckets, tc.histCount, 0.95) * 1000
		sm.P99Ms = histogramPercentile(tc.buckets, tc.histCount, 0.99) * 1000
		toolResult = append(toolResult, sm)
	}
	sort.Slice(toolResult, func(i, j int) bool {
		if toolResult[i].Server != toolResult[j].Server {
			return toolResult[i].Server < toolResult[j].Server
		}
		if toolResult[i].Tool != toolResult[j].Tool {
			return toolResult[i].Tool < toolResult[j].Tool
		}
		return toolResult[i].Status < toolResult[j].Status
	})

	return metricsResponse{Servers: result, ToolCalls: toolResult}
}
