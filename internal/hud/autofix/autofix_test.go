...
// retryingPipelineBridge implements both PipelineBridge and PipelineRetrier,
// mirroring bridge.AgentBridge in production.
type retryingPipelineBridge struct {
	retried  []string
	retryErr error
}

func (m *retryingPipelineBridge) GetPipelineDetail(string, int) (*bridge.PipelineDetail, error) {
	return nil, fmt.Errorf("not implemented")
}

func (m *retryingPipelineBridge) RetryPipeline(project string, pipelineID int) error {
	m.retried = append(m.retried, fmt.Sprintf("%s:%d", project, pipelineID))
	return m.retryErr
}

func TestExecuteAutoFix_RetryStrategy(t *testing.T) {
	diag := &Diagnosis{PipelineID: 42, Project: "loom-core", Category: "infra"}

	t.Run("no retrier reports honest failure", func(t *testing.T) {
		engine := NewAutoFixEngine(nil, nil, nil, nil, "", nil)
		engine.diagnoses["loom-core:42"] = diag

		exec, err := engine.ExecuteAutoFix(context.Background(), AutoFixProposal{
			ID: "prop-1", DiagnosisID: "loom-core:42", Strategy: "retry",
		})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		if !strings.Contains(exec.Result, "no pipeline retrier configured") {
	...
	t.Run("retrier triggers real pipeline retry", func(t *testing.T) {
		pb := &retryingPipelineBridge{}
		engine := NewAutoFixEngine(nil, pb, nil, nil, "", nil)
		engine.diagnoses["loom-core:42"] = diag

		exec, err := engine.ExecuteAutoFix(context.Background(), AutoFixProposal{
			ID: "prop-2", DiagnosisID: "loom-core:42", Strategy: "retry",
		})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		if len(pb.retried) != 1 || pb.retried[0] != "loom-core:42" {
			t.Errorf("expected one retry of loom-core:42, got %v", pb.retried)
		}
	})

	t.Run("retrier error reported honestly", func(t *testing.T) {
		pb := &retryingPipelineBridge{retryErr: fmt.Errorf("gitlab 403")}
		engine := NewAutoFixEngine(nil, pb, nil, nil, "", nil)
		engine.diagnoses["loom-core:42"] = diag

		exec, err := engine.ExecuteAutoFix(context.Background(), AutoFixProposal{
			ID: "prop-3", DiagnosisID: "loom-core:42", Strategy: "retry",
		})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		if !strings.Contains(exec.Result, "gitlab 403") {
			t.Errorf("expected retry error in result, got %q", exec.Result)
		}
	})

	t.Run("missing diagnosis fails without retrying", func(t *testing.T) {
		pb := &retryingPipelineBridge{}
		engine := NewAutoFixEngine(nil, pb, nil, nil, "", nil)

		exec, err := engine.ExecuteAutoFix(context.Background(), AutoFixProposal{
			ID: "prop-4", DiagnosisID: "unknown:1", Strategy: "retry",
		})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		if len(pb.retried) != 0 {
			t.Errorf("expected no retries, got %v", pb.retried)
		}
	})
}
