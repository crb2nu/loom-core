...
// PipelineRetrier re-runs a failed pipeline. Optional: the engine detects it
// via type assertion on the PipelineBridge (bridge.AgentBridge implements
// both), so the constructor signature stays stable. When absent, the "retry"
// strategy reports failure honestly instead of pretending a retry happened.
type PipelineRetrier interface {
	RetryPipeline(project string, pipelineID int) error
}
...
func (e *AutoFixEngine) ExecuteAutoFix(ctx context.Context, proposal AutoFixProposal) (*AutoFixExecution, error) {
	...
	case "retry":
		retrier, ok := e.pipeline.(PipelineRetrier)
		switch {
		case !ok || retrier == nil:
			exec.Status = "failed"
			exec.Result = "pipeline retry not supported: no pipeline retrier configured"
		case diag == nil:
			exec.Status = "failed"
			exec.Result = "pipeline retry failed: diagnosis not found for proposal"
		default:
			if err := retrier.RetryPipeline(diag.Project, diag.PipelineID); err != nil {
				exec.Status = "failed"
				exec.Result = fmt.Sprintf("pipeline retry failed: %v", err)
			} else {
				exec.Status = "succeeded"
				exec.Result = fmt.Sprintf("pipeline %d retry triggered", diag.PipelineID)
			}
		}
		now := time.Now()
		exec.CompletedAt = &now
