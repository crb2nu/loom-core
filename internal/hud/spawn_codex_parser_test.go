...
func TestCodexFailureMessage(t *testing.T) {
	tests := []struct {
		name     string
		raw      json.RawMessage
		fallback string
	...
	}{
		{
			...
			raw:  json.RawMessage(`{"message":"HTTP 400: model rejected"}`),
			want: "HTTP 400: model rejected",
		},
		{
			name: "string error",
			raw:  json.RawMessage(`"transport closed"`),
			want: "transport closed",
		},
		{
			name:     "top-level fallback",
			raw:      json.RawMessage(`{"type":"invalid_request_error"}`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := codexFailureMessage(tt.raw, tt.fallback); got != tt.want {
				t.Fatalf("codexFailureMessage() = %q, want %q", got, tt.want)
			}
		})
	}
}
...
// TestCodexParser_TurnCompletedSweepsAbandonedToolCalls replays the captured
// shape of spawn-d9889e05e2e9 (2026-07-21): codex started three commands in a
// parallel batch, completed one, abandoned two (`make changelog-check`,
// `go test`), then pushed its work and ended the turn cleanly. The
// turn.completed sweep must close the stragglers so the completion guard does
// not fail a spawn whose work landed — that false failure burned three Mills
// implement attempts and escalated a successful run (loom-core#365).
func TestCodexParser_TurnCompletedSweepsAbandonedToolCalls(t *testing.T) {
	acc := bridge.NewSpawnTelemetryAccumulator()
	guard := newCompletionGuardSink(acc)
	p := NewCodexJSONLParser(guard, "agent-1", "spawn-1", nil, nil)

	p.HandleLine([]byte(`{"type":"turn.started"}`))
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"item_8","type":"command_execution","command":"git status --short"}}`))
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"item_9","type":"command_execution","command":"make changelog-check"}}`))
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"item_10","type":"command_execution","command":"go test ./..."}}`))
	p.HandleLine([]byte(`{"type":"item.completed","item":{"id":"item_8","type":"command_execution","command":"git status --short","exit_code":0}}`))
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"item_13","type":"command_execution","command":"git push -u origin HEAD"}}`))
	p.HandleLine([]byte(`{"type":"item.completed","item":{"id":"item_13","type":"command_execution","command":"git push -u origin HEAD","exit_code":0}}`))

	if got := guard.openToolCallCount(); got != 2 {
		t.Fatalf("open tool calls before turn.completed = %d, want 2", got)
	}
	p.HandleLine([]byte(`{"type":"turn.completed","usage":{"input_tokens":100,"cached_input_tokens":0,"output_tokens":50}}`))
	if got := guard.openToolCallCount(); got != 0 {
		t.Fatalf("open tool calls after turn.completed = %d, want 0 (sweep)", got)
	}
	if err := guard.completionError(); err != nil {
		t.Fatalf("completion error after clean turn.completed = %v, want nil", err)
	}
}

// TestCodexParser_TruncatedStreamStillFailsClosed proves the sweep does not
// weaken the guard: a stream that dies mid-turn (no turn.completed) leaves
// its open calls in place and still fails closed, naming the stragglers.
func TestCodexParser_TruncatedStreamStillFailsClosed(t *testing.T) {
	acc := bridge.NewSpawnTelemetryAccumulator()
	guard := newCompletionGuardSink(acc)
	p := NewCodexJSONLParser(guard, "agent-1", "spawn-1", nil, nil)
	...
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"item_9","type":"command_execution","command":"make changelog-check"}}`))
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"item_10","type":"command_execution","command":"go test ./..."}}`))
	// Process exits here: no item.completed, no turn.completed.

	err := guard.completionError()
	if err == nil || !strings.Contains(err.Error(), "2 tool call(s) still open") {
		t.Fatalf("completion error = %v, want fail-closed 2-open error", err)
	}
	if !strings.Contains(err.Error(), "Bash[item_9]") || !strings.Contains(err.Error(), "Bash[item_10]") {
		t.Fatalf("completion error should name the open calls, got: %v", err)
	}
}

// TestCodexParser_ItemUpdatedTerminalCompletesToolCall covers the alternate
// completion path: a terminal item.updated closes the call, and a subsequent
// item.completed for the same id must not double-complete it (the accumulator
// would clobber a different open entry).
func TestCodexParser_ItemUpdatedTerminalCompletesToolCall(t *testing.T) {
	sink := &mockSink{}
	p := NewCodexJSONLParser(sink, "test-codex", "", nil, nil)

	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"cmd-1","type":"command_execution","command":"go vet ./..."}}`))
	p.HandleLine([]byte(`{"type":"item.updated","item":{"id":"cmd-1","type":"command_execution","command":"go vet ./...","status":"completed","exit_code":0}}`))
	if len(sink.toolCompletes) != 1 {
		t.Fatalf("tool completes after terminal item.updated = %d, want 1", len(sink.toolCompletes))
	}

	p.HandleLine([]byte(`{"type":"item.completed","item":{"id":"cmd-1","type":"command_execution","command":"go vet ./...","exit_code":0}}`))
	if len(sink.toolCompletes) != 1 {
		t.Fatalf("tool completes after duplicate item.completed = %d, want 1 (idempotent)", len(sink.toolCompletes))
	}
}

// TestCodexParser_ItemUpdatedNonTerminalIgnored: in-progress status updates
// must not complete anything.
func TestCodexParser_ItemUpdatedNonTerminalIgnored(t *testing.T) {
	sink := &mockSink{}
	p := NewCodexJSONLParser(sink, "test-codex", "", nil, nil)

	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"cmd-1","type":"command_execution","command":"go build ./..."}}`))
	p.HandleLine([]byte(`{"type":"item.updated","item":{"id":"cmd-1","type":"command_execution","command":"go build ./...","status":"in_progress"}}`))
	if len(sink.toolCompletes) != 0 {
		t.Fatalf("tool completes after non-terminal item.updated = %d, want 0", len(sink.toolCompletes))
	}
}

// TestCodexParser_SweepOnlyClosesCurrentTurnStragglers: a second turn's items
// are unaffected by the first turn's sweep bookkeeping, and a completion
// arriving after a sweep for an already-swept id stays idempotent.
func TestCodexParser_SweepIdempotentAcrossTurns(t *testing.T) {
	sink := &mockSink{}
	p := NewCodexJSONLParser(sink, "test-codex", "", nil, nil)

	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"cmd-1","type":"command_execution","command":"make lint"}}`))
	p.HandleLine([]byte(`{"type":"turn.completed","usage":{"input_tokens":1,"cached_input_tokens":0,"output_tokens":1}}`))
	if len(sink.toolCompletes) != 1 {
		t.Fatalf("tool completes after sweep = %d, want 1", len(sink.toolCompletes))
	}
	// Late completion for the swept id must be a no-op.
	p.HandleLine([]byte(`{"type":"item.completed","item":{"id":"cmd-1","type":"command_execution","command":"make lint","exit_code":0}}`))
	if len(sink.toolCompletes) != 1 {
		t.Fatalf("tool completes after late duplicate = %d, want 1", len(sink.toolCompletes))
	}
	// A fresh item in the next turn completes normally.
	p.HandleLine([]byte(`{"type":"item.started","item":{"id":"cmd-2","type":"command_execution","command":"make test"}}`))
	p.HandleLine([]byte(`{"type":"item.completed","item":{"id":"cmd-2","type":"command_execution","command":"make test","exit_code":0}}`))
	if len(sink.toolCompletes) != 2 {
		t.Fatalf("tool completes after second turn = %d, want 2", len(sink.toolCompletes))
	}
}
