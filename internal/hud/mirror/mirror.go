...
type Config struct {
	...
	// VendorSessionsDisabled turns off the vendor transcript sync that
	// otherwise rides the mirror loop (LOOM_HUD_MIRROR_VENDOR_SESSIONS=0).
	// The presence heartbeat is unaffected.
	VendorSessionsDisabled bool
	// VendorInterval is the cadence of the vendor transcript sync
	// (default 60s, floor 15s) — deliberately slower than the presence
	// interval: transcripts change slowly and each changed session ships
	// a tail of extracted lines.
...
func NewConfigFromEnv() Config {
	...
	switch strings.ToLower(strings.TrimSpace(os.Getenv("LOOM_HUD_MIRROR_VENDOR_SESSIONS"))) {
	case "0", "false", "off", "no":
		cfg.VendorSessionsDisabled = true
	}
	if v := strings.TrimSpace(os.Getenv("LOOM_HUD_MIRROR_VENDOR_INTERVAL")); v != "" {
		if d, err := time.ParseDuration(v); err == nil && d > 0 {
			cfg.VendorInterval = d
		}
	}
	return cfg
}
...
type Service struct {
	...
	// Vendor transcript sync state (vendorsync.go). vendorStore nil =
	// disabled.
	vendorStore    *vendorsessions.Store
	vendorHost     string
	vendorCursors  map[string]vendorCursor
	vendorEpoch    string
	lastVendorSync time.Time
...
func New(cfg Config, reader PresenceReader, client Doer, logger *slog.Logger) *Service {
	...
	s := &Service{
		...
		vendorCursors: make(map[string]vendorCursor),
	}
	if !cfg.VendorSessionsDisabled {
		store := defaultVendorStore()
		s.vendorStore = &store
...
func (s *Service) postHeartbeat(ctx context.Context, body map[string]any) error {
	_, err := s.postJSON(ctx, "/api/agent/heartbeat", body)
	return err
}

// postJSON marshals body and POSTs it to path on the remote HUD with the
// mirror's auth headers, returning a bounded slice of the response body.
// Shared by the heartbeat (which ignores the body) and the vendor-session
// sync (which reads the receiver epoch out of it).
func (s *Service) postJSON(ctx context.Context, path string, body any) ([]byte, error) {
	...
	defer cancel()
	url := strings.TrimRight(s.cfg.URL, "/") + path
