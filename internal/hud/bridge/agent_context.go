...
// SendAgentMessage delivers a directed durable inbox message to another agent
// via agent_message_send (the cross-vendor bridge). Used by the mrwatch M5
// notifier to nudge the agent that owns a stalled MR's branch, so even a future
// session recalls it. fromID/subject/sessionRef are optional; empty values are
// omitted so the tool applies its own defaults. Returns the bridge error, which
// the caller logs and swallows (delivery is best-effort).
func (a *AgentBridge) SendAgentMessage(toAgentID, fromID, subject, body, sessionRef string) error {
	args := map[string]any{
		"to_agent_id": toAgentID,
		"body":        body,
	}
	if fromID != "" {
		args["from_agent_id"] = fromID
	}
	if subject != "" {
		args["subject"] = subject
	}
	if sessionRef != "" {
		args["session_ref"] = sessionRef
	}
	return a.callAgentTool("agent_message_send", args, nil)
}
