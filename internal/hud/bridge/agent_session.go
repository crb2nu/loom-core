...
type SessionEndParams struct {
	...
	SkipPresenceDeregister bool   `json:"skip_presence_deregister,omitempty"`
}
...
func (a *AgentBridge) EndSession(p SessionEndParams) (bool, error) {
	...
	if p.AgentID != "" && !p.SkipPresenceDeregister {
...
// PresenceDeregisterWithTimeout removes an agent from the presence registry
// with a bounded daemon/tool call. It is used by terminal spawn cleanup so a
// slow or unhealthy agent-context server cannot keep a completed spawn in the
// cleanup-pending state indefinitely.
func (a *AgentBridge) PresenceDeregisterWithTimeout(agentID string, timeout time.Duration) error {
	if strings.TrimSpace(agentID) == "" {
		return fmt.Errorf("agent_id required")
	}
	return a.callAgentToolTimeout("agent_presence_deregister", map[string]any{
		"agent_id": agentID,
	}, nil, timeout)
}
