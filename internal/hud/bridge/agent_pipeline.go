...
// GetJobTrace fetches the tail of a job's trace log via the gitlab MCP
// server. The server-side defaults (last 200 lines, 200KB cap) keep the
// payload bounded; callers that need less should truncate further.
func (a *AgentBridge) GetJobTrace(project string, jobID int) (string, error) {
	raw, err := a.client.CallToolWithTimeout("gitlab__get_job_trace", map[string]any{
		"project": project,
		"job_id":  jobID,
	}, pipelineDetailToolTimeout)
	if err != nil {
		return "", fmt.Errorf("get job trace %d: %w", jobID, err)
	}
	...
	if err := unmarshalGitLabResult(raw, &result); err != nil {
		return "", fmt.Errorf("unmarshal job trace %d: %w", jobID, err)
	}
	return result.Trace, nil
}

// RetryPipeline triggers a re-run of a pipeline's failed jobs via the
// gitlab MCP server.
func (a *AgentBridge) RetryPipeline(project string, pipelineID int) error {
	raw, err := a.client.CallToolWithTimeout("gitlab__retry_pipeline", map[string]any{
		"project":     project,
		"pipeline_id": pipelineID,
	}, pipelineDetailToolTimeout)
	if err != nil {
		return fmt.Errorf("retry pipeline %d: %w", pipelineID, err)
	}
	// A nil target still surfaces tool-level isError envelopes.
	if err := unmarshalGitLabResult(raw, nil); err != nil {
		return fmt.Errorf("retry pipeline %d: %w", pipelineID, err)
	}
	return nil
}
