...
const (
	...
	// AgentDeregisterEndpoint is the HTTP surface for presence deregistration.
	// `loom agent keepalive` posts here on SIGINT/SIGTERM so a terminating agent
	// drops out of the fleet view immediately instead of lingering until the
	// orphan reaper's threshold expires.
	AgentDeregisterEndpoint = "/api/agent/deregister"
...
// DeregisterRequest defines the shared body contract for
...
// Normalize trims identifiers.
func (r DeregisterRequest) Normalize() DeregisterRequest {
	r.AgentID = strings.TrimSpace(r.AgentID)
	return r
}
