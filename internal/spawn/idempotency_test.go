...
func TestSpawnWithKeyIsDeterministicAndIdempotent(t *testing.T) {
	...
	if second.SpawnID != first.SpawnID || second.AgentID != first.AgentID ||
		second.Status != first.Status || !second.StartedAt.Equal(first.StartedAt) ||
		second.Request.IdempotencyKey != first.Request.IdempotencyKey {
		t.Errorf("duplicate-key create replaced the logical state: first=%#v second=%#v", first, second)
	}
}
