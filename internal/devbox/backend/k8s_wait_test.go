...
func TestIsRuncInitFailure(t *testing.T) {
	// The literal error a devbox sandbox returns when the cgroup OOM killer
	// reaps runc init: nothing in it mentions memory, which is exactly why
	// it needs recognising.
	oomExec := errors.New(`OCI runtime exec failed: exec failed: unable to start container process: ` +
		`error during container init: error waiting for init process to start: procReady not received: unknown`)
	...
	tests := []struct {
		...
		{"procReady from a memory-killed exec", oomExec, true},
		{"container init failure", errors.New("error during container init: no such file"), true},
		{"nil error", nil, false},
		{"ordinary non-zero exit", errors.New("command terminated with exit code 2"), false},
		{"pod gone", errors.New(`pods "devbox-foo" not found`), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isRuncInitFailure(tt.err); got != tt.want {
				t.Errorf("isRuncInitFailure(%v) = %v, want %v", tt.err, got, tt.want)
			}
		})
	}
}
...
func TestContainerOOMKilled(t *testing.T) {
	oomState := corev1.ContainerState{
		Terminated: &corev1.ContainerStateTerminated{ExitCode: 137, Reason: "OOMKilled"},
	}

	t.Run("current state", func(t *testing.T) {
		pod := &corev1.Pod{Status: corev1.PodStatus{
			ContainerStatuses: []corev1.ContainerStatus{{Name: "devbox", State: oomState}},
		}}
		if got := containerOOMKilled(pod); got != "devbox" {
			t.Errorf("got %q, want %q", got, "devbox")
		}
	})

	t.Run("previous state after restart", func(t *testing.T) {
		// A container OOM-killed and restarted is Running again by the time
		// the next exec fails against it — the evidence is in lastState.
		pod := &corev1.Pod{Status: corev1.PodStatus{
			ContainerStatuses: []corev1.ContainerStatus{{
				Name:                 "devbox",
				State:                corev1.ContainerState{Running: &corev1.ContainerStateRunning{}},
				LastTerminationState: oomState,
			}},
		}}
		if got := containerOOMKilled(pod); got != "devbox" {
			t.Errorf("got %q, want %q", got, "devbox")
	...
	t.Run("healthy pod", func(t *testing.T) {
		pod := &corev1.Pod{Status: corev1.PodStatus{
			ContainerStatuses: []corev1.ContainerStatus{{
				Name:  "devbox",
				State: corev1.ContainerState{Running: &corev1.ContainerStateRunning{}},
		...
		if got := containerOOMKilled(pod); got != "" {
			t.Errorf("got %q, want empty", got)
		}
	})

	t.Run("nil pod", func(t *testing.T) {
		if got := containerOOMKilled(nil); got != "" {
			t.Errorf("got %q, want empty", got)
		}
	})

	t.Run("non-OOM termination is not reported", func(t *testing.T) {
		pod := &corev1.Pod{Status: corev1.PodStatus{
			ContainerStatuses: []corev1.ContainerStatus{{
				Name: "devbox",
				State: corev1.ContainerState{
...
func TestContainerMemoryLimit(t *testing.T) {
	...
	if got := containerMemoryLimit(pod, "devbox"); got != "4Gi" {
		t.Errorf("got %q, want %q", got, "4Gi")
	}
	if got := containerMemoryLimit(pod, "sidecar"); got != "" {
		t.Errorf("unknown container: got %q, want empty", got)
	}
	if got := containerMemoryLimit(nil, "devbox"); got != "" {
		t.Errorf("nil pod: got %q, want empty", got)
	}

	unlimited := &corev1.Pod{Spec: corev1.PodSpec{
		Containers: []corev1.Container{{Name: "devbox"}},
	}}
	if got := containerMemoryLimit(unlimited, "devbox"); got != "" {
		t.Errorf("no limit set: got %q, want empty", got)
	}
}

func TestExecMemoryHint(t *testing.T) {
	// Unconfirmed: runc init died and OOM is the likely-but-unproven cause,
	// so the text must hedge rather than assert an OOM kill.
	unconfirmed := execMemoryHint("4Gi", false)
	if !strings.Contains(unconfirmed, "4Gi") {
		t.Errorf("hint should name the limit: %q", unconfirmed)
	}
	if !strings.Contains(unconfirmed, "usual cause") {
		t.Errorf("unconfirmed hint should hedge: %q", unconfirmed)
	}
	if strings.Contains(unconfirmed, "was OOM-killed") {
		t.Errorf("unconfirmed hint must not assert an OOM kill: %q", unconfirmed)
	}

	// Confirmed: the kubelet reported OOMKilled, so state it plainly.
	confirmed := execMemoryHint("4Gi", true)
	if !strings.Contains(confirmed, "was OOM-killed") {
		t.Errorf("confirmed hint should state the OOM kill: %q", confirmed)
	}

	// Both forms must name the two knobs that fix it.
	for _, hint := range []string{unconfirmed, confirmed} {
		if !strings.Contains(hint, ".devbox.yaml") || !strings.Contains(hint, "DEVBOX_DEFAULT_MEMORY_MB") {
			t.Errorf("hint should name both remedies: %q", hint)
		}
	}

	if !strings.Contains(execMemoryHint("", false), "unset") {
		t.Errorf("unreadable limit should render as 'unset'")
	}
}
