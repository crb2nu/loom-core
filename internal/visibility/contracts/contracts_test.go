...
import (
	...
	visibilitycontracts "github.com/crb2nu/loom/internal/visibility/contracts"
...
func TestCIIncidentClassification_ExternalDependencyWireContract(t *testing.T) {
	incident := visibilitycontracts.CIIncidentClassification{
		Class:        visibilitycontracts.CIIncidentExternalDependency,
		Disposition:  visibilitycontracts.CIIncidentDispositionWaitDependency,
		Dependency:   "model_provider",
		Evidence:     "OpenAI request failed with 429 rate limit",
		Reason:       "failure points to a shared dependency outside the branch diff",
		Confidence:   0.93,
		RetryAllowed: false,
	}
	body, got := roundTrip(t, incident)
	requireFields(t, body,
		"class", "disposition", "dependency", "evidence", "reason", "confidence", "retry_allowed",
	)
	if got.Class != visibilitycontracts.CIIncidentExternalDependency {
		t.Fatalf("class = %q, want %q", got.Class, visibilitycontracts.CIIncidentExternalDependency)
	}
	if got.Class == visibilitycontracts.CIIncidentRepositoryRegression {
		t.Fatal("external dependency incident regressed to a repository regression")
	}
	if got.Disposition != visibilitycontracts.CIIncidentDispositionWaitDependency {
		t.Fatalf("disposition = %q, want %q", got.Disposition, visibilitycontracts.CIIncidentDispositionWaitDependency)
	}
	if got.Dependency != "model_provider" {
		t.Fatalf("dependency = %q, want model_provider", got.Dependency)
	}
	if got.RetryAllowed {
		t.Fatal("external dependency incident must not advertise retry permission")
	}
}
