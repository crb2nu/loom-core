...
func TestEscalationRecheck_DurableExponentialBackoff(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	item := &BacklogItem{ID: "ESC-SWEEP", Title: "sweep", State: BacklogEscalated, Priority: P2, CreatedBy: "test"}
	if err := st.Backlog.Put(ctx, item); err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, 8, 9, 0, 0, 0, 0, time.UTC)
	first, err := st.Backlog.DeferEscalationRecheck(ctx, item.ID, 17, now)
	if err != nil {
		t.Fatal(err)
	}
	if first.Streak != 1 || !first.RecheckAfter.Equal(now.Add(30*time.Minute)) {
		t.Fatalf("first = %+v", first)
	}
	second, err := st.Backlog.DeferEscalationRecheck(ctx, item.ID, 17, now)
	if err != nil {
		t.Fatal(err)
	}
	if second.Streak != 2 || !second.RecheckAfter.Equal(now.Add(time.Hour)) {
		t.Fatalf("second = %+v", second)
	}
	// A new DAO over the same database observes the throttle, modelling restart.
	got, err := (&BacklogDAO{db: st.DB()}).EscalationRecheck(ctx, item.ID)
	if err != nil || got.Streak != 2 || !got.RecheckAfter.Equal(second.RecheckAfter) {
		t.Fatalf("restart state = %+v, %v", got, err)
	}
	changed, err := st.Backlog.DeferEscalationRecheck(ctx, item.ID, 18, now)
	if err != nil {
		t.Fatal(err)
	}
	if changed.Streak != 1 {
		t.Fatalf("changed IID streak = %d, want 1", changed.Streak)
	}
}
...
func TestMigrate_v2_Idempotent(t *testing.T) {
	...
	// 005 backlog plan_id, 006 pipeline escalation_class, 007 spin_runs,
	// 008 backlog target_project, 009 bootstrapped_projects, 010 pipeline
	// escalation metadata, 011 transactional pipeline-start kernel, 012
	// quiescence indexes, 013 stage model/backend attribution, 014 transactional
	// Council admission/accounting kernel, 015 durable maintenance cursors,
	// 016 mr head transition ledger, 017 per-item memory journal,
	// 018 council-lane memory journal, 019 workflow start kernel,
...
func TestSquadOutcomes_SuccessRate(t *testing.T) {
	...
	for i, kind := range []SquadOutcomeKind{
		...
		SquadOutcomeFailed, SquadOutcomeMergedClean, SquadOutcomeFailed,
	} {
		runID := nextRunID(t, i)
...
// Council is upserted; the shared backlog row is inserted once. Caller passes
...
func seedPipelineRun(ctx context.Context, st *Store, runID, backlogID string, attempt int) error {
	...
	if _, err := st.Backlog.Get(ctx, backlogID); errors.Is(err, ErrNotFound) {
		if err := st.Backlog.Put(ctx, &BacklogItem{
