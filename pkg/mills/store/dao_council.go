...
// ErrCouncilRunAdmissionManaged means the run belongs to the atomic Council
// admission/finalization kernel and cannot be rewritten through the legacy
// upsert path.
var ErrCouncilRunAdmissionManaged = errors.New("council run is managed by atomic admission")
...
// Put inserts or replaces a legacy council run record. IDs created by
// ClaimCouncilStart are reservation-managed and immutable through this path;
// callers must use FinalizeCouncilRun, otherwise Put returns
// ErrCouncilRunAdmissionManaged.
func (d *CouncilDAO) Put(ctx context.Context, run *CouncilRun) error {
	...
	result, err := d.db.ExecContext(ctx, `
		...
		WHERE NOT EXISTS (
			SELECT 1 FROM council_budget_reservations WHERE run_id = ?
		)
	`,
	...
	if err != nil {
		return fmt.Errorf("council put %s rows: %w", run.ID, err)
	}
	if rows == 0 {
		return fmt.Errorf("council put %s: %w", run.ID, ErrCouncilRunAdmissionManaged)
	}
	return nil
}
