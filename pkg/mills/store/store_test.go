...
func TestOpen_AppliesMigrations(t *testing.T) {
	...
	want := []string{
		...
		"kpi_snapshots", "eval_scores", "events", "council_budget_reservations",
	}
	...
	for _, column := range []string{"grade", "grade_note", "grade_actor", "graded_at"} {
		var count int
		if err := st.DB().QueryRowContext(context.Background(), `SELECT count(*) FROM pragma_table_info('backlog_items') WHERE name = ?`, column).Scan(&count); err != nil || count != 1 {
...
func TestBacklog_GradeRoundTrip(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	gradedAt := time.Now().UTC().Truncate(time.Second)
	graded := &BacklogItem{
		ID: "BL-GRADED", Title: "graded", State: BacklogMerged, Priority: P2, CreatedBy: "test",
		Grade: "keep", GradeNote: "more like this", GradeActor: "operator.manual", GradedAt: &gradedAt,
	}
	if err := st.Backlog.Put(ctx, graded); err != nil {
		t.Fatal(err)
	}
	got, err := st.Backlog.Get(ctx, graded.ID)
	if err != nil {
		t.Fatal(err)
	}
	if got.Grade != graded.Grade || got.GradeNote != graded.GradeNote || got.GradeActor != graded.GradeActor || got.GradedAt == nil || !got.GradedAt.Equal(gradedAt) {
		t.Fatalf("grade round trip = %+v", got)
	}
	absent := &BacklogItem{ID: "BL-UNGRADED", Title: "ungraded", State: BacklogQueued, Priority: P2, CreatedBy: "test"}
	if err := st.Backlog.Put(ctx, absent); err != nil {
		t.Fatal(err)
	}
	got, err = st.Backlog.Get(ctx, absent.ID)
	if err != nil || got.Grade != "" || got.GradedAt != nil {
		t.Fatalf("absent grade round trip = %+v, %v", got, err)
	}
}
...
func TestPipelinePutRun_TerminalClassificationIsWriteOnce(t *testing.T) {
	for _, tc := range []struct {
		name      string
		persisted PipelineState
		requested PipelineState
	}{
		{name: "done_cannot_become_escalated", persisted: PipelineDone, requested: PipelineEscalated},
		{name: "escalated_cannot_become_done", persisted: PipelineEscalated, requested: PipelineDone},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			backlog := &BacklogItem{
				ID:        "BACKLOG-" + tc.name,
				Title:     "terminal classification test",
				State:     BacklogRunning,
				Priority:  P2,
				CreatedBy: "test",
			}
			if err := st.Backlog.Put(ctx, backlog); err != nil {
			...
			terminal := *run
			terminal.State = tc.persisted
			if err := st.Pipeline.PutRun(ctx, &terminal); err != nil {
				t.Fatalf("persist terminal state %q: %v", tc.persisted, err)
			}

			// Re-persisting the same classification is accepted and must not
			// be counted as a conflict.
			beforeSame := telemetry.TerminalStateConflictsTotal()
			if err := st.Pipeline.PutRun(ctx, &terminal); err != nil {
				t.Fatalf("idempotent terminal write %q: %v", tc.persisted, err)
			}
			if got := telemetry.TerminalStateConflictsTotal(); got != beforeSame {
				t.Fatalf("conflicts after same-state write = %d, want %d", got, beforeSame)
			}

			stale := terminal
			stale.Revision--
			beforeStale := telemetry.TerminalStateConflictsTotal()
			if err := st.Pipeline.PutRun(ctx, &stale); !errors.Is(err, ErrStaleWrite) {
				t.Fatalf("ordinary stale write error = %v, want ErrStaleWrite", err)
			}
			if got := telemetry.TerminalStateConflictsTotal(); got != beforeStale {
				t.Fatalf("conflicts after ordinary stale write = %d, want %d", got, beforeStale)
			}

			conflicting := terminal
			conflicting.State = tc.requested
			beforeConflict := telemetry.TerminalStateConflictsTotal()
			if err := st.Pipeline.PutRun(ctx, &conflicting); !errors.Is(err, ErrStaleWrite) {
				t.Fatalf("conflicting terminal write error = %v, want ErrStaleWrite", err)
			}
			if got := telemetry.TerminalStateConflictsTotal(); got != beforeConflict+1 {
				t.Fatalf("conflicts after rejected write = %d, want %d", got, beforeConflict+1)
			}
			...
			if persisted.State != tc.persisted {
				t.Fatalf("persisted state after rejected write = %q, want %q", persisted.State, tc.persisted)
			}
		})
	}
}
