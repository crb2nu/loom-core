...
func TestGitLabImporter_GlobalAdmissionBarrier(t *testing.T) {
	client := &stubClient{issues: []clients.IssueListItem{{IID: 1, ProjectID: 2, State: "opened"}}}
	im := NewGitLabImporter(client, newStubStore(), GitLabImporterConfig{}, nil)
	im.Enabled = func() bool { return false }
	if n, err := im.Tick(context.Background()); err != nil || n != 0 || client.calls != 0 || im.ActiveOperations() != 0 {
		t.Fatalf("disabled Tick() n=%d calls=%d active=%d err=%v", n, client.calls, im.ActiveOperations(), err)
	}
}
