...
// MCPHubServerHealth is the live, process-local dependency state for one MCP
// server. It changes on every completed logical CallTool, unlike the operator's
// startup-only "configured" bit. Consumers use it to fail autonomy closed when
// a once-healthy hub starts returning transport errors.
type MCPHubServerHealth struct {
	Known               bool
	Healthy             bool
...
// mcpHubServerState owns the context-aware single-flight gate for one server's
// request/response stream plus its live health. WebSocketTransport serializes
// individual Recv calls, but that is insufficient: two independent CallTool
// loops can otherwise consume and discard each other's response IDs. A channel
// gate both serializes the complete exchange and lets queued callers honor
// their own context instead of waiting behind a non-context-aware mutex.
type mcpHubServerState struct {
...
func newMCPHubServerState() *mcpHubServerState {
	return &mcpHubServerState{callSlot: make(chan struct{}, 1)}
}
...
func (c *MCPHubClient) CallTool(ctx context.Context, serverName, toolName string, args map[string]any) (string, error) {
	...
	} else if errors.Is(err, context.DeadlineExceeded) || isTransportError(err) {
		state.recordFailure(time.Now(), err)
	} else if !errors.Is(err, context.Canceled) {
		// A JSON-RPC/tool error proves the request/response session itself is
		// alive. Keep it visible in calls_total{outcome="error"}, but do not
		// fail the transport capability closed for a caller's invalid args or a
...
func mcpHubCallOutcome(err error) string {
	switch {
	case err == nil:
		return "ok"
...
func (c *MCPHubClient) serverStateFor(serverName string) *mcpHubServerState {
	c.mu.Lock()
	defer c.mu.Unlock()
	state := c.serverState[serverName]
	if state == nil {
		state = newMCPHubServerState()
		c.serverState[serverName] = state
	}
	return state
}

// ServerHealth returns a lock-safe snapshot. Unknown means the process has not
// completed a logical call to that server yet; callers should fail closed when
// the server is required for autonomy.
func (c *MCPHubClient) ServerHealth(serverName string) MCPHubServerHealth {
	if c == nil || strings.TrimSpace(serverName) == "" {
		return MCPHubServerHealth{}
	}
	state := c.serverStateFor(serverName)
	state.mu.RLock()
	...
	return MCPHubServerHealth{
		Known:               state.known,
		Healthy:             state.known && state.consecutiveFailures == 0,
...
func (s *mcpHubServerState) recordSuccess(now time.Time) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.known = true
...
func (s *mcpHubServerState) recordFailure(now time.Time, err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.known = true
	s.consecutiveFailures++
	s.lastFailure = now.UTC()
	s.lastError = truncateText(err.Error(), 512)
}
