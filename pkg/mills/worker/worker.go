...
type WorkerRequest struct {
	...
	Namespace       string
	Substrate       string
	// CompletionHoldSeconds keeps a successful worker invocation attached to
	// its durable spawn for a bounded, driver-owned foreground hold. Zero keeps
	// normal worker behavior. The S1c canary uses this instead of asking the
	// model to manage an asynchronous shell session, so the crash window is
