...
type CrossRunScheduler struct {
	...
	Now func() time.Time
	// Enabled is the global admission barrier. Nil preserves the historical
	// always-enabled behavior for callers/tests; production wires
	// policy.enabled so the weekly checker cannot start during maintenance.
	Enabled func() bool
...
// ActiveOperations reports weekly cross-run checks currently executing.
func (s *CrossRunScheduler) ActiveOperations() int64 {
	if s == nil {
		return 0
	}
