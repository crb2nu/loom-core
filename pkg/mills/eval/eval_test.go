...
func TestJudge_ErrorPreservesProviderCost(t *testing.T) {
	providerErr := errors.New("parse provider verdict")
	judge := &Judge{Criteria: []Criterion{&ContradictionFree{LLM: &FakeLLMJudge{
		Err: providerErr, CostUSD: 0.27, Backend: "litellm", CostUnpriced: true,
	}}}}

	verdict, err := judge.Run(context.Background(), Input{Sidecar: goodSidecar()})
	if !errors.Is(err, providerErr) {
		t.Fatalf("judge error = %v, want provider error", err)
	}
	if verdict == nil || verdict.CostUSD != 0.27 {
		t.Fatalf("verdict = %+v, want cost 0.27 on failure", verdict)
	}
	if len(verdict.ProviderCosts) != 1 || verdict.ProviderCosts[0].Backend != "litellm" {
		t.Fatalf("provider costs = %+v, want litellm attribution", verdict.ProviderCosts)
	}
	if !verdict.CostUnpriced {
		t.Fatal("verdict did not preserve unpriced remote provider marker")
	}
}
