...
func (a *OutcomeAttributor) computeScore(ctx context.Context, run *store.PipelineRun, item *store.BacklogItem) (float64, map[string]any, error) {
	...
	// gate_pass_rate. Skips are advisory non-verdicts (e.g. the scope gate
	// with no slices to enforce) — excluded from the denominator so they do
	// not depress the score like a fail, matching the KPI writer's
	// gate_pass_rate semantics.
	passes, total := 0, 0
	for _, g := range gates {
		if g.Outcome == store.GateOutcomeSkip {
			continue
		}
		total++
