...
func TestCrossRunScheduler_HonorsGlobalAdmissionBarrier(t *testing.T) {
	st := openTestStore(t)
	now := time.Date(2026, 4, 26, 6, 0, 0, 0, time.UTC)
	sched := NewCrossRunScheduler(&CrossRunChecker{Store: st, Now: func() time.Time { return now }})
	sched.Now = func() time.Time { return now }
	sched.Enabled = func() bool { return false }
	sched.maybeFire(context.Background())
	scores, err := st.Eval.ListSince(context.Background(), now.Add(-time.Hour), 100)
	if err != nil || len(scores) != 0 || sched.ActiveOperations() != 0 {
		t.Fatalf("disabled scheduler scores=%d active=%d err=%v", len(scores), sched.ActiveOperations(), err)
	}
}
