...
func TestLLMGate_TransportErrorStillSurfacesAsError(t *testing.T) {
	// 5xx, timeout, 400/429 and other network errors are infrastructure
	// failures: the judge never graded the diff. The gate must surface them as
	// err — NOT as a Pass=false verdict, which would read as a graded fail on
	// the diff. Classification and the bounded free re-judge live in the runner
	// (Runner.handleGateError), which is what turns this into an
	// [class=infra]/[class=transient] escalation rather than an unclassified
	// park (issue #378).
	...
	out, err := g.Evaluate(context.Background(), StageInput{})
	if err == nil {
		t.Fatalf("transport error must return non-nil err so the runner classifies it")
	}
	if !strings.Contains(err.Error(), "status 500") {
		t.Errorf("err should preserve underlying transport message, got %v", err)
	}
	if out.JudgedBy != "" || len(out.Reasons) > 0 {
		t.Errorf("a failed judge CALL must not mint a verdict, got %+v", out)
	}
}
...
// The dissent tiebreaker fires ONLY when the primary judge fails a gate
// whose deterministic tests stage passed (escalations #309/#310: a judge
// fixation failed a correct SQL idiom across three runs; model diversity is
// the structural fix prompt clauses could not deliver).
func TestLLMGate_TiebreakerOverrulesPrimaryFailWhenTestsPassed(t *testing.T) {
	g := &LLMGate{
		GateName: "pr_self_review", RubricName: PRSelfReviewRubricName, Threshold: 0.7,
		Judge:          &FakeRubricJudge{Default: RubricVerdict{Score: 0.5, Reasons: []string{"speculative sql concern"}, Model: "gemma"}},
	...
	if !out.Pass {
		t.Fatalf("tiebreaker pass should overrule primary fail; got %+v", out)
	}
	if want := "flexinfer:gemma overruled-by anthropic:claude-sonnet-5"; out.JudgedBy != want {
		t.Errorf("JudgedBy = %q, want %q", out.JudgedBy, want)
	}
	joined := strings.Join(out.Reasons, " | ")
	if !strings.Contains(joined, "speculative sql concern") {
		t.Errorf("primary dissent reasons must be preserved for audit; got %q", joined)
	}
}

func TestLLMGate_TiebreakerCorroboratesPrimaryFail(t *testing.T) {
	g := &LLMGate{
		GateName: "pr_self_review", RubricName: PRSelfReviewRubricName, Threshold: 0.7,
		Judge:          &FakeRubricJudge{Default: RubricVerdict{Score: 0.5, Reasons: []string{"real defect"}, Model: "gemma"}},
		Tiebreaker:     &FakeRubricJudge{Default: RubricVerdict{Score: 0.4, Reasons: []string{"agrees: real defect"}, Model: "claude-sonnet-5"}},
		TiebreakerName: "anthropic",
	}
	out, err := g.Evaluate(context.Background(), StageInput{TestsPassed: true})
	if err != nil {
	...
	if out.Pass {
		t.Fatal("corroborated fail must stay a fail")
	}
	if !strings.Contains(out.JudgedBy, "corroborated-by anthropic:claude-sonnet-5") {
		t.Errorf("JudgedBy should record corroboration; got %q", out.JudgedBy)
	}
	joined := strings.Join(out.Reasons, " | ")
	if !strings.Contains(joined, "agrees: real defect") {
		t.Errorf("tiebreaker reasons should be appended; got %q", joined)
	}
}

func TestLLMGate_TiebreakerErrorFailsClosed(t *testing.T) {
	g := &LLMGate{
		GateName: "pr_self_review", RubricName: PRSelfReviewRubricName, Threshold: 0.7,
		Judge:          &FakeRubricJudge{Default: RubricVerdict{Score: 0.5, Model: "gemma"}},
		Tiebreaker:     &FakeRubricJudge{Err: errors.New("anthropic 529")},
		TiebreakerName: "anthropic",
	}
	out, err := g.Evaluate(context.Background(), StageInput{TestsPassed: true})
	if err != nil {
		t.Fatalf("a tiebreaker error must not surface as a gate error: %v", err)
	}
	if out.Pass {
		t.Fatal("tiebreaker outage must fail closed to the primary verdict")
	}
	if !strings.Contains(strings.Join(out.Reasons, " "), "tiebreaker anthropic unavailable") {
		t.Errorf("unresolved dissent should be recorded; got %v", out.Reasons)
	}
}

func TestLLMGate_NoTiebreakerWithoutTestsPassed(t *testing.T) {
	tb := &recordingJudge{response: RubricVerdict{Score: 0.9}}
	g := &LLMGate{
		GateName: "pr_self_review", RubricName: PRSelfReviewRubricName, Threshold: 0.7,
		Judge:      &FakeRubricJudge{Default: RubricVerdict{Score: 0.5, Model: "gemma"}},
		Tiebreaker: tb,
	}
	out, err := g.Evaluate(context.Background(), StageInput{TestsPassed: false})
	if err != nil {
		t.Fatalf("evaluate: %v", err)
	}
	if out.Pass {
		t.Fatal("primary fail without tests evidence must stand")
	}
	if tb.calls != 0 {
		t.Fatal("tiebreaker must not run when the tests stage did not pass (no dissent to resolve)")
	}
}

func TestLLMGate_NoTiebreakerOnPrimaryPass(t *testing.T) {
	tb := &recordingJudge{response: RubricVerdict{Score: 0.1}}
	g := &LLMGate{
		GateName: "pr_self_review", RubricName: PRSelfReviewRubricName, Threshold: 0.7,
		Judge:      &FakeRubricJudge{Default: RubricVerdict{Score: 0.9, Model: "gemma"}},
		Tiebreaker: tb,
	}
	out, err := g.Evaluate(context.Background(), StageInput{TestsPassed: true})
	if err != nil {
	...
	if !out.Pass {
		t.Fatal("primary pass must stand")
	}
	if tb.calls != 0 {
