...
type LLMGate struct {
	...
	Disabled bool
	// Tiebreaker, when non-nil, is consulted for a second opinion whenever
	// the primary judge FAILS a gate whose deterministic tests stage PASSED
	// (StageInput.TestsPassed) — that combination is a dissent between a
	// sampled-once local model and the strongest evidence the pipeline
	// holds, not a verdict. A different model family defeats fixations that
	// same-model retries and prompt clauses cannot (escalations #309/#310:
	// the local judge failed a correct dynamic-WHERE SQL idiom across three
	// runs and nine review attempts, re-hedging its wording around every
	// grounding clause). The tiebreaker runs ONLY on dissent, so its cost
	// is zero on the happy path.
	//
	//   - Tiebreaker passes  → gate passes; JudgedBy records the overrule
	//     and Reasons preserve the primary's dissent for the audit trail.
	//   - Tiebreaker fails   → corroborated fail; the normal retry loop
	//     proceeds with feedback now worth an agent re-spawn.
	//   - Tiebreaker errors  → fail CLOSED to the primary verdict (an
	//     Anthropic outage must never silently weaken a gate).
...
func (g *LLMGate) Evaluate(ctx context.Context, in StageInput) (Outcome, error) {
	...
	out := Outcome{
		...
		Judgements: []Judgement{{
			Role: JudgeRolePrimary, Model: model,
			Score: v.Score, Threshold: threshold, Pass: v.Score >= threshold,
		}},
	}
	if !out.Pass {
...
// green deterministic tests stage with one second opinion from a different
// backend. See the Tiebreaker field doc for the outcome contract.
func (g *LLMGate) breakTie(ctx context.Context, in StageInput, primary Outcome, primaryModel string, threshold float64) (Outcome, error) {
	name := g.TiebreakerName
	if name == "" {
		name = "tiebreaker"
	}
	...
	if terr != nil {
		...
		// the dissent went unresolved.
		g.logger().Warn("llm gate: tiebreaker unavailable; keeping primary fail verdict",
			"gate", g.GateName, "rubric", g.RubricName, "tiebreaker", name, "error", terr)
		primary.Reasons = append(primary.Reasons,
			fmt.Sprintf("[tiebreaker %s unavailable: verdict stands unresolved against a passing tests stage]", name))
		return primary, nil
	}
	tModel := tv.Model
	if tModel == "" {
	...
	if tv.Score >= threshold {
		...
		return Outcome{
			...
			Reasons: append(
				...
				primary.Reasons...),
			// The primary's own dissenting score rides along: an overrule is
			// exactly the evidence a calibration read needs, and folding it
			// into the tiebreaker's row would hide which model was wrong.
			Judgements: appendTiebreakerJudgement(primary.Judgements, tModel, tv.Score, threshold),
		}, nil
	}
	g.logger().Info("llm gate: tiebreaker corroborated primary fail",
		"gate", g.GateName, "rubric", g.RubricName,
		"primary_model", primaryModel, "tiebreaker_model", tModel,
		"tiebreaker_score", tv.Score)
	...
	primary.Judgements = appendTiebreakerJudgement(primary.Judgements, tModel, tv.Score, threshold)
	return primary, nil
}

// appendTiebreakerJudgement records the second opinion alongside the primary's
// without aliasing the caller's slice.
func appendTiebreakerJudgement(primary []Judgement, model string, score, threshold float64) []Judgement {
	out := make([]Judgement, 0, len(primary)+1)
	out = append(out, primary...)
	return append(out, Judgement{
		Role: JudgeRoleTiebreaker, Model: model,
...
func RegisterLLMGates(r *Registry, judge RubricJudge) {
	RegisterLLMGatesWithTiebreaker(r, judge, nil, "")
}

// RegisterLLMGatesWithTiebreaker is RegisterLLMGates plus the dissent
