...
// ErrNoAuditReviewersAvailable reports that every configured bulk reviewer
...
//
// Returns an error for top-level failures (validation, rubric render) or when
// every bulk reviewer is unavailable. Individual member errors are folded into
// Members[i].ParseErr + Result.SkippedMembers so partial degradation does not
// fail an otherwise valid audit.
func (d *Dispatcher) Run(ctx context.Context, req *Request) (*Result, error) {
	...
	if bulkSkipped == len(req.Pool) {
		...
		return nil, fmt.Errorf("%w: all %d configured bulk reviewers failed", ErrNoAuditReviewersAvailable, len(req.Pool))
	}
...
// MemberOutputs. Members whose Backend is unregistered or whose call fails are
// folded into skipped; invalid responses carry ParseErr without being skipped
// because the provider did return audit evidence.
