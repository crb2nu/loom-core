...
func TestDispatcher_AllReviewerCallsFailReturnsUnavailable(t *testing.T) {
	rev := &fakeReviewer{
		backend:     "flexinfer",
		defaultCost: 0.04,
		errByModel: map[string]error{
			"llama-4-70b": errors.New("upstream 503"),
			"qwen-3-32b":  errors.New("upstream 503"),
		},
	}
	d := newDispatcher(t, rev)
	...
	if res != nil {
		t.Fatalf("result = %#v, want nil when every reviewer call fails", res)
	}
...
func TestDispatcher_AllReviewerCallsFailPreservesParentContextError(t *testing.T) {
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rev := &fakeReviewer{
				backend: "flexinfer",
				errByModel: map[string]error{
					"llama-4-70b": tt.wantErr,
					"qwen-3-32b":  tt.wantErr,
				},
			}
			d := newDispatcher(t, rev)
			res, err := d.Run(ctx, makeRequest(
				[]PoolMember{
					{Backend: "flexinfer", Model: "llama-4-70b"},
					{Backend: "flexinfer", Model: "qwen-3-32b"},
				},
			...
			if errors.Is(err, ErrNoAuditReviewersAvailable) {
				t.Fatalf("run error = %v, must preserve the parent context error", err)
			}
			if res != nil {
				t.Fatalf("result = %#v, want nil after parent context cancellation", res)
			}
		})
	}
}
