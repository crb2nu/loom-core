...
type QueueWorker struct {
	...
	stopOnce   sync.Once
	logger     *slog.Logger
	// outstanding spans the entire accepted-job lifetime: it increments before
	// a successful channel handoff can be observed and decrements only after
	// process returns. Keeping queue + execution in one counter eliminates the
	// dequeue-to-active sampling gap that separate len(jobs)/active reads have.
	outstanding atomic.Int64

	// OnRecorded fires after a finding has been persisted to the
	// canonical store. Reserved for a follow-up hook that auto-opens
	// advisory issues for low-survival findings (no production wiring
	// yet). Errors from the hook are logged + swallowed — the worker
	// keeps draining.
...
func (w *QueueWorker) Enqueue(req Request) bool {
	...
	// Increment before publishing to jobs. A receiver may run immediately after
	// the send, so incrementing afterward creates a false-zero window between
	// dequeue and the active counter update. A rejected enqueue rolls back.
	w.outstanding.Add(1)
	select {
	case w.jobs <- req:
		return true
...
// Activity reports every accepted audit job that has not finished processing,
// whether it is still queued or currently executing. The queue is intentionally
// not durable, so deliberate operator restarts must observe zero before fault
// injection.
func (w *QueueWorker) Activity() int64 {
	if w == nil {
		return 0
	}
...
func (t *Triggers) OnPipelineMerged(ctx context.Context, run *store.PipelineRun, item *store.BacklogItem) error {
	...
	diff = stripChangelogFromDiff(diff)
	if !strings.Contains(diff, "diff --git ") {
		// The merge touched ONLY the changelog — nothing auditable
		// remains after the strip, so skip rather than audit a marker.
		t.warn("audit: merged diff changelog-only; skipping enqueue", "run", run.ID)
		return nil
	}
	req := Request{
