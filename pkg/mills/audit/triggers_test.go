...
func TestQueueWorker_EnqueueAndDrain(t *testing.T) {
	...
	if got := env.worker.Activity(); got != 1 {
		t.Fatalf("activity after enqueue = %d, want 1", got)
	}

	env.drain(context.Background())
	if got := env.worker.Activity(); got != 0 {
		t.Fatalf("activity after process completion = %d, want 0", got)
	}
...
func TestQueueWorker_AllReviewerCallsFailDoesNotRecordOrPublish(t *testing.T) {
	env := newTriggerEnv(t)
	env.reviewer.err = errors.New("upstream 503")
	hookCalls := 0
	env.worker.OnRecorded = func(context.Context, *store.AuditFinding) error {
		hookCalls++
		return nil
	}

	if !env.worker.Enqueue(Request{
		SubjectKind: store.AuditSubjectPipelineMerge,
		SubjectID:   "PIPE-PROVIDER-OUTAGE",
	...
	rows, err := env.store.Audit.ListForSubject(context.Background(),
		store.AuditSubjectPipelineMerge, "PIPE-PROVIDER-OUTAGE")
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(rows) != 0 {
		t.Fatalf("audit rows = %d, want 0 when every reviewer call fails", len(rows))
	}
	if hookCalls != 0 {
		t.Fatalf("OnRecorded calls = %d, want 0", hookCalls)
	}
	if got := env.reviewer.Calls(); got != 1 {
...
func TestQueueWorker_ActivitySpansDequeueThroughProcessCompletion(t *testing.T) {
	env := newTriggerEnv(t)
	...
	if !env.worker.Enqueue(Request{
		SubjectKind: store.AuditSubjectCouncilArtifact,
		SubjectID:   "COUNCIL-ACTIVITY-HANDOFF",
		Artifact:    "hello",
	}) {
	...
	case <-time.After(2 * time.Second):
		t.Fatal("worker did not reach completion hook")
	}
	if queued := len(env.worker.jobs); queued != 0 {
		t.Fatalf("job should be dequeued, queue length = %d", queued)
	}
	if got := env.worker.Activity(); got != 1 {
		t.Fatalf("activity during post-record processing = %d, want 1", got)
	}

	// Closing the input does not complete the accepted job while its final hook
	// is still executing.
	env.worker.Stop()
	if got := env.worker.Activity(); got != 1 {
		t.Fatalf("activity after Stop during processing = %d, want 1", got)
	}
	close(releaseHook)
	select {
	case <-runDone:
	case <-time.After(2 * time.Second):
		t.Fatal("worker did not finish after releasing hook")
	}
	if got := env.worker.Activity(); got != 0 {
		t.Fatalf("activity after process completion = %d, want 0", got)
	}
...
func TestQueueWorker_ActivityClearsOnProcessError(t *testing.T) {
	env := newTriggerEnv(t)
	if !env.worker.Enqueue(Request{}) {
		t.Fatal("enqueue should succeed")
	}
	...
	env.drain(context.Background())
	if got := env.worker.Activity(); got != 0 {
		t.Fatalf("activity after failed processing = %d, want 0", got)
	}
}
...
func TestQueueWorker_FullQueueDrops(t *testing.T) {
	...
	if got, want := env.worker.Activity(), int64(cap(env.worker.jobs)); got != want {
		t.Errorf("activity after rejected enqueue = %d, want %d", got, want)
	}
	env.worker.Stop()
}
...
func TestTriggers_OnPipelineMergedStripsChangelog(t *testing.T) {
	env := newTriggerEnv(t)
	env.triggers.LoadMergedDiff = func(_ context.Context, _ *store.PipelineRun, _ *store.BacklogItem) (string, error) {
		return changelogSection + "\n" + codeSection, nil
	}
	if err := env.triggers.OnPipelineMerged(context.Background(),
		&store.PipelineRun{ID: "PIPE-STRIP"}, &store.BacklogItem{ID: "MILLS-STRIP"}); err != nil {
		t.Errorf("OnPipelineMerged should not error; got %v", err)
	}
	env.drain(context.Background())

	rows, err := env.store.Audit.ListForSubject(context.Background(),
		store.AuditSubjectPipelineMerge, "PIPE-STRIP")
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(rows) != 1 {
...
func TestTriggers_OnPipelineMergedSkipsChangelogOnlyDiff(t *testing.T) {
	env := newTriggerEnv(t)
	env.triggers.LoadMergedDiff = func(_ context.Context, _ *store.PipelineRun, _ *store.BacklogItem) (string, error) {
		return changelogSection, nil
	}
	if err := env.triggers.OnPipelineMerged(context.Background(),
		&store.PipelineRun{ID: "PIPE-CLONLY"}, &store.BacklogItem{ID: "MILLS-CLONLY"}); err != nil {
		t.Errorf("OnPipelineMerged should not error; got %v", err)
	}
	env.drain(context.Background())

	rows, err := env.store.Audit.ListForSubject(context.Background(),
		store.AuditSubjectPipelineMerge, "PIPE-CLONLY")
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(rows) != 0 {
		t.Fatalf("changelog-only merge must not be audited; got %d rows", len(rows))
	}
}
