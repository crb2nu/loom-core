...
type DAOJournal struct {
	...
	ensured sync.Map // run ids whose workflow_runs row exists
}
...
func (j *DAOJournal) Append(r Record) error {
	...
	if r.Status == StatusQuarantined {
		stored, err := j.dao.QuarantineStep(j.ctx, r.RunID, r.StepKey, r.CallHash)
		if err != nil {
			if errors.Is(err, store.ErrStepCallHashMismatch) {
				return quarantineHashMismatch(r, stored)
			}
			return fmt.Errorf("workflow journal quarantine %s/%s: %w", r.RunID, r.StepKey, err)
		}
		return nil
	}
	...
	stored, err := j.dao.AppendStep(j.ctx, step)
	if err != nil {
		if errors.Is(err, store.ErrStepCallHashMismatch) {
...
// is idempotent and cached per run id to avoid a write on every append.
// EnsureWorkflowRun is insert-only: when the scheduler/API pre-created the run,
// its real template, pinned versions, params, and session binding survive this
// journal bootstrap.
func (j *DAOJournal) ensureRun(runID string) error {
	if _, ok := j.ensured.Load(runID); ok {
		return nil
	}
	now := time.Now().UTC()
	...
	if err := j.dao.EnsureWorkflowRun(j.ctx, run); err != nil {
		return fmt.Errorf("workflow journal ensure run %s: %w", runID, err)
	}
	j.ensured.Store(runID, struct{}{})
	return nil
}

func quarantineHashMismatch(incoming Record, stored *store.WorkflowStep) *QuarantineError {
	want := incoming.CallHash
	if stored != nil {
		want = stored.CallHash
	}
	return &QuarantineError{StepKey: incoming.StepKey, Want: want, Got: incoming.CallHash}
}
