...
func TestRouterTasteGuardPreservesLegacyConfidenceBelowThreshold(t *testing.T) {
	r, st := newRouterFixture(t, map[string]string{"hud-frontend": validHUDFrontend})
	path := "internal/hud/frontend/**"
	seedOutcomes(t, st, "hud-frontend", path, 4, 1)
	// Four graded regrets are deliberately below the default guard. The
	// confidence must remain the legacy success rate byte-for-byte.
	if _, err := st.DB().Exec(`UPDATE squad_outcomes SET grade='regret' WHERE outcome='merged_clean'`); err != nil {
		t.Fatal(err)
	}
	got, n, err := r.confidence(context.Background(), "hud-frontend", path)
	if err != nil || n != 5 || got != .8 {
		t.Fatalf("below guard: confidence=%v n=%d err=%v", got, n, err)
	}
	// Grading the fifth outcome crosses the guard and activates the taste term.
	if _, err := st.DB().Exec(`UPDATE squad_outcomes SET grade='regret' WHERE outcome='failed'`); err != nil {
		t.Fatal(err)
	}
	got, n, err = r.confidence(context.Background(), "hud-frontend", path)
	if err != nil || n != 5 || math.Abs(got-.64) > 1e-12 {
		t.Fatalf("at guard: confidence=%v n=%d err=%v", got, n, err)
	}
}
...
// fakeRoutingGate implements PolicyGate + RoutingPolicy so tests can
// exercise the live policy.squads.routing.min_confidence override.
type fakeRoutingGate struct {
	enabled bool
	minConf float64
}

func (f *fakeRoutingGate) SquadsEnabled() bool          { return f.enabled }
func (f *fakeRoutingGate) SquadsMinConfidence() float64 { return f.minConf }

// TestRouter_PolicyMinConfidenceOverride proves the policy-served floor is
// honored per Pick: a baseline-confidence match (0.65) that routes under
// the compiled-in floor (0.6) must fall back when policy raises the floor
// above it, and a zero policy value must leave the compiled-in floor
// untouched.
func TestRouter_PolicyMinConfidenceOverride(t *testing.T) {
	r, _ := newRouterFixture(t, map[string]string{
		"hud-frontend": validHUDFrontend,
	})
	item := itemWithFiles("internal/hud/frontend/src/lib/components/SpawnPanel.svelte")

	// Policy floor above the cold-start baseline: routing must fall back.
	r.Policy = &fakeRoutingGate{enabled: true, minConf: 0.9}
	dec, err := r.Pick(context.Background(), item)
	if err != nil {
		t.Fatalf("pick: %v", err)
	}
	if dec.SquadName != FallbackName {
		t.Errorf("policy floor 0.9 ignored: got %q (conf=%.3f); want fallback",
			dec.SquadName, dec.Confidence)
	}

	// Zero policy value: compiled-in default applies, baseline 0.65 routes.
	r.Policy = &fakeRoutingGate{enabled: true, minConf: 0}
	dec, err = r.Pick(context.Background(), item)
	if err != nil {
		t.Fatalf("pick: %v", err)
	}
	if dec.SquadName != "hud-frontend" {
		t.Errorf("zero policy floor must keep default: got %q want hud-frontend", dec.SquadName)
	}
}
