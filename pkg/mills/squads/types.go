...
//
// The on-disk source of truth for squad configuration is the
// `configmap-squads.yaml` ConfigMap in platform/gitops/k3s/mills/, mounted
// into the operator at /etc/loom-mills/squads (override: --squads-path).
// The Loader watches that directory, validates manifests, and reflects each
// one into the canonical `squads` table (added by migration 002_v2.sql).
// The Router consults the table at routing time; it never re-parses YAML
// inline.
//
// Manifest fields tests, gates, ensemble, budget_share, and
// recursion_enabled are parsed, persisted, and displayed but NOT yet
// enforced anywhere — they are reserved for the dispatch-time integration
// (see .loom/brainstorm-mill-staff-consolidation-2026-08-01.md, slice S4).
// The enforced surface today is: paths (routing), enabled, and the outcome
// history feeding routing confidence.
//
