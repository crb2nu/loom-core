...
const DefaultMinGradedOutcomes = 5
const tasteConfidenceWeight = 0.2
...
// RoutingPolicy is the optional extension of PolicyGate that serves the
// live `policy.squads.routing.min_confidence` value. When the gate wired
// into Router.Policy also implements this and returns a positive value,
// Pick uses it instead of the compiled-in Router.MinConfidence — so a
// policy hot-reload retunes the routing floor on the next tick. Zero or
// negative values fall back to the router's own configuration.
type RoutingPolicy interface {
	SquadsMinConfidence() float64
}
...
func NewRouter(loader *Loader, st *store.Store) *Router {
	return &Router{
		...
		MinGradedOutcomes:  DefaultMinGradedOutcomes,
	}
}
...
func (r *Router) Pick(ctx context.Context, item *store.BacklogItem) (Decision, error) {
	...
	if rp, ok := r.Policy.(RoutingPolicy); ok {
		if v := rp.SquadsMinConfidence(); v > 0 {
			minConf = v
		}
	}
	...
	if best.confidence < minConf {
		return Decision{
			...
			Reason: fmt.Sprintf("best squad %q confidence %.3f < min %.3f",
				best.name, best.confidence, minConf),
		}, nil
	}
	return Decision{
		SquadName:  best.name,
...
func (r *Router) confidence(ctx context.Context, squad, pathClass string) (float64, int, error) {
	rate, n, taste, graded, err := r.store.Squads.OutcomeStats(ctx, squad, pathClass, r.SampleWindow)
	if err != nil {
	...
	minGraded := r.MinGradedOutcomes
	if minGraded <= 0 {
		minGraded = DefaultMinGradedOutcomes
	}
	if graded >= minGraded {
		rate = (1-tasteConfidenceWeight)*rate + tasteConfidenceWeight*taste
	}
	return rate, n, nil
}
