...
func TestReconciler_SquadAttributionIsStableAcrossDispatchRetry(t *testing.T) {
	env := newRecEnv(t, nil)
	router := &fakeSquadRouter{out: SquadDecision{
		SquadName: "first-squad", PathClass: "pkg/mills/**", Confidence: 0.8,
	}}
	env.rec.SquadRouter = router
	env.starter.fail = errors.New("starter unavailable")
	item := seedQueuedItemForRouting(t, env, "MILLS-ROUTE-RETRY", "pkg/mills/reconciler.go")

	if _, err := env.rec.Tick(context.Background()); err != nil {
		t.Fatalf("first tick: %v", err)
	}
	router.out = SquadDecision{SquadName: "second-squad", PathClass: "pkg/**", Confidence: 0.9}
	...
	if err != nil || len(runs) != 1 {
		t.Fatalf("pipeline runs=%d err=%v", len(runs), err)
	}
	events, err := env.store.Events.ListBySubject(context.Background(), "pipeline_run", runs[0].ID, 50)
	if err != nil {
		t.Fatalf("list attribution events: %v", err)
	}
	count := 0
	for _, event := range events {
		if event.Kind != "reconciler.squad_routed" {
			continue
		}
		count++
		if got, _ := event.Payload["squad_name"].(string); got != "first-squad" {
			t.Fatalf("retry changed squad attribution to %q", got)
		}
	}
	if count != 1 {
		t.Fatalf("squad attribution event count=%d want 1", count)
	}
}
