...
func (i *Integrator) Run(ctx context.Context, run *store.PipelineRun, item *store.BacklogItem) error {
	...
	if err != nil {
		return i.escalateWithItem(ctx, &parentRun, item, Classify(err), fmt.Sprintf("sub-run failure: %v", err))
	}
	...
	if integrationBranch == "" {
		return i.escalateWithItem(ctx, &parentRun, item, ClassConfig, "integration branch unavailable")
	}
	...
	if err != nil {
		return i.escalateWithItem(ctx, &parentRun, item, Classify(err), fmt.Sprintf("merge failed: %v", err))
	}
	if mergeResp.Conflict {
		...
		return i.escalateWithItem(ctx, &parentRun, item, ClassConfig, fmt.Sprintf("merge conflict in %d files", len(mergeResp.ConflictedFiles)))
	}
...
func (i *Integrator) escalateWithItem(ctx context.Context, run *store.PipelineRun, item *store.BacklogItem, cls ErrorClass, reason string) error {
	t := i.now()
	...
	// Sync the persisted classification back onto the run: i.Escalator.Handle
	// below builds its FailureRecord from this struct, and without the sync it
	// re-infers a class from the reason prose and overwrites the declared one.
	applyEscalationMetadata(run, stampEscalationMetadataOn(ctx, i.Store.Pipeline, run.ID, cls, reason, "", i.Logger))
	if item != nil {
		current, err := i.Store.Backlog.TransitionState(
