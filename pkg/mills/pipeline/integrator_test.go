...
type recordingEscalationHandler struct {
	reason string
	// Classification as the Escalator sees it. BuildRecord reads these off the
	// run struct it is handed, so they are what actually reaches the handoff.
	sawEscalationClass string
	sawFailureClass    string
}

func (h *recordingEscalationHandler) Handle(
	_ context.Context,
	...
	h.reason = reason
	if run != nil {
		h.sawEscalationClass = run.EscalationClass
		h.sawFailureClass = run.FailureClass
	}
	return nil
}
...
func TestIntegrator_MergeFailureDetailsReachEscalator(t *testing.T) {
	st, run, item := newIntegratorEnv(t)
	const failure = "publish integration branch failed (exit 1): remote rejected protected branch"
	handler := &recordingEscalationHandler{}
	itg := NewIntegrator(
		st,
		&recordingSubRunner{store: st},
		&fakeAllocator{},
		&fakeMerger{err: errors.New(failure)},
	)
	itg.Escalator = handler

	if err := itg.Run(context.Background(), run, item); err != nil {
		t.Fatalf("run: %v", err)
	}
	if !strings.Contains(handler.reason, failure) {
		t.Errorf("escalation reason = %q, want actionable failure %q", handler.reason, failure)
	}
}

// The Integrator's declared class must survive all the way to the Escalator.
// It stamps the class to the DB, then hands the Escalator the in-memory run;
// when the stamp was not synced back onto that struct, BuildRecord fell through
// to inferring a class from the reason prose and SetEscalationMetadata's
// COALESCE then overwrote the declared class in the row with the inferred one.
// A merge conflict declares ClassConfig, whose prose ("merge conflict in 1
// files") carries no marker at all — so an unsynced run reaches the handler
// blank.
func TestIntegrator_DeclaredClassReachesEscalatorAndDB(t *testing.T) {
	st, run, item := newIntegratorEnv(t)
	handler := &recordingEscalationHandler{}
	itg := NewIntegrator(st, &recordingSubRunner{store: st}, &fakeAllocator{}, &fakeMerger{conflict: true, files: []string{"a.go"}})
	itg.Escalator = handler
	...
	if handler.sawEscalationClass != string(ClassConfig) {
		t.Errorf("Escalator saw escalation_class %q, want %q — the stamp did not reach the run it was handed",
			handler.sawEscalationClass, ClassConfig)
	}
	if want := string(FailureClassFromErrorClass(ClassConfig)); handler.sawFailureClass != want {
		t.Errorf("Escalator saw failure_class %q, want %q", handler.sawFailureClass, want)
	}
	...
	if got.EscalationClass != string(ClassConfig) {
		t.Errorf("persisted escalation_class = %q, want %q", got.EscalationClass, ClassConfig)
	}
}
