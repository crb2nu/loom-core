...
func (g *SubrunGuard) SubrunCreate(ctx context.Context, req SubrunRequest) (string, error) {
	...
	target, err := g.Store.Backlog.Get(ctx, req.BacklogID)
	if err != nil {
		return "", fmt.Errorf("recursion: get target backlog: %w", err)
	}
	...
	newRun := &store.PipelineRun{
		...
		AggregateVersion: target.ClaimVersion,
	...
	// authoritative kickoff path for recursion-spawned runs. The target snapshot
	// used here also stamps the subrun's aggregate version, so a requeued target
	// cannot create a version-0 child that later loses every terminal transition.
	if _, err := g.claimBacklog(ctx, target); err != nil {
...
func (g *SubrunGuard) claimBacklog(ctx context.Context, item *store.BacklogItem) (*store.BacklogItem, error) {
	return g.Store.Backlog.TransitionState(
