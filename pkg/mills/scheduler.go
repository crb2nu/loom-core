...
type Scheduler struct {
	...
	Logger     *slog.Logger
	// Enabled is the live admission barrier. Production wires policy.enabled;
	// nil preserves standalone behavior. The activity counter is published
	// before this check so a tick admitted before a reload remains visible.
	Enabled func() bool

	// KPIRecorder, when set, writes best-effort KPI snapshots after successful
	// steady-state reconciler ticks. Run's immediate boot reconcile skips the
	// snapshot so KPI persistence cannot delay scheduler readiness. Snapshot
	// failures are logged but do not fail the scheduler; KPI persistence is
	// observability, not the control law that starts work.
...
// bootTickOnce reconciles immediately at startup without delaying scheduler
// readiness on the best-effort KPI persistence tail.
func (s *Scheduler) bootTickOnce(ctx context.Context) (TickResult, error) {
	return s.runTick(ctx, false)
}

func (s *Scheduler) runTick(ctx context.Context, recordKPI bool) (TickResult, error) {
	s.active.Add(1)
	defer s.active.Add(-1)
	if s.Enabled != nil && !s.Enabled() {
		return TickResult{SkipReason: "work admission closed"}, nil
	}
	tickCtx, cancel := context.WithTimeout(ctx, schedulerTickTimeout)
	res, err := s.Reconciler.Tick(tickCtx)
	tickErr := tickCtx.Err()
	// Release the reconciler budget before allocating a separate KPI budget.
	// In particular, never pass an expired tick context into KPI persistence.
	cancel()
	if err != nil {
		return res, fmt.Errorf("tick: %w", err)
	}
	if tickErr != nil {
		return res, fmt.Errorf("tick: %w", tickErr)
	}
	...
	if recordKPI && s.KPIRecorder != nil {
		...
		if err := s.KPIRecorder.Record(kpiCtx); err != nil && s.Logger != nil {
...
// ActiveOperations reports reconciler ticks currently executing. The
// destructive safety gate waits for zero after policy.enabled is disabled so
// a tick admitted just before the hot reload cannot race fault injection.
func (s *Scheduler) ActiveOperations() int64 {
	if s == nil {
		return 0
	}
