...
type BudgetReader interface {
	...
	// CountBudgetedSince, which discounts escalations that cannot recur
	// without human action: no-op capacity escalations (class
	// transient_quota, cost $0 — spawn pool saturated, no worker acquired)
	// and terminal config escalations (class config — a first-sight verdict
	// no retry can change). Real code/infra failures still count.
...
// WindowUsage is the rolling-24h consumption snapshot for one tier —
// the HUD fuel gauge's payload. Caps echo the active policy so the
...
// the active policy caps. Informational only — enforcement stays in
// Allow, and callers must not gate work on this snapshot.
func (b *Budget) WindowUsage(ctx context.Context, tier Tier) (WindowUsage, error) {
	if b == nil || b.PolicyFunc == nil || b.Reader == nil {
		return WindowUsage{}, fmt.Errorf("budget: not configured")
	}
	limits, err := tierLimits(b.PolicyFunc(), tier)
	if err != nil {
