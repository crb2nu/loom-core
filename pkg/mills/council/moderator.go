...
type ModeratorDecision struct {
	...
	CostUSD float64
	// CostUnpriced means a paid provider attempt lacks trustworthy USD
	// accounting; Debate propagates it to the run-level reservation guard.
	CostUnpriced bool
}
...
type FakeModerator struct {
	...
	CostUSD float64
	// CostUnpriced is propagated through ModeratorDecision.
	CostUnpriced bool
...
func (f *FakeModerator) Assess(ctx context.Context, prior []SidecarDebateRound, latestCritiques []ReviewerOutput) (ModeratorDecision, error) {
	...
	if f.ReturnErr != nil {
		return ModeratorDecision{CostUSD: cost, CostUnpriced: f.CostUnpriced}, f.ReturnErr
	}
