...
type DebateResult struct {
	...
	// CostUnpriced propagates any paid provider attempt without a trustworthy
	// dollar price so the outer runner retains its conservative reservation.
	CostUnpriced bool
}
...
func (d *Debate) Run(ctx context.Context, in DebateInput) (*DebateResult, error) {
	...
	if round0 != nil {
		...
		res.CostUnpriced = res.CostUnpriced || round0.CostUnpriced
	}
	if err != nil {
		return res, fmt.Errorf("debate round 0 (editor propose): %w", err)
	}
	if round0 == nil {
		return res, errors.New("debate round 0 (editor propose): empty result")
	}
	...
	for round := 1; round <= maxRounds; round++ {
		...
		for _, critique := range critiques {
			res.CostUnpriced = res.CostUnpriced || critique.CostUnpriced
		}
		if len(critiques) > 0 {
			res.Rounds = append(res.Rounds, SidecarDebateRound{
				Round:   round,
		...
		if round < maxRounds {
			...
			res.CostUnpriced = res.CostUnpriced || dec.CostUnpriced
			if mErr != nil {
		...
		if revised != nil {
			...
			res.CostUnpriced = res.CostUnpriced || revised.CostUnpriced
		}
		if rErr != nil {
			return res, fmt.Errorf("debate round %d (editor revise): %w", round, rErr)
		}
		if revised == nil {
			return res, fmt.Errorf("debate round %d (editor revise): empty result", round)
		}
...
// sumCost adds every returned reviewer cost, including cost-bearing partial
// outputs from failed requests.
