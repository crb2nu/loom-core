...
// title may be wrapped in single asterisks (italic), double (bold), or
// nothing at all — the workspace roadmap convention (libs/STANDARDS.md
// "Roadmap & Backlog") writes plain-text bullets. Issue links, "(#NNN, P1)"
// trailers, and emoji status markers are intentionally ignored.
var taskBulletRE = regexp.MustCompile(`^\s*-\s+\[([ xX/-])\]\s+\*{0,2}(.+?)\*{0,2}(?:\s+\(.*)?$`)
...
func parseSectionHeader(line string) (string, int, bool) {
	...
	case strings.HasPrefix(title, "Recently Shipped"),
		...
		// "Backlog" holds links to the GitLab issue board under the
		// workspace convention, never plannable work.
		strings.HasPrefix(title, "Backlog"):
	...
	case strings.HasPrefix(title, "Ongoing Engineering"):
		return title, 9, true
	// Workspace roadmap convention (libs/STANDARDS.md "Roadmap & Backlog"):
	// horizon sections instead of tiers. Matched exactly so headings like
	// "Next Steps" still fall through to the unknown-section bucket.
	case title == "Now":
		return title, 1, true
	case title == "Next":
		return title, 2, true
