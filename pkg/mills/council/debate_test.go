...
func TestDebate_ModeratorErrorSurfaces(t *testing.T) {
	d := debateFixture(99, nil)
	d.Moderator = &FakeModerator{
		ReturnErr: errors.New("moderator: model 5xx"), CostUnpriced: true,
	}
	res, err := d.Run(context.Background(), DebateInput{
		Brief:     newFakeBrief(),
		MaxRounds: 3,
	...
	if res == nil || !res.CostUnpriced {
		t.Fatalf("result = %+v, want moderator's unpriced provider attempt preserved", res)
	}
}

func TestDebate_QuorumFailurePreservesCompletedReviewerCost(t *testing.T) {
	d := debateFixture(99, nil)
	d.Reviewers.Reviewers["tech-debt"] = &FakeReviewer{
		ReturnErr: errors.New("reviewer failed"), CostUnpriced: true,
	}

	res, err := d.Run(context.Background(), DebateInput{
		Brief: newFakeBrief(), MinQuorum: 2, MaxRounds: 3, MaxUSD: 8,
	})
	if err == nil {
		t.Fatal("expected reviewer quorum failure")
	}
	if res == nil {
		t.Fatal("debate result is nil; paid partial work must remain accountable")
	}
	if want := 0.40 + 0.20; !approxEqual(res.TotalCostUSD, want, 0.001) {
		t.Fatalf("total cost = %.2f, want editor + successful reviewer %.2f", res.TotalCostUSD, want)
	}
	if !res.CostUnpriced {
		t.Fatal("debate result did not preserve failed reviewer's unpriced remote spend")
	}
}
