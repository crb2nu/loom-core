...
// PatternRef is the council's view of one approved pattern from the
// pattern catalog (Pattern Loom A1). Declared locally (not imported from
// agentcontext) so the council package stays decoupled from the store
// shape; *clients.PatternClient satisfies PatternLister structurally and
// projects the richer Pattern entity down to these three fields — the
// only ones the editor prompt cites.
type PatternRef struct {
	ID    string // canonical pattern id the proposal must carry as pattern_id
	Makes string // the artifact class this pattern produces (e.g. "go-mcp-server")
	Name  string // human-readable label for the catalog listing
	// Slug is the stable kebab identifier stamped plan ids embed
	// (`plan-stamp-<slug>-<primary>`, svc_pattern_stamp.go). The editor
	// prompt ignores it; the take-up pattern harvester matches on it.
	Slug string
}

// PatternLister fetches the approved-pattern catalog the editor conforms
// proposals to (Pattern Loom A1). It is OPTIONAL: an editor with no lister
// (or a lister that errors) simply omits the catalog section and the
// pattern_id instruction, preserving prior behavior. Best-effort by
// contract — implementations MUST NOT block decomposition on a fetch
// failure (return empty + nil). *clients.PatternClient implements it.
type PatternLister interface {
	ListApprovedPatterns(ctx context.Context) ([]PatternRef, error)
}
...
type EditorOutput struct {
	...
	// Objective is the plan-level end-state + through-line the editor
	// synthesized (2-4 sentences), lifted from the top-level "objective" key of
	// the structured "## Backlog Proposals" block. Empty when the model omitted
	// it. The Spinning-Room spin threads it onto the draft plan (Plan.Objective)
	// so a single spun plan reads as one coherent goal, not just a slice list.
	Objective string
	...
	Model   string
	CostUSD float64
	// CostUnpriced means the provider completed or may have completed paid
	// work, but no trustworthy dollar price was available. The runner retains
	// the conservative admission reservation instead of recording zero spend.
	CostUnpriced bool
