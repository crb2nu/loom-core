...
// fixtureRoadmapWorkspaceConvention mirrors the workspace roadmap template
// (libs/STANDARDS.md "Roadmap & Backlog", adopted repo-wide 2026-07):
// horizon sections Now/Next/Later instead of tiers, plain-text task bullets
// without bold wrappers, and a link-only Backlog section.
const fixtureRoadmapWorkspaceConvention = `# Loom Core Roadmap

> Last Updated: 2026-07-02

## Current Status

Prose summary; bullets here are informational, not intents.

- **Plan store**: some-plan-id (not a task bullet)

## Now

- [ ] Fix recurring escalation classes (DEBT-073) (#167, P1)

## Next

- [ ] OTel trace export from daemon (#12, P2)
- [ ] Split spawn.go monolith, 2206 LOC top-churn file (#168)

## Later

- Plain bullets without checkboxes are not intents

## Backlog

- [ ] Checkbox bullets under Backlog must not become intents
`

func TestExtract_ParsesWorkspaceConventionRoadmap(t *testing.T) {
	r, err := Extract(strings.NewReader(fixtureRoadmapWorkspaceConvention), "sha-wc")
	if err != nil {
		t.Fatalf("extract: %v", err)
	}

	wantPriorities := map[string]int{
		"Fix recurring escalation classes":                 1, // Now
		"OTel trace export from daemon":                    2, // Next
		"Split spawn.go monolith, 2206 LOC top-churn file": 2,
	}
	if len(r.Intents) != len(wantPriorities) {
		t.Errorf("intents: got %d want %d (intents=%+v)",
			len(r.Intents), len(wantPriorities), summaries(r.Intents))
	}
	for _, i := range r.Intents {
		want, ok := wantPriorities[i.Summary]
		if !ok {
			t.Errorf("unexpected intent: %q", i.Summary)
			continue
		}
