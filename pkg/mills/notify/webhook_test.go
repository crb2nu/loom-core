...
// TestWebhookHook_PostEvent_PostsPayload verifies the additive PostEvent path
// reuses the same URL/transport as OnMerged and posts the raw payload verbatim.
func TestWebhookHook_PostEvent_PostsPayload(t *testing.T) {
	var mu sync.Mutex
	...
	h := New(Config{URL: srv.URL}, nil, srv.Client(), nil)
	if err := h.PostEvent(context.Background(), map[string]any{"text": "hi", "rule": "budget_burn"}); err != nil {
		t.Fatalf("PostEvent returned %v, want nil", err)
	}
	mu.Lock()
	defer mu.Unlock()
	if received["text"] != "hi" || received["rule"] != "budget_burn" {
		t.Fatalf("PostEvent posted %+v", received)
	}
}

// TestWebhookHook_PostEvent_DisabledReturnsErrDisabled verifies a disabled hook
// (empty URL) never posts and surfaces ErrDisabled to the caller.
func TestWebhookHook_PostEvent_DisabledReturnsErrDisabled(t *testing.T) {
	calls := int64(0)
	srv := httptest.NewServer(http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
	...
	defer srv.Close()
	h := New(Config{URL: ""}, nil, srv.Client(), nil)
	if err := h.PostEvent(context.Background(), map[string]any{"text": "x"}); !errors.Is(err, ErrDisabled) {
		t.Fatalf("PostEvent err = %v, want ErrDisabled", err)
	}
	if atomic.LoadInt64(&calls) != 0 {
		t.Fatalf("disabled hook posted %d times", atomic.LoadInt64(&calls))
	}
}
