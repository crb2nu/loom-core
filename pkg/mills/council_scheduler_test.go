...
func TestCouncilScheduler_PublishesActivityBeforeDetachedRun(t *testing.T) {
	dir := t.TempDir()
	p := Default()
	on := true
	p.Enabled = &on
	...
	s := &CouncilScheduler{
		Policy: pm,
		Now:    func() time.Time { return time.Date(2026, 5, 19, 5, 0, 0, 0, time.UTC) },
		RunFn: func(context.Context, store.CouncilTrigger, string) error {
			close(started)
			<-release
			return nil
		},
	...
	case <-time.After(time.Second):
		t.Fatal("detached council run did not start")
	}
	close(release)
