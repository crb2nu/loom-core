...
// countRefusedWriteDomains counts Write=true domains whose description
// matches at least one query token — i.e. domains the selector would
// have picked for this query but skipped because auto-compose never
// dispatches write domains. Feeds the "refused" outcome metric.
func countRefusedWriteDomains(domains []SubAgent, query string) int {
	tokens := tokenizeQuery(query)
	if len(tokens) == 0 {
		return 0
	}
