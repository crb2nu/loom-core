...
func (t *SubagentTelemetry) RecordTurnEnd(_ context.Context, resp openairesponses.TurnResponse, err error, _ openairesponses.ExecutionIdentity) {
	...
	// cached_prompt is a SUBSET of prompt, not a third peer direction: divide
	// it by prompt for the warm share. Summing the directions would
	// double-count the cached part. Reported only when the engine populates
	// prompt_tokens_details / input_tokens_details — a lane that omits them
	// leaves this series absent rather than pinned at zero, so an absent
	// series means "unmeasured", not "cold".
	if resp.CachedTokens > 0 {
		t.metrics.TokensTotal.WithLabelValues(t.domain, "cached_prompt").Add(float64(resp.CachedTokens))
	}
}
