...
// defaultSynthesizeMaxTokens bounds the synthesized answer when the
// query didn't request an explicit max_tokens.
const defaultSynthesizeMaxTokens = 1024
...
func (r *Router) dispatch(ctx context.Context, domains []string, req QueryRequest, queryID string, logger *slog.Logger) (QueryResult, error) {
	...
	answer := r.synthesize(ctx, results, req.Query, req.MaxTokens, logger)
...
func (r *Router) runSubAgent(ctx context.Context, agent SubAgent, req QueryRequest, queryID string, logger *slog.Logger) DomainResult {
	...
	// Honor the domain's YAML max_tokens as the per-call completion
	// bound (previously parsed but never applied).
	responsesClient.MaxTokens = agent.MaxTokens
...
func (r *Router) synthesize(ctx context.Context, results []DomainResult, query string, maxTokens int, logger *slog.Logger) string {
	if r.tracer != nil {
		var span trace.Span
	...
	// The QueryRequest's max_tokens is documented as bounding the
	// synthesized response; fall back to the historical 1024 default.
	if maxTokens <= 0 {
		maxTokens = defaultSynthesizeMaxTokens
	}
	synth, err := r.client.CompleteSimple(synthCtx, r.cfg.RouterModel, prompt, r.applyModelPrefix(r.cfg.RouterModel, userMsg), maxTokens)
	if err != nil {
...
func (r *Router) TryAutoCompose(ctx context.Context, query string) (QueryResult, bool, error) {
	...
	// Refused bookkeeping: count write-domains that matched this query's
	// tokens and were skipped only because of Write=true. Counting every
	// write-domain in the registry (the previous behavior) inflated the
	// metric on queries that never touched a write domain.
	if refused := countRefusedWriteDomains(r.registry.List(), query); refused > 0 {
		if m := r.autoComposeMetrics(); m != nil {
