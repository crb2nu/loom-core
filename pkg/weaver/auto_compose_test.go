...
func TestCountRefusedWriteDomains(t *testing.T) {
	domains := []SubAgent{
		{Name: "read-match", Description: "cluster health and pods", Write: false},
		{Name: "write-match", Description: "cluster deployment mutations", Write: true},
		{Name: "write-nomatch", Description: "billing invoices", Write: true},
		{Name: "write-empty-desc", Description: "", Write: true},
	}

	// Only write-domains whose description matches a query token count.
	if got := countRefusedWriteDomains(domains, "cluster status"); got != 1 {
		t.Errorf("cluster query: got %d refused, want 1", got)
	}
	// A query touching no write domain refuses nothing (the old code
	// counted every write domain in the registry here).
	if got := countRefusedWriteDomains(domains, "pods overview"); got != 0 {
		t.Errorf("read-only query: got %d refused, want 0", got)
	}
	if got := countRefusedWriteDomains(domains, ""); got != 0 {
		t.Errorf("empty query: got %d refused, want 0", got)
	}
}
