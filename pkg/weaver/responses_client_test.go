...
// TestFlexInferResponsesClient_MaxTokens asserts the configured MaxTokens
// reaches the wire (previously the field didn't exist and 4096 was
// hardcoded, silently ignoring domain/request max_tokens settings).
func TestFlexInferResponsesClient_MaxTokens(t *testing.T) {
	...
	req := openairesponses.TurnRequest{Model: "test-model", Input: "q"}

	// Explicit bound is honored.
	rc := NewFlexInferResponsesClient(client, nil, 60*time.Second, slog.Default())
	rc.MaxTokens = 512
	if _, err := rc.Create(context.Background(), req); err != nil {
		t.Fatalf("Create: %v", err)
	}
	if gotMaxTokens != 512 {
		t.Errorf("max_tokens on wire: got %d want 512", gotMaxTokens)
	}

	// Zero falls back to the default.
	rc2 := NewFlexInferResponsesClient(client, nil, 60*time.Second, slog.Default())
	if _, err := rc2.Create(context.Background(), req); err != nil {
		t.Fatalf("Create: %v", err)
	}
	if gotMaxTokens != defaultCompletionMaxTokens {
		t.Errorf("default max_tokens on wire: got %d want %d", gotMaxTokens, defaultCompletionMaxTokens)
	}
}
