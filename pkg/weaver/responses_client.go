...
type FlexInferResponsesClient struct {
	...
	// MaxTokens bounds the completion size of each chat call. Zero or
	// negative falls back to defaultCompletionMaxTokens. Set from the
	// SubAgent's max_tokens (YAML) by Router.runSubAgent so the field
	// is honored instead of silently ignored.
	MaxTokens int
}

// defaultCompletionMaxTokens is the per-call completion bound used when
// no explicit max_tokens is configured on the client.
const defaultCompletionMaxTokens = 4096
...
func (c *FlexInferResponsesClient) doRequest(ctx context.Context, body []byte) (*chatCompletionResponseWithTools, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		err := c.client.Breaker().Execute(func() error {
			...
			// The stable cross-client usage record. Separate from the debug
			// trace above because that one's fields are this client's business
			// and this one's are queried by name from docs/JOURNAL_ENGINE.md.
			// The log line matters more here than elsewhere: both production
			// wirings construct weaver.NewMetrics(nil), so weaver's Prometheus
			// vecs are unregistered and the cached_prompt series above is not
...
func (c *FlexInferResponsesClient) parseTurnResponse(resp *chatCompletionResponseWithTools) (openairesponses.TurnResponse, error) {
	...
	return openairesponses.TurnResponse{
		...
		// CachedTokens was previously left zero here even though
		// TurnResponse has carried the field all along — the chat-completions
		// shape this client parses simply never populated it. Filling it in is
		// what makes weaver's cache behaviour visible to SubagentTelemetry.
