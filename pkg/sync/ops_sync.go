...
func (m *Manager) SyncToHome(profileName string, backup bool, regen bool, repoOnly bool, hubMode bool, hubURL string, loomMode bool, loomBinary string, resolveSecrets bool) error {
	...
	if p.SyncGeneratedOnly {
		...
		if p.Name == "zed" {
			// Zed's settings.json is user-owned (JSONC): merge the generated
			// context_servers fragment instead of copying over it.
			if err := syncZedGenerated(srcFile, dstFile); err != nil {
				return err
			}
			cleanupZedLegacyGenerated(repoPath, m.HomeDir)
...
func (m *Manager) PullFromHome(profileName string, backup bool) error {
	...
	if p.SyncGeneratedOnly {
		...
		if p.Name == "zed" {
			// Extract only the context_servers block; the home file is the
			// user's full settings.json.
			return pullZedGenerated(srcFile, dstFile)
		}
