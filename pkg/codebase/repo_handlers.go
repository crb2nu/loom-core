...
func (s *Service) HandleStats(ctx context.Context, args map[string]any) (*mcp.CallToolResult, error) {
	...
	// Repo-agnostic (fleet) mode: with no repo_id and no CODEBASE_REPO_ID
	// default, summarize the whole collection across every indexed repo
	// instead of erroring. This is what fleet-level pollers (e.g. the HUD
	// codebase panel) want, and it means an unconfigured server answers the
	// call successfully rather than failing on every poll forever.
	if strings.TrimSpace(repoID) == "" {
		return s.aggregateStats(ctx, langs, ctypes)
	}
...
// aggregateStats summarizes the entire collection across all indexed repos.
// Counts use repo-agnostic filters (no repo_id condition), so the call
// succeeds even when no repo is configured — the case a fleet-level HUD panel
// polls in. Zero-valued language/chunk-type buckets are omitted to keep the
// fleet summary compact.
func (s *Service) aggregateStats(ctx context.Context, langs, ctypes []string) (*mcp.CallToolResult, error) {
	total, err := s.qdrant.Count(ctx, qdrant.FilterAcrossRepos(nil, nil))
	if err != nil {
		return nil, err
	}
	...
	byLang := map[string]int{}
	for _, l := range langs {
		n, err := s.qdrant.Count(ctx, qdrant.FilterAcrossRepos([]string{l}, nil))
		if err != nil {
			return nil, err
		}
		if n > 0 {
	...
	byType := map[string]int{}
	for _, t := range ctypes {
		n, err := s.qdrant.Count(ctx, qdrant.FilterAcrossRepos(nil, []string{t}))
		if err != nil {
			return nil, err
		}
		if n > 0 {
