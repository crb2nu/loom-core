...
// checkVectorDim rejects an embedding whose dimension differs from the target
// collection's. This must happen client-side: bulk upserts run with
// wait=false, so Qdrant ACKs a mismatched vector into the WAL and only
// declines it at apply time — the point is silently lost and the stuck WAL
// entry re-logs a dimension error on every server restart. Mixed-dimension
// batches are real: an upstream provider can change a model's output size in
// place (observed 2026-07-09: morph-embedding-v3 went 1024 -> 1536) while
// vectors cached under the same embed_model payload keep the old dimension.
func checkVectorDim(chunkID string, vector []float64, collectionDim int) error {
	if len(vector) == 0 {
		return fmt.Errorf("missing vector for chunk %s", chunkID)
	}
	if collectionDim > 0 && len(vector) != collectionDim {
		return fmt.Errorf(
			"chunk %s: embedding dim %d does not match collection dim %d (upstream embedding model changed output size?); refusing upsert that Qdrant would silently decline",
			chunkID, len(vector), collectionDim,
		)
	}
	return nil
}
...
func (s *Service) runIndexJob(
	...
	flush := func() error {
		...
		for _, p := range pending {
			if err := checkVectorDim(p.chunk.ID, p.vector, vectorSize); err != nil {
				return err
			}
			payload := qdrant.ChunkToPayload(p.chunk, true, embedModel)
			payload[qdrant.EmbeddingFallbackPayloadKey] = !embeddings
