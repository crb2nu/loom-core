...
func TestResilientEmbedder_BreakerOpensAfterThreshold(t *testing.T) {
	...
	events := sink.events()
	if len(events) != 2 {
		t.Fatalf("expected threshold-open and short-circuit telemetry, got %d events: %+v", len(events), events)
	}
	if events[0].Outcome != telemetry.EmbeddingOutcomeThresholdOpen ||
		events[0].Reason != telemetry.EmbeddingReasonFailureThresholdExceeded ||
		events[0].Path != telemetry.EmbeddingPathQuery ||
		events[0].FailureThreshold != 3 ||
		events[0].ConsecutiveFails != 3 ||
		events[0].Cooldown != time.Minute {
		t.Fatalf("unexpected threshold event: %+v", events[0])
	}
	if events[1].Outcome != telemetry.EmbeddingOutcomeShortCircuit ||
		events[1].Reason != telemetry.EmbeddingReasonCircuitOpen ||
		events[1].Path != telemetry.EmbeddingPathQuery ||
		events[1].FailureThreshold != 3 ||
		events[1].ConsecutiveFails != 3 {
		t.Fatalf("unexpected short-circuit event: %+v", events[1])
	}
}

func TestResilientEmbedder_DocumentsThresholdTelemetryIncludesBatchAndTimeout(t *testing.T) {
	inner := &fakeEmbedder{err: errors.New("upstream 522")}
	sink, restore := captureEmbeddingFallbackTelemetry(t)
	defer restore()
	r := NewResilientEmbedder(inner, ResilientConfig{
	...
	if _, err := r.EmbedDocuments(context.Background(), []string{"a", "b", "c", "d"}); err == nil {
		t.Fatal("expected upstream error")
	}

	events := sink.events()
	if len(events) != 1 {
		t.Fatalf("expected 1 threshold event, got %d: %+v", len(events), events)
	}
	got := events[0]
	if got.Path != telemetry.EmbeddingPathDocuments ||
		got.BatchSize != 4 ||
		got.Outcome != telemetry.EmbeddingOutcomeThresholdOpen ||
		got.Reason != telemetry.EmbeddingReasonFailureThresholdExceeded ||
		got.FailureThreshold != 1 ||
		got.ConsecutiveFails != 1 ||
		got.Cooldown != 45*time.Second ||
		got.Timeout != 250*time.Millisecond {
		t.Fatalf("unexpected documents threshold event: %+v", got)
	}
}
