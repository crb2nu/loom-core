...
// ScrollPointsPage returns one bounded page including vectors and payloads.
func (c *Client) ScrollPointsPage(ctx context.Context, limit int, offset any) ([]Point, any, error) {
	...
	points := make([]Point, 0, len(resp.Result.Points))
	for _, p := range resp.Result.Points {
		points = append(points, Point{ID: fmt.Sprint(p.ID), Vector: p.Vector, Payload: p.Payload})
	}
	return points, resp.Result.Next, nil
}
