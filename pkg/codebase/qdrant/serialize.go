...
// EmbeddingFallbackPayloadKey marks points written with a deterministic dummy
// vector so maintenance sweeps can repair them later.
...
// FilterAcrossRepos builds a Qdrant filter that matches points in ANY
// repository, optionally narrowed by language and/or chunk type. Unlike
// Filter it deliberately omits the repo_id condition, so callers can compute
// fleet-wide aggregates over every indexed repo in the collection. When both
// languages and chunkTypes are empty it returns nil, which Qdrant's count
// endpoint treats as "match every point" — no payload index required.
func FilterAcrossRepos(languages, chunkTypes []string) map[string]any {
	conds := make([]any, 0, 2)
	if len(languages) > 0 {
		conds = append(conds, filterShould(matches("language", languages)...))
	}
