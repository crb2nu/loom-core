...
func GenerateConfigsWithPath(reg *registry.Registry, registryPath string, outputDir string, targets []string, hubMode bool, hubURL string, loomMode bool, loomBinary string, resolveSecrets bool) error {
	...
	for _, target := range targets {
		...
		// Generate standalone Codex profile overlay files
		// (~/.codex/<name>.config.toml) — no-op for non-codex targets and for a
		// codex registry with no profiles declared.
		if err := generateCodexProfileConfigs(reg, outputDir, target, profile); err != nil {
			return fmt.Errorf("generate codex profiles for %s: %w", target, err)
		}

		// Generate the loom-managed execpolicy block (rules/default.rules) —
		// no-op for non-codex targets and for a registry with no codex rules.
		// pkg/sync merges it into ~/.codex/rules/default.rules marker-wise so
		// Codex-appended approval rules survive regen.
		if err := generateCodexRulesConfig(reg, outputDir, target, profile); err != nil {
			return fmt.Errorf("generate codex rules for %s: %w", target, err)
		}
	}
