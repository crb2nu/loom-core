...
//
// If this test breaks, Codex changed the canonical hash algorithm. To
// refresh the fixtures, re-run a live probe: write a minimal hooks.json,
// trust it via `codex` `/hooks`, then copy the trusted_hash Codex records
// under [hooks.state] in ~/.codex/config.toml. (The old
// scripts/ci/codex_hooks_smoke.sh helper was removed.)
