...
func generateOpenCodeJSONConfig(p *GenerateParams, targets map[string]*registry.TargetSpec) error {
	...
	// Platforms with fine-grained permissions (Kilo 1.0) express tool
	// auto-approval via a top-level `permission` map keyed by the namespaced
	// tool name {server}_{tool}, with glob support ({server}_*). An empty
	// AlwaysAllow keeps parity with the legacy always_allow=["*"] default.
	if p.Profile.Capabilities.Permissions {
...
func generateTomlConfig(p *GenerateParams) error {
	...
	for _, name := range names {
		...
		// Gemini and other TOML platforms use `always_allow = ["tool1", "tool2"]`.
...
func generateHooksConfig(reg *registry.Registry, outputDir, target string, profile *PlatformProfile, loomBinary string) error {
	...
	// Template-driven path: profiles whose hook shape is the shared
	// {"hooks": ...} document select templates/hooks.tmpl. Platforms with
	// additional settings or a vendor-specific shape keep their specialized
	// builders below.
