...
// installed binaries: https://code.claude.com/docs/en/hooks documented 31
// events on 2026-08-07, while the 2.1.61 binary's enum accepts only an
// 18-event subset — writing any of the 13 docs-only names into that CLI's
// settings.json disables all hooks. Validation therefore probes the installed
// binary for its own enum and only falls back to this pinned baseline when no
// binary is available (e.g. CI).
//
// claudeHookEventBaseline is the union of the event-name enum embedded in the
// Claude Code 2.1.61 binary and the events documented at
// https://code.claude.com/docs/en/hooks as of 2026-08-07. The fallback leans
// wide deliberately: a too-narrow baseline silently drops hooks configured on
// newer events wherever no binary is probeable, while machines running older
// CLIs are already protected by the probe path (the installed binary's own
// enum governs there). The binary portion was extracted with:
//
//	strings -a ~/.local/share/claude/versions/2.1.61 | \
//	  grep -oE '"PreToolUse"[,"A-Za-z]{20,400}'
var claudeHookEventBaseline = []string{
	// Enum embedded in the 2.1.61 binary (verified accepted).
	"PreToolUse",
	"PostToolUse",
	"PostToolUseFailure",
	"Notification",
	...
	"WorktreeCreate",
	"WorktreeRemove",
	// Documented at code.claude.com/docs/en/hooks (2026-08-07) but absent
	// from the 2.1.61 binary enum; newer CLIs accept them.
	"UserPromptExpansion",
	"PermissionDenied",
	"PostToolBatch",
	"MessageDisplay",
...
func claudeAcceptedHookEvents() (map[string]bool, string) {
	claudeHookEventsOnce.Do(func() {
		...
		claudeHookEventsSource = "pinned baseline: Claude Code 2.1.61 binary enum + docs events as of 2026-08-07 (claude binary not probeable)"
