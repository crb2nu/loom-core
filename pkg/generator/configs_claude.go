...
func claudeHooks(reg *registry.Registry, profile *PlatformProfile, loomBinary string) map[string]any {
	...
	// Append the delta-gated MR-status context-injection hook (mrwatch M3a):
	// UserPromptSubmit → `loom agent mr-status --hook claude`. Appended before
	// event validation so an unsupported UserPromptSubmit (future CLI) is
	// stripped rather than emitted blind (see mrwatch_hooks.go).
	appendMRStatusHook(hooks, reg, "claude", profile.Hooks, loomBinary)
