...
func geminiHooks(reg *registry.Registry, profile *PlatformProfile, loomBinary string) map[string]any {
	...
	// BeforeAgent → `loom agent mr-status --hook gemini`. BeforeAgent (NOT
	// BeforeModel) is Gemini's additionalContext-capable per-turn event.
	// Appended before event validation (see mrwatch_hooks.go).
	appendMRStatusHook(hooks, reg, "gemini", profile.Hooks, loomBinary)
