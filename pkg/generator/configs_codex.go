...
// codexProfile is a resolved named profile from the registry's
// platform_permissions.codex.settings.profiles map. Codex 0.134.0+ reads each
// profile from a SEPARATE file ~/.codex/<Name>.config.toml (top-level keys that
// overlay ~/.codex/config.toml), selected with `codex --profile <Name>`. The
// old [profiles.<name>] table in config.toml is no longer read (see the
// "Profiles" section of https://learn.chatgpt.com/docs/config-file/config-advanced),
// so we emit standalone overlay files instead. Only keys that differ from the
// base config belong here; unset keys inherit the base.
...
// buildCodexProfiles resolves the named-profile overlays from the registry's
// codex platform settings (platform_permissions.codex.settings.profiles).
// Returns nil (skip all profile files) when none are declared. Profiles are
// sorted by name for deterministic output. A profile with no recognized
// override keys is dropped — an empty overlay file would be meaningless.
func buildCodexProfiles(pp *registry.PlatformPermission) []codexProfile {
	if pp == nil || pp.Settings == nil {
		return nil
	}
	raw, ok := pp.Settings["profiles"].(map[string]any)
	if !ok || len(raw) == 0 {
		return nil
	}
	...
	for _, name := range names {
		...
		prof := codexProfile{Name: name}
		if v, ok := body["model"].(string); ok && v != "" {
			prof.Model = v
		}
		if v, ok := body["model_reasoning_effort"].(string); ok && v != "" {
			prof.ModelReasoningEffort = v
		}
		// Skip an overlay that would override nothing.
		if prof.Model == "" && prof.ModelReasoningEffort == "" {
			continue
		}
...
// renderCodexProfileTOML renders a standalone codex profile overlay file
// (~/.codex/<name>.config.toml). Only the keys that differ from the base
// config are emitted; codex overlays them onto ~/.codex/config.toml when the
// profile is selected via `codex --profile <name>`.
func renderCodexProfileTOML(prof codexProfile) string {
	var sb strings.Builder
	fmt.Fprintf(&sb, "# Generated Codex profile %q for `loom sync codex --regen`\n", prof.Name)
	sb.WriteString("# Source: mcp/context/registry.yaml (platform_permissions.codex.settings.profiles)\n")
	sb.WriteString("# Codex 0.134.0+ reads a profile from ~/.codex/<name>.config.toml (top-level\n")
	sb.WriteString("# keys overlaying ~/.codex/config.toml); select with `codex --profile " + prof.Name + "`\n")
	sb.WriteString("# or `codex exec --profile " + prof.Name + "`. The legacy [profiles.<name>] table in\n")
	sb.WriteString("# config.toml is no longer read. Ref: https://learn.chatgpt.com/docs/config-file/config-advanced\n")
	if prof.Model != "" {
		fmt.Fprintf(&sb, "model = %q\n", prof.Model)
	}
	if prof.ModelReasoningEffort != "" {
		fmt.Fprintf(&sb, "model_reasoning_effort = %q\n", prof.ModelReasoningEffort)
	}
	return sb.String()
}

// generateCodexProfileConfigs writes one ~/.codex/<name>.config.toml overlay
// file per registry-declared codex profile. It is a no-op for non-codex
// targets and for a codex registry with no profiles. Called from
// GenerateConfigsWithPath alongside generateHooksConfig. The sync manager's
// codex ExtraGeneratedFiles list must include each emitted "<name>.config.toml"
// for it to land in ~/.codex/ (see pkg/sync/manager.go).
func generateCodexProfileConfigs(reg *registry.Registry, outputDir, target string, profile *PlatformProfile) error {
	// Only codex (the requires_preamble TOML platform) uses this profile shape.
	if profile == nil || !profile.Features.RequiresPreamble {
		return nil
	}
	profiles := buildCodexProfiles(registryPlatformPerms(reg, target))
	if len(profiles) == 0 {
		return nil
	}
	...
	for _, prof := range profiles {
		content := renderCodexProfileTOML(prof)
		path := filepath.Join(destDir, prof.Name+".config.toml")
		if err := os.WriteFile(path, []byte(content), 0644); err != nil {
			return fmt.Errorf("write codex profile %q: %w", prof.Name, err)
		}
	}
	return nil
}
