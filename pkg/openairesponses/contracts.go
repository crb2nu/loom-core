...
type TurnRequest struct {
	...
	// PromptCacheKey is passed through as the Responses API `prompt_cache_key`.
	// OpenAI prompt caching is automatic for prompts >=1024 tokens (static
	// content must lead the prompt); this key is combined with the prefix hash
	// to improve routing so requests sharing a long common prefix land on the
	// same cache. Optional — empty ⇒ the field is omitted.
...
type TurnResponse struct {
	...
	// CachedTokens is the portion of PromptTokens served from OpenAI's prompt
	// cache (usage.input_tokens_details.cached_tokens). A persistent 0 across
	// requests sharing a long prefix means the prefix is below the 1024-token
	// cache threshold or a silent invalidator moved it.
	CachedTokens int
}
