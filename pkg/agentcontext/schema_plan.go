...
// Plan priority buckets ("warp beam" order). Values align with Mills
// store.Priority so the plan-slice emitter propagates a plan's priority
// verbatim onto emitted backlog items — the Mills dispatcher picks queued
// items priority ASC, so P0 dispatches first. Empty = unset (emitter default).
const (
	PlanPriorityP0 = "P0"
	PlanPriorityP1 = "P1"
	PlanPriorityP2 = "P2"
...
// normalizePlanPriority canonicalizes a priority arg ("p1 " -> "P1").
func normalizePlanPriority(p string) string {
	return strings.ToUpper(strings.TrimSpace(p))
}

// planPriorityValid reports whether p is a known priority bucket. The empty
// string is handled by callers (unset is legal everywhere).
func planPriorityValid(p string) bool {
	switch p {
	case PlanPriorityP0, PlanPriorityP1, PlanPriorityP2, PlanPriorityP3:
		return true
	}
	return false
}

// planPriorityRank orders buckets for beam sorting: P0 first, unset last.
func planPriorityRank(p string) int {
	switch p {
	case PlanPriorityP0:
		return 0
	case PlanPriorityP1:
...
type Plan struct {
	...
	// Objective is the plan's synthesized end-state + the through-line that
	// connects its slices — the "why" a reviewer reads before the slice list.
	// It is NOT an echo of the raw brief (which lives in SpecDoc); the producer
	// (the Spinning-Room frame) synthesizes it. Optional: older plans and plans
	// authored before this field predate it and render cleanly without one.
	...
	// RespunFrom is the source plan_id this plan was respun from (an operator
	// redoing an older/sparse plan in the Spinning Room). Provenance only —
	// immutable after create, like CreatedBy; the HUD links the fresh draft back
	// to the plan it supersedes.
...
func payloadToPlan(payload map[string]any) *Plan {
	...
	p := &Plan{
		...
		Objective:          toString(payload["objective"]),
		...
		RespunFrom:         toString(payload["respun_from"]),
