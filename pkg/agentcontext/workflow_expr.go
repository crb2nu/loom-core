...
func resolveVariables(args map[string]any, input, context map[string]any) map[string]any {
	...
	for k, v := range args {
		result[k] = resolveAny(v, input, context)
	}
	return result
}

// resolveAny resolves variable references in a value of any JSON shape,
// recursing through nested maps and arrays (e.g. an entries array whose
// elements carry "${...}" templates).
func resolveAny(v any, input, context map[string]any) any {
	switch val := v.(type) {
	case string:
		// Check for variable reference like "${input.key}" or "${step_id.result}"
		return resolveString(val, input, context)
	case map[string]any:
		return resolveVariables(val, input, context)
	case []any:
		out := make([]any, len(val))
		for i, item := range val {
			out[i] = resolveAny(item, input, context)
		}
		return out
	default:
		return v
