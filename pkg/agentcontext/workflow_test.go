...
func TestWorkflowEngine_AutoVerifyPasses(t *testing.T) {
	...
	if verifyStep.RetryCount != 0 {
		t.Fatalf("retry count = %d, want 0", verifyStep.RetryCount)
	}
}
...
func TestWorkflowEngine_AutoVerifyRetriesAndFails(t *testing.T) {
	...
	verifyStep := wf.StepStates["verify"]
	if verifyStep == nil || verifyStep.Result == nil {
		t.Fatal("expected retry-exhausted verify step result to be preserved")
	}
	if verifyStep.RetryCount != 2 {
		t.Fatalf("retry count = %d, want 2", verifyStep.RetryCount)
	}
	if verifyStep.MaxRetries != 2 {
		t.Fatalf("max retries = %d, want 2", verifyStep.MaxRetries)
	}
	if checks, ok := verifyStep.Result["checks"].([]any); !ok || len(checks) != 1 {
		t.Fatalf("verify step result checks = %#v, want single preserved failure check", verifyStep.Result["checks"])
	}
}

func TestWorkflowEngine_AutoVerifySkippedWhenUnavailable(t *testing.T) {
	engine := NewWorkflowEngine(nil)
	def := &WorkflowDefinition{
		Name: "auto-verify-skipped",
		Steps: []WorkflowStep{
			{
				ID:       "verify",
				Name:     "Verify",
	...
	if wf.Status != WorkflowStatusCompleted {
		t.Fatalf("expected completed with skipped quality gate, got %s: %s", wf.Status, wf.Error)
	}

	verifyStep := wf.StepStates["verify"]
	if verifyStep == nil {
		t.Fatal("expected verify step state")
	}
	if verifyStep.Status != StepStatusSkipped {
		t.Fatalf("verify status = %s, want skipped", verifyStep.Status)
	}
	if verifyStep.Result == nil || verifyStep.Result["skipped"] != true {
		t.Fatalf("verify result = %#v, want skipped marker", verifyStep.Result)
	}
	if wf.Context["verify"] == nil {
		t.Fatal("expected skipped auto verify result to be stored in workflow context")
	}
}

func TestWorkflowStatusIncludesRetryStateAndResult(t *testing.T) {
	...
	executor := func(ctx context.Context, server, tool string, args map[string]any) (map[string]any, error) {
		return map[string]any{
			"passed": false,
			"checks": []any{
				map[string]any{"name": "lint", "passed": false, "output_tail": "lint failed"},
			},
		}, nil
	}

	engine := NewWorkflowEngine(executor)
	svc := &Service{workflowEngine: engine}
	svc.workflow = &WorkflowSvc{Service: svc}

	def := &WorkflowDefinition{
		Name: "auto-verify-status",
		Steps: []WorkflowStep{
			{ID: "verify", Name: "Verify", StepType: StepTypeAutoVerify, ToolArgs: map[string]any{"project": "loom-core"}, MaxRetries: 1, RetryDelay: 1},
		},
	}
	if err := engine.RegisterDefinition(def); err != nil {
		t.Fatalf("RegisterDefinition failed: %v", err)
	...
	result, err := svc.HandleWorkflowStatus(context.Background(), map[string]any{"workflow_id": wf.ID})
	if err != nil {
		t.Fatalf("HandleWorkflowStatus returned error: %v", err)
	}
	if result.IsError {
	...
	if !ok || len(steps) != 1 {
		t.Fatalf("steps = %#v, want 1 step", payload["steps"])
	}
	step, ok := steps[0].(map[string]any)
	if !ok {
		t.Fatalf("step summary type = %T, want map[string]any", steps[0])
	}
	if step["retry_count"] != float64(1) {
		t.Fatalf("retry_count = %v, want 1", step["retry_count"])
	}
	if step["max_retries"] != float64(1) {
		t.Fatalf("max_retries = %v, want 1", step["max_retries"])
	}
	if _, ok := step["result"].(map[string]any); !ok {
		t.Fatalf("expected structured result in status payload, got %#v", step["result"])
	}
}
