...
func TestNewQdrantRegistry(t *testing.T) {
	...
	cfg := Config{
		...
		PatternsCollection:       "patterns_v1",
	}

	reg := NewQdrantRegistry(hc, cfg)

	// 16 unique map entries: annotations merged into context (same key),
	// templates removed, plans + slices + patterns added.
	names := reg.Names()
	if len(names) != 16 {
...
func TestQdrantRegistry_NamesStable(t *testing.T) {
	...
	// CollAnnotations == CollContext after SIMP-12, so 16 unique keys (plans + slices + patterns + messages added).
	expected := []string{
		CollContext, CollFileClaims, CollGraphEntities,
		CollGraphRelations, CollHandoffs, CollMemory, CollMessages, CollPatterns, CollPlans, CollPresence,
