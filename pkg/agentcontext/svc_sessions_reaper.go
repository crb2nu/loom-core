...
func (ss *SessionSvc) EndStale(ctx context.Context, maxAgeHours int) int {
	...
	for _, p := range points {
		...
		ss.mu.Unlock()
		// Notify SSE subscribers so the HUD live-sessions store can mark this
		// session ended and reap it instead of leaving it in the "live"
		// list forever as a zombie with no captured activity.
