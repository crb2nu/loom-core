...
func (e *WorkflowEngine) executeStep(ctx context.Context, wf *Workflow, stepID string) {
	...
	if step.StepType == StepTypeAutoVerify && isSkippedQualityGateResult(result) && err == nil {
		step.Status = StepStatusSkipped
		wf.CompletedSteps++
		if result != nil {
		...
		e.emitEvent(WorkflowEvent{
			...
			Details:    map[string]any{"reason": skippedQualityGateReason(result), "result": result},
		})
		return
	}
...
func (e *WorkflowEngine) executeAutoVerifyStep(ctx context.Context, wf *Workflow, step *WorkflowStep) (map[string]any, error) {
	...
	if e.toolExecutor == nil {
		return map[string]any{
			"skipped":     true,
			"unavailable": true,
			"reason":      "quality gate unavailable: no tool executor configured",
		}, nil
	}
	...
	if isSkippedQualityGateResult(result) {
		return result, nil
	}
...
func isSkippedQualityGateResult(result map[string]any) bool {
	if result == nil {
		return false
	}
	...
	if unavailable, ok := result["unavailable"].(bool); ok && unavailable {
		return true
	}
	return false
}

func skippedQualityGateReason(result map[string]any) string {
	if result == nil {
