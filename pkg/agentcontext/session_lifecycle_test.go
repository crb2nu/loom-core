...
func TestSessionEnd_PostSessionRetroQueuesWork(t *testing.T) {
	t.Setenv("LOOM_MCP_OUTPUT_FORMAT", "json")
	...
	queued := make(chan string, 1)
	svc.sess.runRetroAsync = func(session *Session) {
		queued <- session.ID
	}
	...
	payload := decodeToolPayload(t, result.Content[0].Text)
	if payload["retro_queued"] != true {
		t.Fatalf("retro_queued = %v, want true", payload["retro_queued"])
	}
	...
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for async retro callback")
	}
