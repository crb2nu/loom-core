...
type Config struct {
	...
	// Direct messages (agent_message_send/inbox)
	MessageExpirationHours int
	// HUDBaseURL is the local HUD endpoint for best-effort live message
	// nudges. Empty disables the nudge attempt (store-only delivery).
	HUDBaseURL string

	// Vendor session transcript roots (agent_vendor_session_list/search).
	// Empty values fall back to ~/.claude/projects and ~/.codex/sessions.
	...
	// Plan truth sweep (plan_reconciler.go). The sweep is inert without
	// HUDBaseURL — that is where the merged signal comes from.
	PlanReconcilerEnabled bool
	PlanReconcileInterval time.Duration // default 15m
...
func LoadConfigFromEnv() (Config, error) {
	cfg := Config{
		...
		MessageExpirationHours: env.IntWithZero("AGENT_CONTEXT_MESSAGE_EXPIRATION_HOURS", 168),
		HUDBaseURL:             env.StringChain([]string{"AGENT_CONTEXT_HUD_URL", "LOOM_HUD_URL"}, "http://127.0.0.1:3333"),
		ClaudeSessionsDir:      env.StringChain([]string{"AGENT_CONTEXT_CLAUDE_SESSIONS_DIR"}, ""),
		CodexSessionsDir:       env.StringChain([]string{"AGENT_CONTEXT_CODEX_SESSIONS_DIR"}, ""),
		...
		MessagesCollection:    env.StringChain([]string{"AGENT_CONTEXT_MESSAGES_COLLECTION"}, "agent_messages_v1"),
		...
		PatternsCollection:   env.StringChain([]string{"AGENT_CONTEXT_PATTERNS_COLLECTION"}, "agent_patterns_v1"),
		...
		// Plan truth sweep
		PlanReconcilerEnabled: env.Bool("AGENT_CONTEXT_PLAN_RECONCILER_ENABLED", true),
		PlanReconcileInterval: env.Duration("AGENT_CONTEXT_PLAN_RECONCILE_INTERVAL", 15*time.Minute),
