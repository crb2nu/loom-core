...
func TestStoreContextEntries_EmbedFailureStillPersists(t *testing.T) {
	...
	if snap.EmbedWriteAttempts != 1 || snap.EmbedFallbackWrites != 1 || snap.EmbedFallbackRatio != 1 {
		t.Errorf("embed fallback telemetry = attempts %d, fallbacks %d, ratio %v; want 1, 1, 1",
			snap.EmbedWriteAttempts, snap.EmbedFallbackWrites, snap.EmbedFallbackRatio)
	}
}
