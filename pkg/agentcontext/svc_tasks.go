...
type TaskSvc struct {
	...
	// Fail-closed gate + counters for the write-path embed fallback (wired by
	// Service after construction; nil keeps the unbounded best-effort behavior
...
func (ts *TaskSvc) Add(ctx context.Context, args map[string]any) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		// Bounded best-effort: below the fail-closed thresholds
		// (embed_degradation.go) the write persists with fallback vectors;
		// past them it is rejected with a typed ErrEmbedderDegraded.
		if derr := recordEmbedWriteOutcome(ts.embedDegradation, ts.metrics, true); derr != nil {
			ts.logger.Warn("task embed degraded past fail-closed thresholds; rejecting write",
				"error", err, "degraded", derr, "count", len(tasks))
			return mcp.ErrorResult(fmt.Errorf("task embed degraded (last error: %v): %w", err, derr)), nil
		}
		...
		if derr := recordEmbedWriteOutcome(ts.embedDegradation, ts.metrics, true); derr != nil {
			ts.logger.Warn("task embed degraded past fail-closed thresholds; rejecting write",
				"got", len(vectors), "want", len(tasks), "degraded", derr)
			return mcp.ErrorResult(fmt.Errorf("task embed degraded (count mismatch %d/%d): %w", len(vectors), len(tasks), derr)), nil
		}
		ts.logger.Warn("task embed count mismatch; persisting with fallback vectors",
			"got", len(vectors), "want", len(tasks))
		vectors = nil
	} else {
		_ = recordEmbedWriteOutcome(ts.embedDegradation, ts.metrics, false)
	}
