...
func newTasksQdrantStub(t *testing.T, seeded ...map[string]any) (*QdrantClient, *tasksQdrantStub) {
	...
	mux.HandleFunc("/collections/"+CollTasks+"/points/delete", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.NotFound(w, r)
			return
		}
		...
		stub.mu.Lock()
		for id := range stub.tasks {
			if wantUUIDs[toPointID(id)] {
				delete(stub.tasks, id)
			}
		}
		stub.mu.Unlock()
		writeJSON(t, w, map[string]any{"status": "ok"})
...
func (s *tasksQdrantStub) hasTask(id string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	_, ok := s.tasks[id]
	return ok
