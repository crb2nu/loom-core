...
func (r *TaskReconciler) reconcile(ctx context.Context) (*ReconcileStats, error) {
	...
	for i := range tasks {
		...
		// Only in-flight work (in_progress/blocked) is deleted when its
		// session record is gone. Pending tasks are backlog items that
		// intentionally outlive their creating session — and its later
		// pruning (see MarkSessionTasksStale).
		if t.SessionID != "" && r.getSession != nil &&
			(t.Status == TaskStatusInProgress || t.Status == TaskStatusBlocked) {
