...
// offset, allowing maintenance sweeps to checkpoint and resume.
func (c *QdrantClient) ScrollPointsPage(ctx context.Context, filter map[string]any, limit int, offset any, withVector bool) ([]RawPoint, any, error) {
	body := map[string]any{"limit": limit, "with_payload": true, "with_vector": withVector}
	if filter != nil {
		body["filter"] = filter
	}
	if offset != nil {
	...
	if err := c.doJSON(ctx, http.MethodPost, path, body, &resp); err != nil {
		if errors.Is(err, ErrCollectionNotFound) {
			return []RawPoint{}, nil, nil
		}
		return nil, nil, err
	}
	out := make([]RawPoint, 0, len(resp.Result.Points))
	for _, p := range resp.Result.Points {
		out = append(out, RawPoint{ID: p.ID, Vector: p.Vector, Payload: p.Payload})
	}
