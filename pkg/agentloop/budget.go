...
// EstimateTokens is a rough chars/4 heuristic used only when a caller does
// not supply a measured system-token count. The budget's hard stop relies on
// the engine-reported prompt_tokens, so this estimate only affects the
// displayed Usable() figure.
