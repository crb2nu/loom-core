...
// Proxy instrumentation headers (see flexinfer internal/proxy/usage_log.go).
// The engine pins routing with HeaderCacheKey on the request and reads the
// rest off each response.
const (
...
// authoritative when present; the header is the fallback.
//
// usageCachedTokens is the body-reported cached count
// (usage.prompt_tokens_details.cached_tokens, or the Responses-shaped
// input_tokens_details equivalent), 0 when the engine omitted it. The body wins
// over the header for the same reason prompt_tokens does: the header is the
// proxy restating what the engine said, so when they disagree the engine is
// right. CachedTokens stays nil unless one of the two sources reported, keeping
// "not reported" distinguishable from a reported zero — the distinction the
// whole prefix-cache measurement rests on.
func parseTurnMetrics(h http.Header, usagePromptTokens, usageCachedTokens int) TurnMetrics {
	...
	if usageCachedTokens > 0 {
		ct := usageCachedTokens
		m.CachedTokens = &ct
	} else if raw := h.Get(HeaderCachedTokens); raw != "" {
		if ct, err := strconv.Atoi(raw); err == nil {
			m.CachedTokens = &ct
		}
	}
	if m.CachedTokens != nil && m.PromptTokens > 0 {
		ratio := float64(*m.CachedTokens) / float64(m.PromptTokens)
