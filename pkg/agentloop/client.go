...
type chatResponse struct {
	...
	Usage struct {
		...
		// PromptTokensDetails / InputTokensDetails carry the cached share of
		// the prompt when the engine reports it. Until now this client read
		// cached tokens ONLY from the X-Flexinfer-Cached-Tokens header, which
		// works because the proxy synthesizes it — but a lane reached without
		// that proxy in front reported nothing. Parsing the body closes that
		// gap and makes the two sources cross-checkable.
...
func (c *ChatClient) Complete(ctx context.Context, msgs []Message, tools []ToolDef, maxTokens int) (Message, TurnMetrics, error) {
	...
	metrics := parseTurnMetrics(httpResp.Header, resp.Usage.PromptTokens,
		llmusage.CachedTokens(resp.Usage.PromptTokensDetails, resp.Usage.InputTokensDetails))
