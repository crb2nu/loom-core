...
func (g *gitlabServer) request(ctx context.Context, method, path string, body any) (map[string]any, error) {
	...
	if err := json.Unmarshal(respBody, &result); err != nil {
		...
		return sanitizeGitLabResponse(map[string]any{"items": arr}).(map[string]any), nil
	}

	return sanitizeGitLabResponse(result).(map[string]any), nil
}
...
func (g *gitlabServer) requestListWithMeta(ctx context.Context, path string) ([]any, map[string]any, error) {
	...
	return sanitizeGitLabResponse(result).([]any), parsePaginationHeaders(respHeaders), nil
}
...
func (g *gitlabServer) doRequest(ctx context.Context, method, reqURL string, body []byte, headers map[string]string) ([]byte, http.Header, error) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 {
			safeBody := sanitizeGitLabErrorText(string(respBody), g.token)
			return nil, respHeaders, mcperror.APIError("GitLab", resp.StatusCode, strutil.TruncateNoEllipsis(safeBody, maxErrorBodyBytes))
		}
...
func (g *gitlabServer) doRequestLimited(ctx context.Context, method, reqURL string, body []byte, headers map[string]string, maxBytes int) ([]byte, *http.Response, bool, error) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 {
			safeBody := sanitizeGitLabErrorText(string(limited), g.token)
			return nil, resp, truncated, mcperror.APIError("GitLab", resp.StatusCode, strutil.TruncateNoEllipsis(safeBody, maxErrorBodyBytes))
		}
...
func (g *gitlabServer) doRequestTail(ctx context.Context, method, reqURL string, headers map[string]string, maxBytes int) ([]byte, *http.Response, int, error) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 {
			safeBody := sanitizeGitLabErrorText(string(tail), g.token)
			return nil, resp, totalRead, mcperror.APIError("GitLab", resp.StatusCode, strutil.TruncateNoEllipsis(safeBody, 8192))
		}
