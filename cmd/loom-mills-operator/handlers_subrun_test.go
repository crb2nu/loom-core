...
func TestSubrunCreate_UsesRequeuedTargetAggregateVersion(t *testing.T) {
	op, cleanup := newTestOperator(t)
	defer cleanup()
	flipRecursion(op, 2, 0.5)
	seedParentRun(t, op, "BACK-VERSION-PARENT", "PIPE-VERSION-PARENT", 0, nil)
	ctx := context.Background()
	target := &store.BacklogItem{
		ID: "BACK-VERSION-CHILD", Title: "requeued child", State: store.BacklogQueued,
		Priority: store.P2, CreatedBy: "test",
	}
	if err := op.store.Backlog.Put(ctx, target); err != nil {
		t.Fatalf("seed child backlog: %v", err)
	}
	claim, err := op.store.ClaimPipelineStart(ctx, store.ClaimPipelineStartRequest{
		BacklogID:            target.ID,
		ExpectedClaimVersion: target.ClaimVersion,
		ExpectedRevision:     target.Revision,
		Template:             "prior-attempt",
		Now:                  time.Now().UTC(),
	})
	if err != nil {
		t.Fatalf("seed prior aggregate: %v", err)
	}
	requeued, err := op.store.Backlog.TransitionState(
		ctx, target.ID, claim.Run.AggregateVersion, store.BacklogRunning, store.BacklogQueued,
	)
	if err != nil {
		t.Fatalf("requeue child aggregate: %v", err)
	}

	rec := postSubrun(t, op, "PIPE-VERSION-PARENT",
		`{"backlog_id":"BACK-VERSION-CHILD","template":"mills-default","estimate_usd":0.4}`)
	if rec.Code != http.StatusCreated {
		t.Fatalf("status: got %d want 201; body=%s", rec.Code, rec.Body.String())
	}
	var resp subrunCreateResponse
	...
	persisted, err := op.store.Pipeline.GetRun(ctx, resp.RunID)
	if err != nil {
		t.Fatalf("load subrun: %v", err)
	}
	if persisted.AggregateVersion != requeued.ClaimVersion || persisted.AggregateVersion != 1 {
		t.Fatalf("subrun aggregate_version=%d want requeued target version %d",
			persisted.AggregateVersion, requeued.ClaimVersion)
	}
	claimed, err := op.store.Backlog.Get(ctx, target.ID)
	if err != nil {
		t.Fatalf("load claimed child: %v", err)
	}
	if claimed.State != store.BacklogRunning || claimed.ClaimVersion != persisted.AggregateVersion {
		t.Fatalf("claimed child=%+v subrun aggregate=%d", claimed, persisted.AggregateVersion)
	}
}
