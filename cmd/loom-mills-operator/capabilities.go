...
type capabilityWiring struct {
	...
	// MCPHubLiveHealth is evaluated on every capability report. Startup
	// session readiness alone goes stale when the gateway later loses DNS or a
	// shared transport fails; the live callback turns that incident into an
	// autonomy blocker until a subsequent call succeeds.
	MCPHubLiveHealth      func() (healthy bool, message string)
	...
	for _, def := range capabilityDefinitions {
		...
		case capabilityMCPHubSession:
			if w.MCPHubSessionReady {
				healthy, message := true, "MCP hub and operator agent-context session are live"
				if w.MCPHubLiveHealth != nil {
					healthy, message = w.MCPHubLiveHealth()
				}
				if healthy {
					row.Status = string(capabilityGreen)
					row.Mode = string(capabilityModeReal)
