...
func TestHandleSquadRouteTest_HappyPath(t *testing.T) {
	...
	defer cleanup()
	ctx := context.Background()
	// route-test is an authenticated read-only diagnostic and must remain
	// usable while workload admission is closed.
	off := false
...
// seedActiveRoutedRun seeds a council + backlog + ACTIVE pipeline run and
// the squad_routed attribution event the reconciler would have emitted.
func seedActiveRoutedRun(t *testing.T, op *operator, runID, backlogID, squadName string) {
	t.Helper()
	ctx := context.Background()
	council := "COUNCIL-INFLIGHT-" + runID
	if err := op.store.Council.Put(ctx, &store.CouncilRun{
		ID: council, Trigger: store.CouncilTriggerCron,
		StartedAt: time.Now().UTC(), Outcome: store.CouncilOutcomeSuccess,
	}); err != nil {
	...
	if err := op.store.Backlog.Put(ctx, &store.BacklogItem{
		ID: backlogID, Title: "in-flight test", State: store.BacklogRunning,
		Priority: store.P2, CreatedBy: "test", CouncilRunID: &council,
	}); err != nil {
		t.Fatalf("seed backlog: %v", err)
	}
	...
	if err := op.store.Events.Append(ctx, &store.Event{
		Actor: "reconciler", Kind: squads.SquadRoutedEventKind,
		SubjectKind: squads.SquadRoutedSubjectKind, SubjectID: runID,
		Payload: map[string]any{
			"run_id": runID, "backlog_id": backlogID,
			"squad_name": squadName, "path_class": "internal/hud/frontend/**",
			"outcome": "ok",
		},
	}); err != nil {
...
func TestInFlightBySquad_CountsActiveAttributedRuns(t *testing.T) {
	op, cleanup := newOperatorWithSquads(t, map[string]string{
		"hud-frontend": fixtureSquadHUD,
	})
	defer cleanup()

	seedActiveRoutedRun(t, op, "PIPE-IF-1", "MILLS-IF-1", "hud-frontend")
	// A fallback-routed active run must not count.
	seedActiveRoutedRun(t, op, "PIPE-IF-2", "MILLS-IF-2", squads.FallbackName)
	// A running imperative (workflow-lane) run with attribution counts too —
	// the S3 parity: in_flight covers both start lanes.
	ctx := context.Background()
	if err := op.store.Backlog.Put(ctx, &store.BacklogItem{
		ID: "MILLS-IF-3", Title: "in-flight workflow test", State: store.BacklogRunning,
		Priority: store.P2, CreatedBy: "test",
	}); err != nil {
		t.Fatalf("seed workflow backlog item: %v", err)
	}
	if _, err := workflow.CreateImperativeRun(ctx, op.store.Workflow, "WFRUN-IF-1", "MILLS-IF-3"); err != nil {
		t.Fatalf("seed imperative run: %v", err)
	}
	if err := op.store.Events.Append(ctx, &store.Event{
		Actor: "reconciler", Kind: squads.SquadRoutedEventKind,
		SubjectKind: squads.SquadRoutedWorkflowSubjectKind, SubjectID: "WFRUN-IF-1",
		Payload: map[string]any{
			"run_id": "WFRUN-IF-1", "backlog_id": "MILLS-IF-3",
			"squad_name": "hud-frontend", "path_class": "internal/hud/frontend/**",
			"outcome": "ok", "lane": "workflow",
		},
	}); err != nil {
		t.Fatalf("seed workflow attribution event: %v", err)
	}

	got, err := op.inFlightBySquad(context.Background())
	if err != nil {
		t.Fatalf("inFlightBySquad: %v", err)
	}
	if got["hud-frontend"] != 2 {
		t.Errorf("hud-frontend in_flight: got %d want 2 (pipeline + workflow lanes; map=%v)", got["hud-frontend"], got)
	}
	if got[squads.FallbackName] != 0 {
		t.Errorf("fallback must not be counted: %v", got)
	}

	// The list endpoint surfaces the count in outcome_stats.in_flight.
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/mills/squads", nil))
	if rec.Code != http.StatusOK {
	...
	for _, e := range entries {
		if e.Squad.Name == "hud-frontend" {
			found = true
			if e.OutcomeStats.InFlight != 2 {
				t.Errorf("list in_flight: got %d want 2 (pipeline + workflow lanes)", e.OutcomeStats.InFlight)
			}
		}
	}
	if !found {
		t.Fatalf("hud-frontend not in list response: %s", rec.Body.String())
	}
}
