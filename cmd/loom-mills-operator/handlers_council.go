...
// councilRequestBudget bounds an HTTP-triggered Council run end-to-end.
// councilWriteDeadline leaves enough room to return the timeout response after
// that budget expires instead of letting httpServer's 30-second default close
// the connection while the Council handler is still executing.
const (
	councilRequestBudget = 10 * time.Minute
	councilWriteDeadline = councilRequestBudget + 30*time.Second
)
...
func (o *operator) runCouncil(w http.ResponseWriter, r *http.Request, dryrun bool) {
	// Council calls routinely exceed httpServer's 30-second WriteTimeout. Extend
	// this response before writing anything so a completed Council run can still
	// return its result to the caller. Some ResponseWriters do not support write
	// deadlines, so treat this as best-effort and retain the request timeout below.
	_ = http.NewResponseController(w).SetWriteDeadline(time.Now().Add(councilWriteDeadline))
