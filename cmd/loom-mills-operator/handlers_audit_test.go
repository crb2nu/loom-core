...
func TestHandleAuditRun_SyncProviderOutageReturns503WithoutFinding(t *testing.T) {
	setAdminToken("secret-abc")
	defer setAdminToken("")

	op, cleanup := auditOpFixture(t)
	defer cleanup()
	op.auditDispatcher.Reviewers["flexinfer"] = &fakeAuditReviewer{err: errors.New("upstream 503")}
	if err := op.store.Council.Put(context.Background(), &store.CouncilRun{
		ID: "COUNCIL-OUTAGE", Trigger: store.CouncilTriggerCron,
		StartedAt: time.Now().UTC(), Outcome: store.CouncilOutcomeSuccess,
	}); err != nil {
		t.Fatalf("seed council: %v", err)
	}

	body := bytes.NewBufferString(`{"subject_kind":"council_artifact","subject_id":"COUNCIL-OUTAGE"}`)
	req := httptest.NewRequest(http.MethodPost, "/api/mills/audit/run?sync=true", body)
	req.Header.Set("Authorization", "Bearer secret-abc")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, req)
	...
	if !strings.Contains(rec.Body.String(), audit.ErrNoAuditReviewersAvailable.Error()) {
		t.Fatalf("response body = %q, want audit outage error", rec.Body.String())
	}
	rows, err := op.store.Audit.ListForSubject(context.Background(),
		store.AuditSubjectCouncilArtifact, "COUNCIL-OUTAGE")
	if err != nil {
		t.Fatalf("list findings: %v", err)
	}
	if len(rows) != 0 {
		t.Fatalf("persisted findings = %d, want 0", len(rows))
	}
}

func TestHandleAuditRun_SyncValidationErrorRemains400(t *testing.T) {
	setAdminToken("secret-abc")
	defer setAdminToken("")
	...
	defer cleanup()
	if err := op.store.Council.Put(context.Background(), &store.CouncilRun{
		ID: "COUNCIL-BAD-POOL", Trigger: store.CouncilTriggerCron,
		StartedAt: time.Now().UTC(), Outcome: store.CouncilOutcomeSuccess,
	}); err != nil {
		t.Fatalf("seed council: %v", err)
	}
	...
	body := bytes.NewBufferString(`{
		"subject_kind":"council_artifact",
		"subject_id":"COUNCIL-BAD-POOL",
		"pool":[{"backend":"","model":""}]
	}`)
	req := httptest.NewRequest(http.MethodPost, "/api/mills/audit/run?sync=true", body)
	req.Header.Set("Authorization", "Bearer secret-abc")
