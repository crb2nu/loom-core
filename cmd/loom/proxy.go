...
func runProxyWithHint(socketPath, agentHint, remoteURL, remoteToken, toolProfile string, maxTools int) error {
	...
	// Repo-local (.loom/proxy.yaml) and env overrides shadow the CLI flags:
	// home-level vendor configs (Antigravity) pass one profile for every
	// repo, so per-repo shaping has to resolve here. See proxy_repo_config.go.
	toolProfile, maxTools = applyProxyProfileOverrides(agentHint, strings.TrimSpace(toolProfile), maxTools)
