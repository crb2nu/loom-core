...
func handleProxyToolsCall(ctx context.Context, daemon mcp.Transport, msg *mcp.Message) (*mcp.Message, error) {
	...
	if resp.Error == nil && len(resp.Result) > 0 {
		...
		if err := json.Unmarshal(resp.Result, &result); err == nil {
			changed := truncateCallToolResult(&result, proxyMaxToolResultBytesFor(serverName, toolName), proxyMaxImageResultBytes())
			// MR-status trailer (mrwatch M3b) runs after truncation so the
			// short trailer is never itself truncated. Fail-open + delta-gated.
			if appendMRStatusTrailer(&result, serverName, toolName) {
				changed = true
			}
			if changed {
