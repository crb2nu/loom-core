...
func TestMillsPipelinesGet_RendersNestedOperatorDetail(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/mills/pipeline/runs/PIPE-DETAIL", func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{
			...
			"gates": [{
				...
				"Outcome": "pass",
				"JudgedBy": "go",
				"Reasons": ["within declared slice"],
				"EvaluatedAt": "2026-07-22T12:02:00Z"
			}]
		}`))
	})
	...
	cmd := newMillsCmd()
	cmd.SetArgs([]string{"pipelines", "get", "PIPE-DETAIL", "--operator-url", srv.URL})
	...
	for _, want := range []string{
		"Pipeline run: PIPE-DETAIL",
		"backlog item:   MILLS-DETAIL",
		"state:          running",
		"current stage:  implement",
		"merge request:  !42",
		"plan_slice",
		"scope after post_implement_gate: pass (go) — within declared slice",
	} {
