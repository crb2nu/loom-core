...
const defaultMillsCouncilRequestTimeout = 10 * time.Minute
...
func newMillsCouncilRunCmd(dryrun bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// Most Mills endpoints use the parent's short HTTP timeout, but a
			// Council pass can legitimately consume the full request budget.
			// Preserve an explicit --timeout while extending only the default.
			if !cmd.Flags().Changed("timeout") {
				client.http.Timeout = defaultMillsCouncilRequestTimeout
			}
			ctx, cancel := context.WithTimeout(cmd.Context(), defaultMillsCouncilRequestTimeout)
			defer cancel()
