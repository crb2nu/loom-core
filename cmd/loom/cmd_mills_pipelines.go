...
// pipelineRunDetail mirrors the operator's GetRun handler shape — a nested
// PipelineRun plus stage_results and gate_outcomes so the CLI can render a
// one-call detail view.
type pipelineRunDetail struct {
	Run    pipelineRunSummary `json:"run"`
...
func millsCanaryBacklogItem(id, title, priority, path string) store.BacklogItem {
	// Delegate to the shared builder so the CLI's manual-enqueue path and the
	// operator's canary-autopilot scheduler produce a byte-identical canary
	// item — no more "kept in sync by hand" drift between the two callers.
	return store.CanaryHeartbeatItem(id, title, priority, path, "loom mills pipelines canary")
}
