...
type millsDeadlineRoundTripper func(*http.Request) (*http.Response, error)

func (f millsDeadlineRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req)
}
...
func TestMillsCouncilRun_RequestTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "run default", mode: "run", wantMin: 9 * time.Minute, wantMax: 10*time.Minute + time.Second},
		{name: "dryrun default", mode: "dryrun", wantMin: 9 * time.Minute, wantMax: 10*time.Minute + time.Second},
		{name: "run explicit override", mode: "run", timeoutArg: "2s", wantMin: time.Second, wantMax: 3 * time.Second},
		{name: "dryrun explicit override", mode: "dryrun", timeoutArg: "2s", wantMin: time.Second, wantMax: 3 * time.Second},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var observed time.Duration
			http.DefaultTransport = millsDeadlineRoundTripper(func(req *http.Request) (*http.Response, error) {
				...
				return &http.Response{
					...
					Body:       io.NopCloser(strings.NewReader(`{"run_id":"COUNCIL-T","score":1}`)),
					Request:    req,
				}, nil
			})

			args := []string{"--operator-url", "http://127.0.0.1", "council", tt.mode}
			if tt.timeoutArg != "" {
				args = append([]string{"--operator-url", "http://127.0.0.1", "--timeout", tt.timeoutArg}, "council", tt.mode)
			}
			cmd := newMillsCmd()
			cmd.SetArgs(args)
			...
			if observed < tt.wantMin || observed > tt.wantMax {
				t.Fatalf("request deadline remaining = %v, want between %v and %v", observed, tt.wantMin, tt.wantMax)
			}
		})
	}
}
