...
// withTool wraps an MCP handler so every outbound ICC request it makes
// carries X-ICC-MCP-Tool: <name>. Pure attribution — no behavior
// change. Handlers that do no HTTP I/O (format-only, lint) are wrapped
// the same way so registration stays uniform; the header just goes
// unused.
func withTool(name string, h iccToolHandler) iccToolHandler {
	return func(ctx context.Context, args map[string]any) (*mcp.CallToolResult, error) {
		return h(iccclient.WithTool(ctx, name), args)
	}
}
...
func registerTools(srv *mcpscaffold.Server, icc *iccClient) {
	...
	}, withTool("icc_archive_raw", makeArchiveRawHandler(icc)))
	...
	}, withTool("icc_capture_email", makeCaptureEmailHandler(icc)))
	...
	}, withTool("icc_capture_meeting", makeCaptureMeetingHandler(icc)))
	...
	}, withTool("icc_capture_slack", makeCaptureSlackHandler(icc)))
	...
	}, withTool("icc_capture_standup", makeCaptureStandupHandler(icc)))
	...
	}, withTool("icc_demote_artifact", makeDemoteArtifactHandler(icc)))
	...
	}, withTool("icc_format_email_extract", handleFormatEmailExtract))
	...
	}, withTool("icc_format_meeting_notes", handleFormatMeetingNotes))
	...
	}, withTool("icc_format_slack_paste", handleFormatSlackPaste))
	...
	}, withTool("icc_format_standup", handleFormatStandup))
	...
	}, withTool("icc_lint_notes", handleLintNotes))
	...
	}, withTool("icc_promote_to_artifact", makePromoteToArtifactHandler(icc)))
	...
	}, withTool("icc_unarchive_raw", makeUnarchiveRawHandler(icc)))
	...
	}, withTool("icc_write_capture", makeWriteCaptureHandler(icc)))
}
