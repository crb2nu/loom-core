...
func registerContextTools(server *mcp.Server, svc *agentcontext.Service, tracer trace.Tracer) {
	...
	// NOTE: agent_context_stats removed in SIMP-8. The capability was removed,
	// not relocated: there is no `loom agent` equivalent. Prometheus/HTTP
	// metrics on the server itself are the remaining introspection surface.

	// NOTE: agent_code_annotate / agent_code_annotations_get removed. Use
	// agent_context_add with entry_type="annotation" (see the annotation_type
	// and symbol properties above) and agent_context_search to read them back.

	// NOTE: agent_context_recall / agent_context_recall_enhanced removed.
	// Both were deprecated aliases that forwarded to the unified recall
	// internals below; call agent_recall directly.

	// NOTE: agent_context_recall_since removed. The delta/cursor primitive
	// never acquired a consumer (no CLI, no HUD endpoint, no skill).
