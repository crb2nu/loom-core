...
func registerTools(server *mcp.Server, svc *agentcontext.Service, tracer trace.Tracer) {
	...
	registerPlanTools(server, svc, tracer)
	registerPatternTools(server, svc, tracer)
	registerEngramTools(server, svc, tracer)
	registerBridgeTools(server, svc, tracer)
	registerMaintenanceTools(server, svc, tracer)
}
