...
// makeCodex constructs the SDK client with an explicit CLI path. codex-sdk's
// findCodexPath() only looks for vendored binaries inside the @openai/codex
// npm package — unreachable from the standalone bundle (see exec-path.ts) —
// so without the override every live codex spawn throws "Unable to locate
// Codex CLI binaries". The pod installs `codex` on PATH.
function makeCodex(): Codex {
  const codexCli = resolveExecutableFromPath("codex", "LOOM_SPAWN_CODEX_CLI");
  return codexCli ? new Codex({ codexPathOverride: codexCli }) : new Codex();
}
