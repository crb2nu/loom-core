...
import { resolveExecutableFromPath } from "./exec-path.js";
...
export async function runClaudeDriver(args: DriverArgs): Promise<number> {
  ...
  const options: Options = {
    ...
    // The 0.3.x Options contract documents this flag as required alongside
    // bypassPermissions ("must be set to true"); 0.2.x tolerated omitting it.
    allowDangerouslySkipPermissions: true,
    includePartialMessages: false,
  };

  // agent-sdk 0.3.x resolves a platform-native CLI from its own optional npm
  // packages by default — impossible next to a standalone bundle (see
  // exec-path.ts). Point it at the pod's PATH-installed `claude` instead;
  // when nothing is on PATH, leave it unset so the SDK's own lookup still
  // serves node_modules-based dev setups.
  const claudeCli = resolveExecutableFromPath("claude", "LOOM_SPAWN_CLAUDE_CLI");
  if (claudeCli) {
...
function forwardMessage(message: SDKMessage): void {
  switch (message.type) {
    case "system": {
      // agent-sdk 0.3.x reports hook execution as system-subtype messages
      // (hook_started / hook_response) instead of a dedicated type, so they
      // dodge the default-case drop below. Pods run injected hooks on every
      // tool call — forwarding each pair would swamp the JSONL stream with
      // events the Go parser ignores.
