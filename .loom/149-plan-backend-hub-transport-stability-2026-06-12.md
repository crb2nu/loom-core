...
> **Promoted by the Slice 2 tier-2 finding (2026-06-18, `.loom/157`)**: the residual close-1006 after keepalive is a synchronized **~17-server forced kill every ~8 min**, i.e. hub-side, not client idle. Client-side levers (Slice 2) can't prevent it.
> **Scoping done 2026-06-18**: ruled OUT hub MCP pod cycling — all loom-hub server pods were `RESTARTS 0`, stable ~63m (the 02:13 mass rollout was just Flux picking up !723/!724, a one-time event, not the 8-min cadence). So the periodic kill is **upstream of the pods**: either **Cloudflare** (`wss://mcp.flexinfer.ai/ws` is CF-fronted — suspect a max-connection-age / WS idle policy) or **`loom-gateway`** (2 replicas `loom-gateway-7d94ff4584-{p92pp 5d8h/0, rxspc 15d/4 restarts}`; emits structured OTEL traces, not plain close logs — needs a targeted trace/span query for close events, or gateway-side disconnect-reason logging à la Slice 1 but on the gateway). NEXT: add gateway client-disconnect-reason logging (lives in `libs/fi-mcp-kit/pkg/gateway/hub.go` per `.loom/150` Slice 1 out-scope) OR check the Cloudflare WS timeout config for `mcp.flexinfer.ai`.
