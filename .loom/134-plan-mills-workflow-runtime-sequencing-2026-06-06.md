...
> ## 🔴🟢 S1c RESULT (2026-07-08, live dual-crash run — harness loom-core!1007, canary window gitops!333/!334)
> **The exactly-once bet HOLDS live; the gate is NOT yet passed because the run-to-terminal leg failed on a spawn-controller liveness defect.** Split verdict:
>
> - **PASS-1 (no double-spawn, durable path) — PASSED.** Canary run `wf-canary-1780869560631332347`, agent step `root/agent~ebadc8c7de4127bf#0`, derived spawn id `spawn-a2c56524561a`. Exactly ONE pod (`spawn-spawn-a2c56524561a`, ns `devbox`) across CRASH A (operator force-delete) + CRASH B (mobile-hud force-delete) + an unplanned third roll (Flux IUA image bump mid-test). Durable-path evidence captured: operator restart logged `workflow resume: re-attaching to in-flight spawn" run_id=wf-canary-… spawn_id=spawn-a2c56524561a` at 23:21:02Z. The pending journal row picked up `spawn_id` via `recordPendingSpawnID` when CRASH B failed the in-flight dispatch — the exact designed recovery path.
> - **PASS-2/4/5 — NOT REACHED (honest timeout, not divergence).** The step never completed: **CRASH B orphans in-flight exec-driven agent turns.** The spawn pod's process model is `sleep infinity` + a mobile-hud-driven exec session; killing mobile-hud severs the exec, the restarted controller `LoadAll`-rehydrates the record (`status: running`, `timeout_minutes: 60`) but nothing re-drives the turn and the 60m timeout is never enforced → the spawn record is a permanent zombie and the workflow step wedges `pending`. No double-spawn, no journal divergence, no quarantine, no crash-loop — the §3.5 FAIL modes did NOT occur.
> - **Consequence**: do NOT fall back to declarative-DAG (`.loom/132 §7b`) — the Layer-3 journal/idempotency machinery behaved exactly as designed. The blocker is in `internal/spawn` (mobile-hud): on `LoadAll`, a rehydrated `running` spawn with no live driver must be re-driven or failed (and the request timeout enforced controller-side). Fix that, re-run the harness (`go run ./cmd/mills-workflow-killtest --run-id …` per the runbook), and S1c should complete.
> - **Two harness-learned corrections folded into loom-core!1007**: (a) §3.3 step 2's "poll until pending row shows spawn_id" never fires on a HEALTHY dispatch (spawn_id is recorded on completion/failure only) — the identity must be DERIVED from `run_id+step_key+call_hash` (`DeriveStepIdempotencyKeyFromHash` + `worker.DeriveSpawnID`); (b) side-finding: `DAOJournal.ensureRun` UPSERTs the run row and clobbers `template`/`started_at` of a pre-existing run (observed: admin-created `workflow-canary` row re-stamped to `workflow-seed`).
> - Evidence JSON: `.loom/local/s1c-evidence-20260708.json` (local). Preflight facts now live-verified: operator selector `app.kubernetes.io/name=loom-mills-operator`, mobile-hud selector `app=mobile-hud`, spawn state ConfigMap `loom-spawn-state` in ns `devbox`.
