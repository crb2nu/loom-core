...
- **Mills — Full-vision roadmap: substrate-proven → autonomous-merges-at-scale (2026-06-01)**: `126-plan-mills-full-vision-roadmap-2026-06-01.md` — converges the autonomy plumbing (`43`), the empty-MR retro (`44`), the harvester-vm substrate (`45`, now shipped through Slice 2d.5d + kill-test PASSED 2026-06-01), and the v2 swarm (`docs/MILLS.md`). North-star `autonomous_merges_24h` is still **0/56**. **Riskiest assumption** = routing a real canary's implement+tests to harvester-vm produces a *non-empty* MR that auto-merges (the empty-MR gap close). **Phase A (critical path)**: A1 wire harvester-vm into prod spawn path → A2 first autonomous merge (kill-test) → A3 sustain 7 green. **Phase B**: curated base image, warm pool, k8s health-fallback. **Phase C**: notify webhook URL, workspace-signals council brief, LLM-ranked dispatch. **Phase D**: flip implement/tests defaults + sequential v2 Phase-8.3 flips. Nothing past A starts until one canary merges unattended. Prod policy `configmap-policy.yaml:104` is `stage_substrate: {}` (default-OFF), flip gated on "≥1 auto-merge".
- **Mills — Harvester KubeVirt VM substrate spec + Slice 2d.5b shipped (2026-05-25 → 2026-05-28)**: `45-product-spec-mills-harvester-vm-substrate-2026-05-25.md` (spec, with Slices 0 + 1.5 + 1 + 2a + 2b + 2c + 2d + 2d.5 + 2d.5b all marked shipped) + `120` + `121` + `122` + `123` + `124` iteration plans. **Slice 0** capacity kill-test PASSED. **Slice 1.5** base VM + plumbing PASSED. **Slice 1** `harvester-vm` devbox backend shipped via `1ed1cd05` + `82c3bc17`. **Slice 2a** policy field shipped 2026-05-26 as `2a9c6c3a`. **Slice 2b** SpawnWorker → SpawnRequest.Substrate shipped 2026-05-27 as `a6979762` ([!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)). **Slice 2c** Substrate over the wire + DEVBOX_BACKEND on pod env shipped 2026-05-27 as `57052ce0` ([!566](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/566)). **Slice 2d** HUD multi-backend init + per-spawn substrate routing shipped 2026-05-27 as `88bc0bcc` ([!568](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/568)). **Slice 2d.5** HarvesterVMBackend env + SecretEnv propagation shipped 2026-05-27 as `0d6f8a03` ([!570](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/570)) — new `backend.SecretResolver` interface; cloud-init `write_files` at `/etc/loom-spawn.env`; Exec sources it. **Slice 2d.5b** SecretMount propagation shipped 2026-05-28 as `6c537079` ([!573](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/573)) — SecretResolver extended with `ResolveSecretMounts`; K8sBackend implements it (per-SecretName cache, Optional semantics, defensive content copy); buildVMManifest takes `files`, renderCloudInitWriteFiles emits a single `write_files:` block combining env + per-file entries with `encoding: b64`, owner `ubuntu:ubuntu`, mode default `0600`. **Next: Slice 2d.5c** — home-dir parity (`agent` user on the VM, or symlink, or mountpath remap) so agent CLIs find the mounted auth files at their expected `$HOME/.…` paths. Codex on harvester-vm stays blocked until 2d.5c. Then Slice 2e (mcp-devbox multi-backend init) + Slice 2 acceptance (1 end-to-end canary auto-merge on harvester-vm).
- **Mills — Autonomy round retro (2026-05-25)**: `44-mills-autonomy-retro-2026-05-25.md` — closes the round tracked in plan 43. 10 MRs shipped (loom-core !517/!519/!521 + gitops !172-!176). All 8 slices + 2 follow-up fixes verified clean on the harness. **Live end-to-end auto-merge did NOT happen across 3 canary cycles**: every cycle produced an empty MR (`!518`/`!520`/`!522`, all closed) because the `implement` codex spawn returns `turn_count=1 status=completed` with zero file modifications. Kill-test confirms this is pre-existing (all 27 historical implements had `no_diff`); the autonomy round's upstream fixes (2c+2e) finally carried a canary to the `mr` stage and exposed the spawn-execution gap. Recommended next: investigate codex spawn integration (workdir/credentials/prompt delivery), then wire WorktreePath through SpawnResponse so the already-shipped `BranchPusher` fallback fires.
- **Mills — Autonomy round: make it ship MRs without me (2026-05-24)**: `43-plan-mills-autonomy-2026-05-24.md` — three-slice balanced plan to lift Mills from "operator-driven with occasional autonomy" to "merged MRs/day with no human touch". Slice 0 kill-test (14d telemetry pull, gates Slice 2 ordering); Slice 1 demand (GitLab issue importer + workspace-signals council brief + LLM-ranked dispatch); Slice 2 supply (wire `MergeWhenPipelineSucceeds=true` + promote `pr_self_review` to merge-blocking + transient-vs-code retry classification + per-spawn error_class metric); Slice 3 loop closure (webhook notify on auto-merge + tick-on-merge + outcome→ranker feedback + bounded escalation auto-retry). Riskiest assumption: supply-side fixes outrank demand-side + loop-closure. Sources: parallel demand/supply/loop-closure explore agents 2026-05-24; cited file:line throughout. Successor to (different layer from) plan 42.
- **Mills — Next round of functionality + UX fixes (2026-05-18)**: `42-plan-mills-next-round-fixes-2026-05-18.md` — five-slice plan gated behind a 30-min operator kill-test (HUD↔CLI parity assumption). Slices: 1) HUD parity for 5 day-2 actions (pause/replay/escalate/audit/run) **— kill-test-gated, not started**; 2) KPI honesty **shipped 2026-05-19 (`dbccd413`)**; 3) sub-tab counts for squads/audit/cross-repo **shipped 2026-05-19 (`f99a48ef`)**; 4) spawn/escalator stabilization debt + STATES.md **shipped 2026-05-19 (`985f2f5d`)** — audit found all 6 fix commits already had regression tests, STATES.md lives at `pkg/mills/pipeline/STATES.md`; 5) idle-state one-click "Run council now" **shipped 2026-05-19 (`53419638`)**. Sources: `mcp/skills/mills-ops/SKILL.md`, commit 23e40ad2, `internal/hud/frontend/src/lib/components/Mills/*`.
- **HUD UI/UX Overhaul — Operator Inbox + Foundations planning trio (2026-05-15)** ✅ **COMPLETE 2026-05-26 (16 days ahead)**: `115-brainstorm-hud-ux-improvements-2026-05-15.md`, `116-product-spec-hud-ux-overhaul-2026-05-15.md`, `117-implementation-plan-hud-ux-overhaul-2026-05-15.md`. All 10 sub-slices merged. Slice A: A1 action toolkit primitives (`4c46f44f`/`11f75bd2`), A2 triage overview rebuild (`406890a1`), A3 theme sweep F4-full (`cd8cfd28`). Slice B: B1 FleetPanel decomp+pattern (`b75f6c1c`, 1777→176 lines), B2.1–B2.5 remaining-panel decomps (`abe7c167`/`1a98acec`/`1aebb573`/`cab43b29`/`ae29b152` — all <300 lines), B3 SSE migration (`df180795` + closeout `4baba66b`), B4 Cmd+P + j/k/Enter nav (`556c6f95`), B5 responsive ≤800px + embed subset (`86b77505`). Six panel stores (fleet/tasks/sandbox/spawn/servers/stream) wired for SSE + staleness with ConnectionBanner stale pill. F6 IA reorg explicitly deferred (no spec yet). One follow-up tracked in plan-doc: align 8 secondary OverviewPanel stores from 30s→≥60s if request volume proves noisy.
...
- **Roadmap reconciliation + next-epic plan (2026-05-07)**: `105-planning-roadmap-reconciliation-and-next-epics-2026-05-07.md` — confirmed Spectator Phases 0–5 + EPIC 2 Batches A–D shipped (13/14 UNIFY slices); identified S4 (UNIFY-1d HUD handler migration) + Spectator Phase 6 + Harbor #1 final as close-out slices. **UNIFY-1d closeout shipped 2026-05-21** (removed residual Cost+Presence aliases; EPIC 2 now 14/14 ✅). **Correction**: SIMP-1..SIMP-12 actually shipped 2026-03-01..03 (commits `6bd95261`..`e050516f`); EPIC 1 #65 is a stale tracker, not a fresh planning target. Pivoted next-epic recommendation to **EPIC 3 (#67)**.
- **EPIC 2 — Unify Visibility planning trio (2026-05-06)** ✅ **COMPLETE 2026-05-21**: `102-research-unify-visibility-2026-05-06.md`, `103-product-spec-unify-visibility-2026-05-06.md`, `104-implementation-plan-unify-visibility-2026-05-06.md` ([Issue #66](https://gitlab.flexinfer.ai/services/loom-core/-/issues/66) — UNIFY-3 shipped `d0d1518`; Batches A–D shipped via [!304](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/304), [!305](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/305), [!308](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/308), [!312](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/312); **S4 (UNIFY-1d) closed out 2026-05-21** — bridge.{PresenceInfo,CostStatsResult,CostAgentUsage,CostServerUsage,CostTotals} aliases removed, `DaemonClient.CostStats()`/`AgentBridge.PresenceList()` signatures use `costpkg.*`/`presencepkg.*` directly; `make ci-contracts` confirmed all 27 mobile golden files byte-identical)
...
- **Close-out slices (< 2 sessions total)** — recommended **before** the next epic:
  - **UNIFY-1d (S4)** ✅ **SHIPPED 2026-05-21**: removed residual `bridge.{PresenceInfo,CostStatsResult,CostAgentUsage,CostServerUsage,CostTotals}` aliases; `DaemonClient.CostStats()`/`AgentBridge.PresenceList()` now use `costpkg.*`/`presencepkg.*` directly. Plan-doc path was wrong (`internal/hud/handlers/*` never existed); real call sites were `internal/hud/monitor/{health,fleet}.go` + bridge internals. `make ci-contracts` confirmed mobile golden files byte-identical.
