...
- **Mills — Harvester KubeVirt VM substrate spec + Slice 2d.5 shipped (2026-05-25 → 2026-05-27)**: `45-product-spec-mills-harvester-vm-substrate-2026-05-25.md` (spec, with Slices 0 + 1.5 + 1 + 2a + 2b + 2c + 2d + 2d.5 all marked shipped) + `120` + `121` + `122` + `123` iteration plans. **Slice 0** capacity kill-test PASSED. **Slice 1.5** base VM + plumbing PASSED. **Slice 1** `harvester-vm` devbox backend shipped via `1ed1cd05` + `82c3bc17`. **Slice 2a** policy field shipped 2026-05-26 as `2a9c6c3a`. **Slice 2b** SpawnWorker → SpawnRequest.Substrate shipped 2026-05-27 as `a6979762` ([!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)). **Slice 2c** Substrate over the wire + DEVBOX_BACKEND on pod env shipped 2026-05-27 as `57052ce0` ([!566](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/566)). **Slice 2d** HUD multi-backend init + per-spawn substrate routing shipped 2026-05-27 as `88bc0bcc` ([!568](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/568)). **Slice 2d.5** HarvesterVMBackend env + SecretEnv propagation shipped 2026-05-27 as `0d6f8a03` ([!570](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/570)) — new `backend.SecretResolver` interface (K8sBackend implements natively); HarvesterVMBackend.Start resolves SecretEnv to plaintext + merges with opts.Env, renders to cloud-init `write_files` at `/etc/loom-spawn.env`; Exec sources the file before applying its export prefix; runSpawn passes `Env: env` through the buffered agent-exec call. **Next: Slice 2d.5b** — SecretMount (file-style auth, `~/.claude.auth/oauth.json` and `~/.codex/auth.json`) propagation via cloud-init `write_files`. Without it codex stays blocked on harvester-vm; claude-code can use ANTHROPIC_API_KEY fallback; gemini works end-to-end now. Then Slice 2e (mcp-devbox multi-backend init) + Slice 2 acceptance (1 end-to-end canary auto-merge on harvester-vm).
