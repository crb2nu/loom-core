...
- **Mills — Harvester KubeVirt VM substrate spec + Slice 2d shipped (2026-05-25 → 2026-05-27)**: `45-product-spec-mills-harvester-vm-substrate-2026-05-25.md` (spec, with Slices 0 + 1.5 + 1 + 2a + 2b + 2c + 2d all marked shipped) + `120-iteration-plan-mills-spawn-substrate-slice2b-2026-05-27.md` + `121-iteration-plan-mills-spawn-substrate-slice2c-2026-05-27.md` + `122-iteration-plan-mills-spawn-substrate-slice2d-2026-05-27.md`. **Slice 0** capacity kill-test PASSED (4 concurrent VMs at 72% CPU / 78% MEM, no node disruption). **Slice 1.5** base VM + plumbing PASSED (DHCP + qemu-guest-agent, 130s cold-boot). **Slice 1** `harvester-vm` devbox backend (~2583 LOC + tests) shipped via `1ed1cd05` + `82c3bc17`. **Slice 2a** `pipeline.stage_substrate` policy field shipped 2026-05-26 as `2a9c6c3a`. **Slice 2b** `SpawnWorker.SubstrateFor` closure + `SpawnRequest.Substrate` field + operator wiring shipped 2026-05-27 as `a6979762` ([!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)). **Slice 2c** `HUDSpawnClient.Run` propagates `Substrate` over the spawn POST + HUD orchestrator promotes it to `DEVBOX_BACKEND` on the pod env shipped 2026-05-27 as `57052ce0` ([!566](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/566)). **Slice 2d** HUD multi-backend init + per-spawn substrate routing shipped 2026-05-27 as `88bc0bcc` ([!568](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/568)) — `SpawnOrchestrator.backends map[string]backend.Backend` + new `substrateBackend(s)` helper; `runSpawn` resolves backend via `req.Substrate`; `StopSpawn`/`failSpawn`/`reapTerminalSpawn` resolve via `state.Request.Substrate`; `initSpawnOrchestrator` registers k8s always + optional HarvesterVMBackend when `SpawnHarvesterKubeconfig != ""`; 9 new `--spawn-harvester-*` flags + `SPAWN_HARVESTER_*` env-vars. **Next: Slice 2d.5** — `HarvesterVMBackend.Start` env+secret propagation via cloud-init systemd env file (currently drops `opts.Env` + `opts.SecretEnv`) + orchestrator's primary agent-exec call threads env through `ExecOpts.Env`. Without this, harvester-vm spawns can't deliver API keys to the agent CLI. Then Slice 2e (mcp-devbox multi-backend init) + Slice 2 acceptance (1 end-to-end canary auto-merge on harvester-vm).
