...
- **Roadmap extraction state — Mills reliability guardrails (2026-06-30)**: canonical Plan Store item `plan-mills-mills-2026-06-30-001` / backlog `MILLS-2026-06-30-001` is the active source for this guardrail slice. The store was resolved before implementation; `.loom` files are documentation mirrors only. Scoped guardrails: fail-closed autonomy thresholds, workspace-signal debt classification, action preflight gates, and three-zero-window cluster resolution.
- **Mills — Full-vision roadmap: substrate-proven → autonomous-merges-at-scale (2026-06-01)**: `126-plan-mills-full-vision-roadmap-2026-06-01.md` — converges the autonomy plumbing (`43`), the empty-MR retro (`44`), the harvester-vm substrate (`45`, now shipped through Slice 2d.5d + kill-test PASSED 2026-06-01), and the v2 swarm (`docs/MILLS.md`). North-star `autonomous_merges_24h` **first ticked 2026-06-24** (fully-unattended bot merges !774/!779/!780, first real-code !788 on 06-25) — the empty-MR gap is **CLOSED on the default k8s substrate**; see the `.loom/126` STATUS RECONCILED banner (2026-06-26). Frontier is now **A3 sustain ≥7 green/day**, not A1/A2. **Riskiest assumption** = routing a real canary's implement+tests to harvester-vm produces a *non-empty* MR that auto-merges (the empty-MR gap close). **Phase A (critical path)**: A1 wire harvester-vm into prod spawn path → A2 first autonomous merge (kill-test) → A3 sustain 7 green. **Phase B**: curated base image, warm pool, k8s health-fallback. **Phase C**: notify webhook URL, workspace-signals council brief, LLM-ranked dispatch. **Phase D**: flip implement/tests defaults + sequential v2 Phase-8.3 flips. Nothing past A starts until one canary merges unattended. Prod policy `configmap-policy.yaml:104` is `stage_substrate: {}` (default-OFF), flip gated on "≥1 auto-merge".
- **Mills — Harvester KubeVirt VM substrate spec + Slice 2e in flight (2026-05-25 → 2026-06-10)**: `45-product-spec-mills-harvester-vm-substrate-2026-05-25.md` (spec) + iteration plans `120` → `125` and `146`. **Slice 0** capacity kill-test PASSED. **Slice 1.5** base VM + plumbing PASSED. **Slice 1** `harvester-vm` devbox backend shipped via `1ed1cd05` + `82c3bc17`. **Slice 2a** policy field shipped 2026-05-26 as `2a9c6c3a`. **Slice 2b** SpawnWorker → SpawnRequest.Substrate shipped 2026-05-27 as `a6979762` ([!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)). **Slice 2c** Substrate over the wire + DEVBOX_BACKEND on pod env shipped 2026-05-27 as `57052ce0` ([!566](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/566)). **Slice 2d** HUD multi-backend init + per-spawn substrate routing shipped 2026-05-27 as `88bc0bcc` ([!568](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/568)). **Slice 2d.5 → 2d.5d** shipped env, SecretMount, home-dir parity, cloud-init Secret delivery, qemu-guest-agent self-heal, ownerRef retry, and the live codex home-parity kill-test. **Slice 2e** (`146`) adds the `cmd/mcp-devbox` backend registry and Harvester SecretResolver wiring. **Next: Slice 2 acceptance** — one end-to-end `mills-canary-harvester-vm` auto-merge with a non-empty MR.
