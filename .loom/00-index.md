...
- **Mills — Harvester KubeVirt VM substrate spec + Slice 2b shipped (2026-05-25 → 2026-05-27)**: `45-product-spec-mills-harvester-vm-substrate-2026-05-25.md` (spec, with Slice 0 + 1.5 + 1 + 2a all marked shipped) + `120-iteration-plan-mills-spawn-substrate-slice2b-2026-05-27.md` (Slice 2b plan). **Slice 0** capacity kill-test PASSED (4 concurrent VMs at 72% CPU / 78% MEM, no node disruption). **Slice 1.5** base VM + plumbing PASSED (DHCP + qemu-guest-agent, 130s cold-boot). **Slice 1** `harvester-vm` devbox backend (~2583 LOC + tests) shipped via commits `1ed1cd05` + `82c3bc17`. **Slice 2a** `pipeline.stage_substrate` policy field shipped 2026-05-26 as `2a9c6c3a`. **Slice 2b** `SpawnWorker.SubstrateFor` closure + `SpawnRequest.Substrate` field + operator wiring shipped 2026-05-27 as `a6979762` ([!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)) — `cmd/loom-mills-operator/main.go` reads `policy.SubstrateForStage` per attempt so hot-reloaded `pipeline.stage_substrate` flips backend at next stage. **Next: Slice 2c** — `HUDSpawnClient.Run` forwards `req.Substrate` to spawn-pod env as `DEVBOX_BACKEND`; multi-backend init in `cmd/mcp-devbox`.
