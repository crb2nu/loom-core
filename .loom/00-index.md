...
- **Mills — Next round of functionality + UX fixes (2026-05-18)**: `42-plan-mills-next-round-fixes-2026-05-18.md` — five-slice plan gated behind a 30-min operator kill-test (HUD↔CLI parity assumption). Slices: 1) HUD parity for 5 day-2 actions (pause/replay/escalate/audit/run); 2) KPI honesty (regression_rate + cost_per_merged_change_usd off proxies, `(proxy)` chips on remainder); 3) sub-tab counts for squads/audit/cross-repo (deferred polish from 23e40ad2); 4) spawn/escalator stabilization debt — backfill regression tests for 6 recent `fix(mills):` commits + state-machine diagram; 5) idle-state one-click "Run council now". Sources: `mcp/skills/mills-ops/SKILL.md`, commit 23e40ad2, `internal/hud/frontend/src/lib/components/Mills/*`.
