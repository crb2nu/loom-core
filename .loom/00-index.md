...
- **Mills — Harvester KubeVirt VM substrate spec + Slice 2c shipped (2026-05-25 → 2026-05-27)**: `45-product-spec-mills-harvester-vm-substrate-2026-05-25.md` (spec, with Slices 0 + 1.5 + 1 + 2a + 2b + 2c all marked shipped) + `120-iteration-plan-mills-spawn-substrate-slice2b-2026-05-27.md` (Slice 2b plan) + `121-iteration-plan-mills-spawn-substrate-slice2c-2026-05-27.md` (Slice 2c plan). **Slice 0** capacity kill-test PASSED (4 concurrent VMs at 72% CPU / 78% MEM, no node disruption). **Slice 1.5** base VM + plumbing PASSED (DHCP + qemu-guest-agent, 130s cold-boot). **Slice 1** `harvester-vm` devbox backend (~2583 LOC + tests) shipped via commits `1ed1cd05` + `82c3bc17`. **Slice 2a** `pipeline.stage_substrate` policy field shipped 2026-05-26 as `2a9c6c3a`. **Slice 2b** `SpawnWorker.SubstrateFor` closure + `SpawnRequest.Substrate` field + operator wiring shipped 2026-05-27 as `a6979762` ([!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)). **Slice 2c** `HUDSpawnClient.Run` propagates `Substrate` over the spawn POST + HUD orchestrator promotes it to `DEVBOX_BACKEND` on the pod env shipped 2026-05-27 as `57052ce0` ([!566](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/566)) — new typed `internal/spawn.Request.Substrate` field + extracted `buildSpawnPodEnv` helper covered by `TestBuildSpawnPodEnv` (table-driven). **Next: Slice 2d** — multi-backend init in `internal/hud/embed.go initSpawnOrchestrator` + per-spawn backend lookup in `runSpawn` so the pod itself runs on `harvester-vm` (not just its in-pod `mcp-devbox`). Slice 2e (mcp-devbox multi-backend init) + Slice 2 acceptance (1 end-to-end canary auto-merge on harvester-vm) follow.
