...
- **Slice 2d (next)**: multi-backend init in `internal/hud/embed.go initSpawnOrchestrator` so `SpawnOrchestrator.backends map[string]backend.Backend` holds both `k8s` + `harvester-vm`; `runSpawn` looks up `req.Substrate` to pick the backend (default: k8s) so the pod IS created on the named substrate.
