...
- **Slice 2d.5c (next)**: home-dir parity on the harvester VM so agent CLIs find the mounted auth files at their expected `$HOME/.…` paths. Three viable approaches: (a) add `agent` user (uid 1000, `HOME=/home/agent`) to the cloud-init `users:` stanza and SSH as `agent` instead of `ubuntu`; (b) symlink `/home/ubuntu` → `/home/agent` (or vice versa) in the cloud-init `runcmd`; (c) remap mountpaths at the orchestrator boundary (rewrite `/home/agent/...` to `/home/ubuntu/...` in `agentSecretMounts` when substrate=harvester-vm). Path (a) matches the K8s pattern most closely and likely unblocks `injectAgentConfig` (spawn.go:990-1050) on harvester-vm for free; decide in 2d.5c planning.
