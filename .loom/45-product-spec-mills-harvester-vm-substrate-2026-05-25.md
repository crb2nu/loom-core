...
- **Slice 2d.5b (next)**: HarvesterVMBackend `SecretMount` propagation. Resolve `agentSecretMounts` (`~/.claude.auth/oauth.json`, `~/.codex/auth.json`, `~/.gcp/sa.json`) via `SecretResolver`-style Secret reads and render as additional cloud-init `write_files` entries with base64-decoded file content at the requested MountPath. Unblocks codex on harvester-vm; nice-to-have for claude-code (OAuth JSON path).
