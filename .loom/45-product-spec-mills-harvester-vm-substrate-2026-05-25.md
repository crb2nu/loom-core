...
- **Slice 2d.5 (next)**: HarvesterVMBackend env+secret propagation. `HarvesterVMBackend.Start` writes a systemd env file via cloud-init that carries `StartOpts.Env` + `StartOpts.SecretEnv` (rendered via the existing K8s Secret lookup) into the VM. Orchestrator's primary agent-exec call at `internal/hud/spawn.go:~738` also threads env through `ExecOpts.Env` so the SSH shell prefix delivers it on every Exec. Unblocks Slice 2 acceptance.
