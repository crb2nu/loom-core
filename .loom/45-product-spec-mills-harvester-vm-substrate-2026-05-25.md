...
- **Slice 2c (next)**: thread `SpawnRequest.Substrate` through `clients.HUDSpawnClient.Run` into `hudSpawnRequestBody.Metadata["DEVBOX_BACKEND"]` (or a dedicated field on `internal/spawn.Request`), and have the HUD spawn orchestrator initialize the matching backend at pod-start time (requires multi-backend initialization in `cmd/mcp-devbox` + a backend lookup in `internal/hud/spawn.go`).
