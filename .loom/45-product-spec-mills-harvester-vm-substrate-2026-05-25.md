...
- **Slice 2d.5c ✅ SHIPPED 2026-05-30 (branch `feat/mills-harvester-home-parity`)**: home-dir parity on the harvester VM. Chose **approach (a)** (agent user + SSH as agent — mirrors the K8s spawn pod's uid-1000 `agent` user with `HOME=/home/agent`), which unblocks `injectAgentConfig` + every `/home/agent` SecretMount path for free. Changes: `buildVMManifest` cloud-init now provisions a `name: agent` user with `home: /home/agent` (was `ubuntu`); `HarvesterVMBackendConfig.SSHUser` defaults to `agent` (new `defaultHarvesterSSHUser = vmAgentUser` const); new `vmAgentUser`/`vmAgentHome` consts. **Planning-time discovery (riskiest-assumption discipline)**: the original "add user to `users:` stanza + chown write_files to `agent:agent`" plan has two non-obvious failure modes — (1) cloud-init's `write-files` module runs *before* `users-groups`, so the agent user doesn't exist when secret files are written; (2) pre-creating the user in `bootcmd` is unsafe because some cloud-init versions skip `ssh_authorized_keys` application for a pre-existing user, which would break SSH login entirely. Robust fix landed: `users:` stanza creates `agent` canonically (reliable SSH key), secret files are written **root-owned** (`vmWriteFilesOwner = root:root`), and a `runcmd` step `chown -R agent:agent /home/agent` (`vmSecretFileOwner`) hands them to the agent after the user exists. Tests: `TestBuildVMManifest_CreatesAgentUserForHomeParity` (asserts `name: agent`, `home: /home/agent`, no `ubuntu` user, root-owned write_files, runcmd chown) + updated owner/SSHUser assertions across `harvester_vm_objects_test.go` / `harvester_vm_test.go`. **Plan**: `.loom/125-iteration-plan-mills-harvester-home-parity-slice2d5c-2026-05-30.md`. **Kill-test status: PASSED 2026-06-01** (codex unblocked on harvester-vm) — after Slice 2d.5d unblocked three live-only bugs the shape-only unit tests missed. The kill-test is the `integration`-tagged `TestHarvesterVMBackend_Integration_CodexHomeParity` (ran 110s on-LAN: VM boots, SSH as `agent`, `/home/agent/.codex.auth/auth.json` owned `agent:agent` + byte-for-byte == the k3s secret, `~/.codex/auth.json` symlink resolves). Evidence: `.loom/local/handoffs/mills-harvester-home-parity-killtest-2026-05-30.md`.

**Slice 2d.5d ✅ SHIPPED 2026-06-01** (the live kill-test surfaced all three; home-parity cloud-init from 2d.5c was confirmed correct via serial console — agent user created + SSH key applied):
1. **Inline cloud-init 2048-byte cap**: the real 4784-byte codex `auth.json` (base64 ≈ 6.4 KB) in `cloudInitNoCloud.userData` was rejected at VM-create (`userdata exceeds 2048 byte limit`). Fix: deliver cloud-init out-of-line via a per-VM Secret + `cloudInitNoCloud.secretRef` (NOTE: the JSON key is `secretRef`, **not** `userDataSecretRef` — only the network ref uses the `…SecretRef` form; an unknown `userDataSecretRef` is silently dropped, then the VM is rejected for "no userdata source").
2. **Base image lacks qemu-guest-agent**: stock-Ubuntu `longhorn-image-mc9ph` has no `qemu-guest-agent`, so KubeVirt never reports the VM's DHCP IP and `Start.waitForVMReady` times out (VMI `Running` but `AgentConnected` never flips). Fix: a self-healing first runcmd `command -v qemu-ga || apt-get install -y qemu-guest-agent` (no-op on the curated image).
3. **PVC/Secret ownerRef race**: the post-create ownerRef update lost an optimistic-concurrency race to the CDI/Longhorn controllers, orphaning a 20Gi PVC per run. Fix: `setOwnerRefWithRetry` (re-Get + `retry.RetryOnConflict`) for both the PVC and cloud-init Secret; verified clean cascade (no orphans). **Kill-test now codified as a one-command harness** — `TestHarvesterVMBackend_Integration_CodexHomeParity` (`internal/devbox/backend/harvester_vm_integration_test.go`, build tag `integration` + `HARVESTER_KUBECONFIG`/`HARVESTER_K3S_KUBECONFIG`/`HARVESTER_VM_STORAGECLASS` env gate; skips cleanly otherwise) drives `Start` with the codex `SecretMount` + k3s `SecretResolver`, SSHs as `agent`, and asserts home parity (`whoami`/`$HOME`), `agent:agent` ownership (runcmd chown), byte-for-byte secret content, and `~/.codex/auth.json` symlink resolution. The next on-LAN agent runs it instead of hand-rolling a harness.
- **Slice 2e 🚧 IN FLIGHT 2026-06-10**: `cmd/mcp-devbox` backend registry + harvester SecretResolver wiring. `newManager` now normalizes backend names, preserves the existing env-selected singleton handler path, records an explicit `backends` map/`backendFor` lookup helper, and opportunistically initializes K8s alongside Harvester so `HarvesterVMBackendConfig.SecretResolver` can resolve SecretEnv/SecretMount artifacts when the companion K8s backend is available. Plan: `.loom/146-iteration-plan-mcp-devbox-multibackend-slice2e-2026-06-10.md`.
