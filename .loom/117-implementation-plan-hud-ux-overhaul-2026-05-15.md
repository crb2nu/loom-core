...
- **Open**: wire `millsStore` + `otelStore` to SSE before bumping their polling cadence. `hud.pipeline` event today carries GitLab CI pipeline state, not Mills internal `pipeline_runs`; a `hud.mills` snapshot would need a new monitor in `internal/hud/monitor/` + `OnRefresh` broadcast in `embed.go`. `hud.otel.*` would need both daemon emission + store wiring.
