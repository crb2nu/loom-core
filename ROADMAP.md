...
- ✅ **Loom Plan Store — first-class, worktree-resilient, cross-agent Plan entity (S0 → S8, 2026-06-20 → 2026-06-24)**
  ...
  - **S7b-α (2026-06-25)**: default-off Mills backlog→Plan backfill — `clients.PlanClient` authors Plans via `agent_plan_create`, `intake.PlanBackfiller` stamps `plan_id` onto un-linked items, gated by `LOOM_MILLS_PLAN_BACKFILL` (off the reconciler hot path). Live canary operator-gated.
  - **S7b-β (2026-06-25)**: default-off born-linked import — `GitLabImporter.Tick` authors a Plan for each newly imported item before its first `Put`, gated by `LOOM_MILLS_PLAN_AUTHORING`; best-effort, off the reconciler hot path.
  - **Remaining (next loops)**: S7b-γ — council `persistOne` inline authoring (shares the same `PlanClient` + `LOOM_MILLS_PLAN_AUTHORING` gate); end-to-end in-pod deploy verification after the loom-core image rebuild + operator redeploy.
...
- 🚧 **Mills harvester-vm substrate (Slices 0 → 2e, 2026-05-25 → 2026-06-10)**
  ...
  - Slice 0 (capacity kill-test, 4 concurrent VMs at 72% CPU / 78% MEM) ✅; Slice 1.5 (base VM + DHCP + qemu-guest-agent, 130s cold-boot) ✅; Slice 1 (`harvester-vm` devbox backend, ~2583 LOC + tests, `1ed1cd05` + `82c3bc17`) ✅; Slice 2a (`pipeline.stage_substrate` policy field, `2a9c6c3a`) ✅; Slice 2b (`SpawnWorker → pipeline.SpawnRequest.Substrate`, `a6979762` [!564](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/564)) ✅; Slice 2c (`HUDSpawnClient` sends `Substrate`; pod env gets `DEVBOX_BACKEND`, `57052ce0` [!566](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/566)) ✅; Slice 2d (HUD multi-backend init + per-spawn substrate routing, `88bc0bcc` [!568](https://gitlab.flexinfer.ai/services/loom-core/-/merge_requests/568)) ✅; Slice 2d.5 → 2d.5d (env/SecretMount/home parity + live codex home-parity kill-test) ✅; Slice 2e (`cmd/mcp-devbox` backend registry + harvester SecretResolver wiring, plan `.loom/146-iteration-plan-mcp-devbox-multibackend-slice2e-2026-06-10.md`) ✅.
  - Remaining: plan_slice spawn/runtime readiness unblocker (`.loom/148-iteration-plan-mills-plan-slice-spawn-readiness-2026-06-10.md`), then Slice 2 acceptance kill-test (1 end-to-end canary auto-merge on harvester-vm; non-empty MR required), then Phase B curated base image / warm-pool hardening.
