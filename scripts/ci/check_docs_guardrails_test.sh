...
# A per-MR changelog fragment satisfies the docs requirement for a code change.
run_case "pkg code with changelog fragment" 0 "pkg/foo/bar.go,changelog.d/feat-x.added.md"
run_case "cmd code with changelog fragment" 0 "cmd/loom/cmd_new.go,changelog.d/2026-07-17-topic.fixed.md"
# A fragment-only change (no code) never requires docs and passes.
run_case "changelog fragment only"       0 "changelog.d/feat-x.added.md"
