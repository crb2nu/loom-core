...
# shellcheck source=scripts/ci/go_package_filter.sh
source "${SCRIPT_DIR}/go_package_filter.sh"
# shellcheck source=scripts/ci/go_build_package_filter.sh
source "${SCRIPT_DIR}/go_build_package_filter.sh"

cd "${REPO_ROOT}"

# Package selection and the final build must evaluate the same build tags.
# Respect an explicit caller value; otherwise match the repository's fast hook
# convention and disable CGo.
