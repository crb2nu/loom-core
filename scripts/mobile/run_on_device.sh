...
if [ -n "$CONFIGURE_URL" ]; then
	# Sanitize for echo: keep mode + url, mask every secret (bearer, CF
	# secret, and the HUD admin token).
	SAFE_URL="$(printf '%s' "$CONFIGURE_URL" | sed -E 's/(bearer=)[^&]+/\1<token>/; s/(cf_secret=)[^&]+/\1<secret>/; s/(admin=)[^&]+/\1<admin>/')"
