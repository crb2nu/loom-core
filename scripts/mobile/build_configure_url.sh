...
# The HUD admin token (HUD_ADMIN_TOKEN) is a SEPARATE secret from the mobile
# bearer above; it authorizes Mills mutations (spin a plan, escalate a
# pipeline) and rides the configure URL as `admin=` so a freshly installed
# build lands able to act, not just read. Optional — omitted when unset.
ADMIN="$(resolve HUD_ADMIN_TOKEN)"
...
if [ -n "${ADMIN}" ]; then
  QUERY="${QUERY}&admin=$(urlencode "${ADMIN}")"
fi
