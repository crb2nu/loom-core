...
> **Note:** Recipes are Tier-1 entries in the [engram tech tree](./engram-schema.md).
> The `agent_recipe_add/recall/list` tools were **removed** — they forwarded verbatim
> to the engram tools. Write recipes with `agent_engram_add` (`tier=1`, no
> `prerequisites`), read them with `agent_engram_recall` / `agent_engram_list`.
> When you need prerequisites or a stronger proof contract, raise the tier.
