...
# FRONTEND_TREE anchors the layers below on the git tree hash of
# internal/hud/frontend (CI passes `git rev-parse HEAD:internal/hud/frontend`).
# BuildKit's own context checksum for the source COPY has produced FALSE cache
# hits: build 20260727-161926 (pipeline 21165) reported the COPY + pnpm build
# steps CACHED and shipped a pre-0055e789 HUD bundle even though 9 frontend
# commits had landed since the cached layer — silently deploying a stale HUD
# to the whole loom-hub fleet. With the tree hash in the layer chain, an
# unchanged tree is the only way to reuse the bundle. Local builds keep the
# default and behave as before.
ARG FRONTEND_TREE=untracked
RUN echo "frontend source tree: ${FRONTEND_TREE}"
